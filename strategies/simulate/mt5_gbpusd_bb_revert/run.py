#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""MT5 GBPUSD Bollinger Band Reversion Strategy Runner."""
...
from strategy_mt5_gbpusd_bb_revert import Mt5GbpusdBbRevertStrategy
...
def run():
    ...
    cerebro.addstrategy(Mt5GbpusdBbRevertStrategy, **params)

    store = bt.stores.BtApiStore(provider="mt5_gateway")
    store.start()

    data = store.getdata(dataname=symbol, timeframe=bt.TimeFrame.Minutes, compression=30)
    ...
    print(f"Starting MT5 GBPUSD BB Revert strategy on {symbol}...")
    cerebro.run()
