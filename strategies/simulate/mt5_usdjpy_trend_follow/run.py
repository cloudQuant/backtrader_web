#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""MT5 USDJPY ADX Trend Following Strategy Runner."""
...
from strategy_mt5_usdjpy_trend_follow import Mt5UsdjpyTrendFollowStrategy
...
def run():
    ...
    cerebro.addstrategy(Mt5UsdjpyTrendFollowStrategy, **params)

    store = bt.stores.BtApiStore(provider="mt5_gateway")
    store.start()

    data = store.getdata(dataname=symbol, timeframe=bt.TimeFrame.Minutes, compression=240)
    ...
    print(f"Starting MT5 USDJPY Trend Follow strategy on {symbol}...")
    cerebro.run()
