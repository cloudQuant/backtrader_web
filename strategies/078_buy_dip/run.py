#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Run script for Buy Dip Strategy."""
...
from strategy_buy_dip import BuyDipStrategy, load_config
...
def run():
    """Run the Buy Dip strategy backtest.

    This function sets up and executes a backtest using the BuyDipStrategy
    with Oracle stock data from 2010-2014. It validates that the strategy
    produces expected performance metrics.
    ...
    assert abs(sharpe_ratio - (1.030465261762291)) < 1e-6, f"Expected sharpe_ratio=1.030465261762291, got {sharpe_ratio}"
    assert abs(annual_return - (0.00017522539339535496)) < 1e-6, f"Expected annual_return=0.00017522539339535496, got {annual_return}"
    assert abs(max_drawdown - 0.08379818319219477) < 1e-6, f"Expected max_drawdown=0.08379818319219477, got {max_drawdown}"
...
if __name__ == "__main__":
    print("=" * 60)
    print("Buy Dip Strategy Backtest")
    print("=" * 60)
    run()
