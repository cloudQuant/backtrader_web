"""Run script for BOLLKDJ Bollinger Bands + KDJ strategy.
...
from strategy_boll_kdj import BOLLKDJStrategy, load_config
...
def run():
    """Run the BOLLKDJ Bollinger Bands + KDJ strategy backtest.

    This function:
    1. Loads configuration from config.yaml
    2. Creates a Cerebro backtest engine
    3. Loads Shanghai stock data
    4. Adds the BOLLKDJStrategy with configured parameters
    ...
    Expected Results:
        ...
        - sell_count: 81
        - total_trades: 66
        - final_value: 75609.19
        - sharpe_ratio: -0.08347216120029895
        - annual_return: -0.012927216297173407
        - max_drawdown: 0.6605349686435283
    """
    # Load configuration
    config = load_config()
    ...
    cerebro.addstrategy(BOLLKDJStrategy,
                       ...
                       boll_period=params.get('boll_period', 53),
                       boll_mult=params.get('boll_mult', 2),
                       kdj_period=params.get('kdj_period', 9),
                       kdj_ma1=params.get('kdj_ma1', 3),
                       kdj_ma2=params.get('kdj_ma2', 3),
                       price_diff=params.get('price_diff', 0.5))
...
if __name__ == "__main__":
    print("=" * 60)
    print("BOLLKDJ Bollinger Bands + KDJ Strategy Run")
    print("=" * 60)
    run()
