#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Stop Loss Order Strategy Runner."""
...
from strategy_stop_order import StopOrderStrategy, ExtendPandasFeed
...
def load_bond_data(filename: str = "bond_index_000000.csv") -> pd.DataFrame:
    """Load convertible bond data"""
    df = pd.read_csv(resolve_data_path(filename))
    df["datetime"] = pd.to_datetime(df["datetime"])
    df = df.set_index("datetime")
    # Keep bond-specific columns for ExtendPandasFeed
    df = df[["open", "high", "low", "close", "volume",
             "pure_bond_value", "convert_value",
...
def run():
    ...
    cerebro.addstrategy(StopOrderStrategy, **params)
    ...
    symbol = data_config.get('symbol', 'bond_index_000000')
    print(f"Loading convertible bond index data: {symbol}.csv...")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Stop Loss Order Strategy Backtest")
    print("=" * 60)
    run()
