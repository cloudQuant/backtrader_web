#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Strategy Runner - Triple Exponential Moving AverageStrategy"""
...
def run():
    ...
    cerebro.addstrategy(TripleEmaStrategy, **params)
    ...
    assert abs(final_value - 100065.31) < 0.01, f"Expected final_value=100065.31, got {final_value}"
    assert abs(sharpe_ratio - (0.3757309556964078)) < 1e-6, f"Expected sharpe_ratio=0.3757309556964078, got {sharpe_ratio}"
    assert abs(annual_return - (0.00013090572633809417)) < 1e-6, f"Expected annual_return=0.00013090572633809417, got {annual_return}"
    assert abs(max_drawdown - 0.09539203307003034) < 1e-6, f"Expected max_drawdown=0.09539203307003034, got {max_drawdown}"
