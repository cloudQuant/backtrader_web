#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Strategy Runner - HMACrossoverMoving AverageStrategy"""
...
def run():
    ...
    cerebro.addstrategy(HmaCrossoverStrategy, **params)
    ...
    print(f"Data loaded from {datetime.datetime(2010, 1, 1)} to {datetime.datetime(2014, 12, 31)}")
    ...
    assert abs(sharpe_ratio - (0.5100011168586044)) < 1e-6, f"Expected sharpe_ratio=0.5100011168586044, got {sharpe_ratio}"
    assert abs(annual_return - (0.00016323774473640581)) < 1e-6, f"Expected annual_return=0.00016323774473640581, got {annual_return}"
    assert abs(max_drawdown - 0.10334345093914488) < 1e-6, f"Expected max_drawdown=0.10334345093914488, got {max_drawdown}"
