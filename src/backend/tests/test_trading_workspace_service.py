...
def test_build_snapshot_recalculates_stale_local_net_pnl_with_local_asset_spec(
    monkeypatch,
    tmp_path,
):
    """Strategy snapshots should use local exchange metadata when runtime specs are absent."""
    log_dir = tmp_path / "logs"
    log_dir.mkdir()
    unit = SimpleNamespace(
        workspace_id="ws-1",
    ...
    assert snapshot["long_market_value"] == 1_500_300.0
    assert snapshot["position_pnl"] == 265.5
    assert snapshot["asset_spec_source"] == "local_futures_commission"
    assert snapshot["positions"][0]["commission"] == 34.5
    assert snapshot["positions"][0]["position_pnl"] == 265.5
