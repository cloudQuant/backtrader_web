...
class TestReqDocsMigrationService:
    ...
    async def test_migrate_structured_entities_preserves_links(self):
        ...
        async with async_session_maker() as session:
            ...
            messages = (await session.execute(select(ChatMessage).order_by(ChatMessage.created_at.asc()))).scalars().all()
    ...
    async def test_migration_is_idempotent_by_reqdocs_source_ids(self):
        ...
        async with async_session_maker() as session:
            kb_count = (
                await session.execute(
                    select(func.count()).select_from(KnowledgeBase).where(KnowledgeBase.name == '幂等测试库')
                )
            ).scalar_one()
            doc_count = (
                await session.execute(
            ...
            conv_count = (
                await session.execute(
                    select(func.count()).select_from(ChatConversation).where(ChatConversation.title == '问答A')
                )
            ).scalar_one()
            msg_count = (
                await session.execute(
                    select(func.count()).select_from(ChatMessage).where(ChatMessage.content == '问题A')
                )
            ).scalar_one()
