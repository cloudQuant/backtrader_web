...
def _action_summary(
    records: list[StockSignalPrediction], action: str, horizon: int
) -> dict[str, Any]:
    ...
    returns = [
        value for record in scorable if (value := _return_for_horizon(record, horizon)) is not None
    ]
    excess = [
        value for record in scorable if (value := _excess_for_horizon(record, horizon)) is not None
    ]
    return {
        "action": action,
...
def _confidence_bins(records: list[StockSignalPrediction], horizon: int) -> list[dict[str, Any]]:
    ...
    for lower, upper, label in bins:
        selected = [
            ...
            if lower <= float(record.confidence_score or 0.0) < upper
            and _success(record, horizon) is not None
        ]
...
def build_performance_summary(
    ...
    items = list(records)
    action_summaries = [
        _action_summary(items, action, horizon) for action in ("BUY", "SELL", "WATCH")
    ]
    ...
    return {
        ...
        "actioned_success_rate": actioned_successes / actioned_scorable
        if actioned_scorable
        else None,
