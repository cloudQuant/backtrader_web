...
class ReqDocsMigrationService:
    ...
    @classmethod
    def _message_keys(cls, conversation_id: Any, role: Any, content: Any, created_at: Any) -> set[tuple]:
    ...
    @classmethod
    def _persist_source_file(cls, document_id: int, source_info: dict[str, Any] | None) -> str | None:
        if not source_info:
            return None
        file_data = source_info.get('data')
        filename = source_info.get('filename')
        if not isinstance(file_data, (bytes, bytearray)) or not isinstance(filename, str) or not filename:
            return None
    ...
    async def migrate_from_structured_data(self, owner_id: str, payload: dict[str, Any]) -> dict[str, int]:
        ...
        async with async_session_maker() as session:
            existing_kbs = (
                await session.execute(select(KnowledgeBase).where(KnowledgeBase.owner_id == owner_id))
            ).scalars().all()
            kb_by_reqdocs_id = {
                int((kb.settings or {}).get('reqdocs_project_id')): kb
                for kb in existing_kbs
                if isinstance(kb.settings, dict) and (kb.settings or {}).get('reqdocs_project_id') is not None
            }
            ...
            doc_by_reqdocs_id = {
                ...
                if isinstance(doc.metadata_json, dict) and (doc.metadata_json or {}).get('reqdocs_document_id') is not None
            }

            existing_conversations = (await session.execute(select(ChatConversation))).scalars().all()
            conv_by_reqdocs_id = {
                int((conv.settings or {}).get('reqdocs_conversation_id')): conv
                for conv in existing_conversations
                if isinstance(conv.settings, dict) and (conv.settings or {}).get('reqdocs_conversation_id') is not None
            }
            ...
            existing_message_keys = {
                ...
                for key in self._message_keys(msg.conversation_id, msg.role, msg.content, msg.created_at)
            }
            ...
            for project_id, docs in docs_by_project.items():
                ...
                for document in docs:
                    ...
                    if new_parent_id:
                        target = (
                            await session.execute(select(KBDocument).where(KBDocument.id == new_doc_id))
                        ).scalar_one()
            ...
            for message in payload.get('kb_chat_messages', []):
                ...
                existing_message_keys.update(
                    self._message_keys(conv_id, message['role'], message['content'], message.get('created_at'))
                )
        ...
        return {
            ...
            'messages': sum(1 for msg in payload.get('kb_chat_messages', []) if msg.get('conversation_id') in conversation_id_map),
        }
    ...
    def read_reqdocs_structured_data(self) -> dict[str, Any]:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute('SELECT id, name, description, created_at, updated_at, current_version FROM projects')
                projects = list(cur.fetchall())
                cur.execute('SELECT id, title, type, status, project_id, parent_id, path, is_folder, sort_order, current_version, created_at, updated_at FROM documents')
                documents = list(cur.fetchall())
                cur.execute('SELECT id, knowledge_base_id, title, created_by, created_at, updated_at FROM kb_conversations')
                conversations = list(cur.fetchall())
                cur.execute('SELECT id, conversation_id, role, content, model, created_at FROM kb_chat_messages')
