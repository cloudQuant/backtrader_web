...
class LiveTradingManager:
    ...
    def __init__(self) -> None:
        ...
        self._pending_attested_paper_instances: dict[str, str] = {}
        # A prepared live handoff is never restartable through a public
        # manager route.  Its one-shot activation token is generated by the
        # server-owned research workflow and is intentionally neither a unit
        # field nor an instance parameter.
        self._pending_attested_live_handoff_instances: dict[str, str] = {}
        self._pending_managed_workspace_instances: set[str] = set()
        # This protects the in-memory, one-start paper-runtime capability
        # together with publication of a newly created instance.  A file lock
        # alone does not serialize reads of this process-local dictionary.
        self._paper_runtime_capability_lock = threading.RLock()
    ...
    def add_instance(
        ...
        *,
        ...
        server_attested_live_handoff_activation_token: str | None = None,
    ) -> InstanceData:
        # Keep the persistent instance publication and the private one-start
        # capability in a single local + interprocess critical section.  A
        # direct start may read the JSON instance before the workspace
        # transaction commits its unit mapping, so it must never observe a
        # newly visible paper instance without its pending capability.
        with self._paper_runtime_capability_lock:
            with _instance_store_lock():
                created = cast(
                    InstanceData,
                ...
                if server_attested_paper_runtime_digest is not None:
                    self._install_paper_runtime_start_capability(
                        str(created["id"]),
                        server_attested_paper_runtime_digest,
                    )
                if server_attested_live_handoff_activation_token is not None:
                    self._install_live_handoff_start_capability(
                        str(created["id"]),
                        server_attested_live_handoff_activation_token,
                    )
        return created

    def _install_paper_runtime_start_capability(
        self,
        instance_id: str,
        runtime_snapshot_digest: str,
    ) -> None:
        """Validate and install a pending server-only paper start capability.

        Callers must hold ``_paper_runtime_capability_lock``.  Keeping this
        separate from the public helper avoids a nested file-lock acquisition
        while ``add_instance`` publishes the instance and capability together.
        """
        normalized_instance_id = str(instance_id or "").strip()
        normalized_digest = str(runtime_snapshot_digest or "").strip()
        if (
        ...
        self._pending_attested_paper_instances[normalized_instance_id] = normalized_digest

    @staticmethod
    def new_live_handoff_runtime_activation_token() -> str:
        """Create an opaque, one-shot token for a server-only live launch."""
        return secrets.token_urlsafe(32)

    def _install_live_handoff_start_capability(
        self,
        instance_id: str,
        activation_token: str,
    ) -> None:
        """Install a one-shot live-handoff activation token under the publish lock."""
        normalized_instance_id = str(instance_id or "").strip()
        normalized_token = str(activation_token or "").strip()
        if not normalized_instance_id or len(normalized_token) < 32:
            raise ValueError("AI_RESEARCH_LIVE_HANDOFF_PROVENANCE_INVALID")
        self._pending_attested_live_handoff_instances[normalized_instance_id] = normalized_token

    def attest_paper_runtime_start(self, instance_id: str, runtime_snapshot_digest: str) -> None:
        """Grant one internal, post-materialization paper-runtime start.

        The capability is held only in manager memory and is installed by the
        workspace service after it revalidates an HMAC anchor and recreates
        the isolated runtime directory.  It is deliberately not read from
        instance parameters, API input, or persisted JSON state.
        """
        with self._paper_runtime_capability_lock:
            with _instance_store_lock():
                self._install_paper_runtime_start_capability(
    ...
    def attest_live_handoff_runtime_start(self, instance_id: str, activation_token: str) -> None:
        """Grant an existing prepared live runtime one server-only launch token."""
        with self._paper_runtime_capability_lock:
            with _instance_store_lock():
                self._install_live_handoff_start_capability(instance_id, activation_token)

    def attest_managed_workspace_runtime_start(self, instance_id: str) -> None:
        """Permit this process to finish a newly-created workspace launch.

        The unit-to-instance database link is committed after ``start_units``
        returns.  Other workers must reject the uncommitted managed runtime;
        only the workspace service that created it receives this short-lived,
        in-memory capability.  It is deliberately unavailable to request
        payloads and is removed after the first launch attempt.
        """
        normalized_instance_id = str(instance_id or "").strip()
        if not normalized_instance_id:
            raise ValueError("WORKSPACE_RUNTIME_START_PROVENANCE_INVALID")
        with self._paper_runtime_capability_lock:
            with _instance_store_lock():
                instance = _load_instances().get(normalized_instance_id)
                runtime_dir = (
                    str(instance.get("runtime_dir") or "").strip()
                    if isinstance(instance, dict)
                    else ""
                )
                from app.services import workspace_unit_runtime

                if not workspace_unit_runtime.is_workspace_unit_runtime_dir(runtime_dir):
                    raise ValueError("WORKSPACE_RUNTIME_START_PROVENANCE_INVALID")
                self._pending_managed_workspace_instances.add(normalized_instance_id)
        ...
        *,
        ...
        async with self._instance_op_lock:
            with business_span(
                ...
                # The manager is shared by /live-trading and /simulation,
                # including their start-all routes.  Check the database unit
                # mapping at this single launch boundary so neither API can
                # bypass an AI-research paper unit's provenance or locks.
                from app.services import workspace_unit_runtime
                from app.services.workspace.units import (
                    AIStrategyResearchLiveHandoffRuntimeStartError,
                    AIStrategyResearchPaperRuntimeStartError,
                    assert_ai_research_live_handoff_runtime_start_allowed,
                    assert_ai_research_paper_runtime_start_allowed,
                    get_ai_research_live_handoff_unit_for_instance,
                )

                # Read the private capability under the same critical section
                # that publishes a new instance in ``add_instance``.  Without
                # this, a direct start can observe the JSON record after it is
                # saved but before the in-memory capability is installed, then
                # fall through to an uncommitted DB mapping.
                with self._paper_runtime_capability_lock:
                    with _instance_store_lock():
                        instances = _load_instances()
                        current = instances.get(str(instance_id))
                        runtime_dir = (
                            str(current.get("runtime_dir") or "").strip()
                            if isinstance(current, dict)
                            else ""
                        )
                        pending_digest = self._pending_attested_paper_instances.get(
                            str(instance_id)
                        )
                        pending_live_handoff_token = (
                            self._pending_attested_live_handoff_instances.get(str(instance_id))
                        )
                        pending_managed_workspace_runtime = str(instance_id) in (
                            self._pending_managed_workspace_instances
                        )
                supplied_live_handoff_token = str(
                    server_attested_live_handoff_activation_token or ""
                ).strip()
                valid_live_handoff_capability = bool(
                    pending_live_handoff_token
                    and supplied_live_handoff_token
                    and hmac.compare_digest(
                        pending_live_handoff_token,
                        supplied_live_handoff_token,
                    )
                )
                if pending_live_handoff_token is not None and not valid_live_handoff_capability:
                    # The process-local token is deliberately not inferable
                    # from a prepared unit, instance JSON, or public API
                    # request. A competing public start must not consume or
                    # reuse this activation window.
                ...
                # Once the workspace transaction has committed, every worker
                # can resolve the reserved live handoff from the database. It
                # is still forbidden to start it directly: only a matching
                # one-shot capability may cross this manager boundary.
                live_handoff_unit = (
                    await get_ai_research_live_handoff_unit_for_instance(
                        instance_id,
                        user_id,
                    )
                    if (
                        user_id is not None
                        or enforce_ai_research_paper_runtime
                        or pending_live_handoff_token is not None
                    )
                    else None
                )
                if live_handoff_unit is not None:
                    if not valid_live_handoff_capability or user_id is None:
                        raise AIStrategyResearchLiveHandoffRuntimeStartError(
                            "AI_RESEARCH_LIVE_HANDOFF_SERVER_ACTIVATION_REQUIRED"
                        )
                    await assert_ai_research_live_handoff_runtime_start_allowed(
                        live_handoff_unit,
                        user_id,
                    )
                # A workspace launch must materialize its isolated files and
                # then pass a one-start digest before its DB transaction can
                # persist the refreshed anchor. Do not re-read stale database
                # state in that small window. Direct starts have no pending
                # capability and must prove the persisted HMAC anchor.
                database_state = (
                    None
                    if pending_digest is not None
                    or pending_live_handoff_token is not None
                    or pending_managed_workspace_runtime
                    or (user_id is None and not enforce_ai_research_paper_runtime)
                    else await assert_ai_research_paper_runtime_start_allowed(
                        instance_id,
                        user_id,
                    )
                )
                # A managed unit runtime with no committed unit mapping is
                # never an ordinary live instance.  This catches another
                # Gunicorn worker observing the instance JSON between its
                # server-side publication and the outer workspace commit, and
                # also rejects any orphan left after an aborted/deleted paper
                # workflow.  A normal mapped workspace unit returns ``False``
                # from the assertion helper and remains unaffected.
                if (
                    pending_digest is None
                    and pending_live_handoff_token is None
                    and not pending_managed_workspace_runtime
                    and database_state is None
                    and workspace_unit_runtime.is_workspace_unit_runtime_dir(runtime_dir)
                ):
                    raise AIStrategyResearchPaperRuntimeStartError(
                        "AI_RESEARCH_PAPER_RUNTIME_UNIT_MAPPING_REQUIRED"
                    )
                database_digest = database_state if isinstance(database_state, str) else None
                runtime_snapshot_digest = pending_digest or database_digest
                server_attested = runtime_snapshot_digest is not None
                if server_attested:
                    with _instance_store_lock():
                        instances = _load_instances()
                        current = instances.get(str(instance_id))
                        if isinstance(current, dict):
                            current[_SERVER_ATTESTED_PAPER_RUNTIME_FIELD] = True
                            current[_SERVER_ATTESTED_PAPER_RUNTIME_DIGEST_FIELD] = (
                                runtime_snapshot_digest
                            )
                    ...
                    if (
                        ...
                        with self._paper_runtime_capability_lock:
                            with _instance_store_lock():
                                instances = _load_instances()
                                current = instances.get(str(instance_id))
                                if server_attested and isinstance(current, dict):
                                    current.pop(_SERVER_ATTESTED_PAPER_RUNTIME_FIELD, None)
                                    current.pop(_SERVER_ATTESTED_PAPER_RUNTIME_DIGEST_FIELD, None)
                                    instances[str(instance_id)] = current
                                    _save_instances(instances)
                                self._pending_attested_paper_instances.pop(str(instance_id), None)
                                self._pending_attested_live_handoff_instances.pop(
        ...
        *,
        ...
        async with self._instance_op_lock:
            with business_span(
                ...
                # A direct stop leaves the mutable workspace unit untouched,
                # so a stale signed paper review could otherwise keep counting
                # wall-clock time after the process has been stopped.  Resolve
                # the unit mapping at the shared manager boundary before any
                # process or instance-store state changes.  Only the internal
                # paper-review failure workflow may opt into this stop.
                from app.services.workspace.units import (
                    assert_ai_research_paper_runtime_stop_allowed,
                )

                await assert_ai_research_paper_runtime_stop_allowed(
                    instance_id,
                    user_id,
        ...
        if user_id is not None or enforce_ai_research_paper_runtime:
            ...
            async def start_owned_instance(instance_id: str) -> StartResult:
                return await self.start_instance(
                    instance_id,
                    user_id=user_id,
                    enforce_ai_research_paper_runtime=enforce_ai_research_paper_runtime,
                )
    ...
    async def stop_all(
        ...
        *,
        allow_server_owned_ai_research_stop: bool = False,
        skip_server_owned_ai_research_paper_runtimes: bool = False,
    ) -> dict[str, StopResult]:
        if skip_server_owned_ai_research_paper_runtimes:
            if allow_server_owned_ai_research_stop:
                raise ValueError("AI_RESEARCH_PAPER_RUNTIME_STOP_POLICY_INVALID")
            return await self._stop_all_excluding_server_owned_paper_runtimes(user_id)

        if not allow_server_owned_ai_research_stop:
            # Keep the public batch operation atomic from the caller's point
            # of view: find protected running units before stopping any
            # ordinary sibling.  The per-instance guard remains below for
            # direct stops and races.
            from app.services.workspace.units import (
                assert_ai_research_paper_runtime_stop_allowed,
            )
            ...
            for instance_id, instance in instances.items():
                if user_id is not None and instance.get("user_id") != user_id:
                    continue
        ...
        if user_id is not None or allow_server_owned_ai_research_stop:
            ...
            async def stop_owned_instance(instance_id: str) -> StopResult:
                return await self.stop_instance(
                    instance_id,
                    user_id=user_id,
                    allow_server_owned_ai_research_stop=allow_server_owned_ai_research_stop,
                )
    ...
    async def _stop_all_excluding_server_owned_paper_runtimes(
        self,
        user_id: str | None,
    ) -> dict[str, StopResult]:
        """Stop ordinary scheduler workloads without stopping attested paper runs.

        Market-close automation is system-owned and spans users.  It may not
        invoke the public all-or-nothing stop path when one attested AI paper
        runtime is present: doing so would leave unrelated ordinary strategies
        running for the entire session.  Protected paper runtimes are instead
        explicitly skipped, while every ordinary stop goes through the usual
        per-instance guard to close a concurrent attachment race.
        """
        ...
        for instance_id, instance in instances.items():
            ...
            try:
                await self.stop_instance(instance_id, user_id=user_id)
            except AIStrategyResearchPaperRuntimeStopError:
                details.append(
                    {
                        "id": str(instance_id),
                        "strategy_id": strategy_id,
                        "result": "skipped_server_owned_ai_research_paper_runtime",
                    }
                )
            except Exception as exc:
                failed += 1
