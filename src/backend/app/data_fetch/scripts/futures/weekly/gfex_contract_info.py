...
class FuturesContractInfoGfex(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "FUTURES_CONTRACT_INFO_GFEX"
        self.create_table_sql = r"""
                                CREATE TABLE `FUTURES_CONTRACT_INFO_GFEX` (
                                  `R_ID` VARCHAR(64) NOT NULL COMMENT '主键ID',
                                  `REFERENCE_CODE` VARCHAR(50) DEFAULT 'GFEX_CONTRACT_INFO' COMMENT '参考编码',
                                  `REFERENCE_NAME` VARCHAR(100) DEFAULT '广州期货交易所合约信息' COMMENT '参考名称',
                                  ...
                                  `PRICE_TICK` INT COMMENT '最小变动单位(元/吨)',
                                  ...
                                  `DATA_SOURCE` VARCHAR(50) DEFAULT '广州期货交易所' COMMENT '数据来源',
                                  ...
                                  PRIMARY KEY (`R_ID`),
                                  UNIQUE KEY `IDX_GFEX_CONTRACT_UNIQUE` (`CONTRACT_CODE`, `TRADE_DATE`),
                                  KEY `IDX_GFEX_CONTRACT_CODE` (`CONTRACT_CODE`),
                                  KEY `IDX_GFEX_TRADE_DATE` (`TRADE_DATE`),
                                  KEY `IDX_GFEX_PRODUCT_CATEGORY` (`PRODUCT_CATEGORY`),
                                  KEY `IDX_GFEX_DELIVERY_MONTH` (`DELIVERY_MONTH`),
                                  KEY `IDX_GFEX_IS_ACTIVE` (`IS_ACTIVE`),
                                  KEY `IDX_GFEX_EXPIRY_DATE` (`EXPIRY_DATE`),
                                  KEY `IDX_GFEX_PRODUCT_NAME` (`PRODUCT_NAME`)
                                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='广州期货交易所合约信息表';

                                """

    def run(self):
        """
        Fetches and stores the latest daily contract information from the Guangzhou Futures Exchange (GFEX).
        """
        # 如果当前表不存在，创建一个新的表
        if not self.table_exists(self.table_name):
            self.create_table(self.create_table_sql)

        self.logger.info("Starting GFEX contract info update.")
        table_name = "FUTURES_CONTRACT_INFO_GFEX"
        ...
        try:
            ...
            df = self.fetch_ak_data("futures_contract_info_gfex")
            time.sleep(2)  # Be respectful to the server

            if df.empty:
                self.logger.warning("No data returned from GFEX.")
                return
            ...
            df["REFERENCE_CODE"] = "GFEX_CONTRACT_INFO"
            df["REFERENCE_NAME"] = "广州期货交易所合约信息"
            ...
            self.logger.error(
                f"An error occurred during the GFEX update process: {e}", exc_info=True
            )
        finally:
            self.disconnect_db()
...
if __name__ == "__main__":
    data_updater = FuturesContractInfoGfex()
    data_updater.run()
