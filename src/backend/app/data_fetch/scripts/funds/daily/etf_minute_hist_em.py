...
class EtfMinuteHistEm(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        # 统一的ETF分钟数据表
        self.table_name = "ETF_MINUTE_HIST_EM"

        # 统一表结构 (支持1分钟和5分钟数据)
        self.create_table_sql = r"""
                                CREATE TABLE `ETF_MINUTE_HIST_EM` (
                                  `R_ID` VARCHAR(64) NOT NULL COMMENT '主键ID',
                                  `REFERENCE_CODE` VARCHAR(50) DEFAULT 'ETF_MINUTE_HIST' COMMENT '参考编码',
                                  `REFERENCE_NAME` VARCHAR(100) DEFAULT 'ETF基金分时行情表(东方财富)' COMMENT '参考名称',
                                  ...
                                  PRIMARY KEY (`R_ID`),
                                  UNIQUE KEY `IDX_ETF_MINUTE_UNIQUE` (`ETF_CODE`, `PERIOD`, `ADJUST_TYPE`, `TRADE_DATETIME`),
                                  KEY `IDX_ETF_CODE_PERIOD` (`ETF_CODE`, `PERIOD`),
                                  KEY `IDX_ETF_CODE_PERIOD_ADJUST` (`ETF_CODE`, `PERIOD`, `ADJUST_TYPE`),
                                  KEY `IDX_TRADE_DATETIME` (`TRADE_DATETIME`),
                                  KEY `IDX_TRADE_DATE` (`TRADE_DATE`),
                                  KEY `IDX_PERIOD` (`PERIOD`),
                                  KEY `IDX_IS_ACTIVE` (`IS_ACTIVE`)
                                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='ETF基金分时行情表(东方财富)';
                                """

    def get_latest_datetime_from_table(self, etf_code, period, adjust_type=""):
        """
        获取数据库中指定ETF代码和周期的最新时间
        ...
        try:
            ...
            WHERE ETF_CODE = %s AND PERIOD = %s AND ADJUST_TYPE = %s AND IS_ACTIVE = 1
            """
            params = (etf_code, period, adjust_type)
    ...
    def get_etf_codes(self):
        ...
        try:
            # 从实时行情表获取ETF代码列表
            if self.table_exists("ETF_REALTIME_QUOTE_EM"):
                etf_codes = self.get_one_column_from_table("ETF_CODE", "ETF_REALTIME_QUOTE_EM")
                if etf_codes:
                    self.logger.info(f"从数据库获取到{len(etf_codes)}个ETF代码")
                    return sorted(etf_codes)

            # 如果没有实时行情表，使用akshare获取
            self.logger.info("从akshare获取ETF代码列表")
            df = self.fetch_ak_data("fund_etf_spot_em")
            if not df.empty and "代码" in df.columns:
                etf_codes = df["代码"].dropna().unique().tolist()
                self.logger.info(f"从akshare获取到{len(etf_codes)}个ETF代码")
                return sorted(etf_codes)

            # 备用代码列表
            self.logger.warning("使用备用ETF代码列表")
            return ["159919", "510300", "512100", "513500", "515000"]

        except Exception as e:
            self.logger.error(f"获取ETF代码列表失败: {e}")
            return ["159919", "510300", "512100", "513500", "515000"]

    def fetch_etf_minute_data(self, etf_code, period, adjust="", start_date=None, end_date=None):
        """
        获取ETF分钟数据
        ...
        try:
            ...
            self.logger.info(
                f"获取 {etf_code} {period}分钟数据, 复权类型: {adjust}, 时间范围: {start_date} - {end_date}"
            )
            ...
            if df.empty:
                self.logger.warning(f"未获取到 {etf_code} 的数据")
                return pd.DataFrame()

            return df

        except Exception as e:
            self.logger.error(f"获取 {etf_code} {period}分钟数据失败: {e}")
            return pd.DataFrame()

    def process_minute_data(self, df, etf_code, period, adjust_type=""):
        """统一处理分钟数据"""
        if df.empty:
            return df
        ...
        try:
            ...
            df["REFERENCE_CODE"] = "ETF_MINUTE_HIST"
            df["REFERENCE_NAME"] = "ETF基金分时行情表(东方财富)"
    ...
    def update_etf_minute_data(self, etf_codes=None):
        ...
        for i, etf_code in enumerate(etf_codes, 1):
            try:
                self.logger.info(f"处理 {etf_code} ({i}/{total_count})")

                # 1. 处理1分钟数据
                self.logger.info(f"处理 {etf_code} 1分钟数据")

                # 获取最新时间
                latest_time_1min = self.get_latest_datetime_from_table(etf_code, "1", "")
                ...
                # 获取1分钟数据
                df_1min = self.fetch_etf_minute_data(
                    etf_code=etf_code,
                    period="1",
                    adjust="",  # 1分钟数据不复权
                ...
                if not df_1min.empty:
                    df_1min_processed = self.process_minute_data(df_1min, etf_code, "1", "")
                    if not df_1min_processed.empty:
                        ...
                        self.logger.info(
                            f"{etf_code} 1分钟数据保存成功，共 {len(df_1min_processed)} 条"
                        )
                else:
                    self.logger.warning(f"{etf_code} 1分钟数据为空")

                # 2. 处理5分钟数据（前复权）
                self.logger.info(f"处理 {etf_code} 5分钟数据(前复权)")

                adjust_type = "qfq"
                latest_time_5min = self.get_latest_datetime_from_table(etf_code, "5", adjust_type)
                ...
                # 获取5分钟数据
                df_5min = self.fetch_etf_minute_data(
                    etf_code=etf_code,
                    period="5",
                    adjust=adjust_type,
                ...
                if not df_5min.empty:
                    df_5min_processed = self.process_minute_data(
                        df_5min, etf_code, "5", adjust_type
                    )
                    if not df_5min_processed.empty:
                        ...
                        self.logger.info(
                            f"{etf_code} 5分钟数据保存成功，共 {len(df_5min_processed)} 条"
                        )
                else:
                    self.logger.warning(f"{etf_code} 5分钟数据为空")
                ...
                self.logger.error(f"处理 {etf_code} 失败: {e}")
                continue
