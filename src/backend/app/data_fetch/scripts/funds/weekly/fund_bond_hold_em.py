...
class FundBondHoldEm(AkshareToMySql):
    ...
    def run(
        ...
        try:
            ...
            total_count = 0
            for code in codes:
                df = self.fetch_bond_hold_data(code, year)
                if df is None or df.empty:
                    continue
                ...
                if success:
                    total_count += len(df)
                    self.logger.info(f"基金 {code} 债券持仓数据更新完成")
                else:
                    total_success = False
                    self.logger.warning(f"基金 {code} 债券持仓数据更新失败")

            self.logger.info(f"基金债券持仓数据更新完成，共处理{total_count}条数据")
