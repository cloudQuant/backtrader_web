...
class FundHkRankEm(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        # 香港基金排行数据表
        self.table_name = "FUND_HK_RANK_EM"

        # 表结构定义
        self.create_table_sql = """
                            CREATE TABLE `FUND_HK_RANK_EM` (
                              `R_ID` VARCHAR(64) NOT NULL COMMENT '主键ID',
                              `REFERENCE_CODE` VARCHAR(50) DEFAULT 'FUND_HK_RANK' COMMENT '参考编码',
                              `REFERENCE_NAME` VARCHAR(100) DEFAULT '香港基金排行数据表(东方财富)' COMMENT '参考名称',

                              -- 基金信息
                              `FUND_CODE` VARCHAR(20) NOT NULL COMMENT '基金代码',
                              `FUND_NAME` VARCHAR(200) COMMENT '基金简称',
                              `CURRENCY` VARCHAR(20) COMMENT '币种',
                              `HK_FUND_CODE` VARCHAR(50) COMMENT '香港基金代码',
                              ...
                              -- 其他信息
                              `IS_PURCHASABLE` TINYINT(1) DEFAULT 0 COMMENT '是否可购买(1:是,0:否)',
                              ...
                              PRIMARY KEY (`R_ID`),
                              UNIQUE KEY `IDX_FUND_HK_RANK_UNIQUE` (`FUND_CODE`, `TRADE_DATE`),
                              KEY `IDX_FUND_CODE` (`FUND_CODE`),
                              KEY `IDX_TRADE_DATE` (`TRADE_DATE`),
                              KEY `IDX_HK_FUND_CODE` (`HK_FUND_CODE`),
                              KEY `IDX_IS_PURCHASABLE` (`IS_PURCHASABLE`),
                              KEY `IDX_IS_ACTIVE` (`IS_ACTIVE`)
                            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='香港基金排行数据表(东方财富)';
                            """
    ...
    def parse_purchasable(self, value: Any) -> int:
        """解析是否可购买字段"""
        if pd.isna(value) or value in ("", "--", "-"):
            return 0
        return 1 if str(value).strip() == "可购买" else 0

    def fetch_hk_rank_data(self) -> pd.DataFrame | None:
        """
        获取香港基金排行数据

        Returns:
            pd.DataFrame: 处理后的香港基金排行数据
        """
        try:
            self.logger.info("开始获取香港基金排行数据...")

            # 获取数据
            df = self.fetch_ak_data("fund_hk_rank_em")

            if df is None or df.empty:
                self.logger.warning("未获取到香港基金排行数据")
                return None

            return df

        except Exception as e:
            self.logger.error(f"获取香港基金排行数据失败: {e}", exc_info=True)
            return None

    def process_hk_rank_data(self, df: pd.DataFrame) -> pd.DataFrame | None:
        """处理香港基金排行数据"""
        if df is None or df.empty:
            return None
        ...
        try:
            ...
            # 处理是否可购买字段
            if "IS_PURCHASABLE_STR" in df.columns:
                df["IS_PURCHASABLE"] = df["IS_PURCHASABLE_STR"].apply(self.parse_purchasable)
            ...
            # 添加系统字段
            df["REFERENCE_CODE"] = "FUND_HK_RANK"
            df["REFERENCE_NAME"] = "香港基金排行数据表(东方财富)"
            df["IS_ACTIVE"] = 1
            df["DATA_SOURCE"] = "东方财富"
            ...
            self.logger.info(f"成功处理{len(df)}条香港基金排行数据")
            return df

        except Exception as e:
            self.logger.error(f"处理香港基金排行数据失败: {e}", exc_info=True)
            return None

    def save_hk_rank_data(self, df: pd.DataFrame) -> bool:
        """
        保存香港基金排行数据到数据库
        ...
        """
        if df is None or df.empty:
            self.logger.warning("没有香港基金排行数据需要保存")
            return False
        ...
        if df.empty:
            self.logger.info("没有新的香港基金排行数据需要保存")
            return True
    ...
    def _get_existing_ids(self) -> set:
        ...
        try:
            self.connect_db()
            sql = "SELECT R_ID FROM FUND_HK_RANK_EM WHERE IS_ACTIVE = 1"
    ...
    def run(self):
        ...
        try:
            self.logger.info("=" * 50)
            self.logger.info("开始执行香港基金排行数据更新")

            # 创建表（如果不存在）
            self.create_table_if_not_exists()

            # 获取数据
            df = self.fetch_hk_rank_data()

            if df is None or df.empty:
                self.logger.error("未获取到有效的香港基金排行数据")
                return False

            # 处理数据
            processed_df = self.process_hk_rank_data(df)

            if processed_df is None or processed_df.empty:
                self.logger.error("处理香港基金排行数据失败")
                return False

            # 保存数据
            success = self.save_hk_rank_data(processed_df)

            if success:
                self.logger.info(f"成功保存{len(processed_df)}条香港基金排行数据")
            else:
                self.logger.error("保存香港基金排行数据失败")
