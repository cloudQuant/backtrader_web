...
class FundIndustryAllocationEm(AkshareToMySql):
    ...
    def run(
        ...
        try:
            ...
            total_count = 0
            for code in codes:
                df = self.fetch_industry_allocation_data(code, year)
                if df is None or df.empty:
                    continue
                ...
                if success:
                    total_count += len(df)
                    self.logger.info(f"基金 {code} 行业配置数据更新完成")
                else:
                    total_success = False
                    self.logger.warning(f"基金 {code} 行业配置数据更新失败")

            self.logger.info(f"基金行业配置数据更新完成，共处理{total_count}条数据")
