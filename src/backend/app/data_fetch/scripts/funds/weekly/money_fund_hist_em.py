...
class MoneyFundHistEm(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        # 货币型基金历史数据表
        self.table_name = "MONEY_FUND_HIST_EM"

        # 表结构定义
        self.create_table_sql = """
                            CREATE TABLE `MONEY_FUND_HIST_EM` (
                              `R_ID` VARCHAR(64) NOT NULL COMMENT '主键ID',
                              `REFERENCE_CODE` VARCHAR(50) DEFAULT 'MONEY_FUND_HIST' COMMENT '参考编码',
                              `REFERENCE_NAME` VARCHAR(100) DEFAULT '货币型基金历史净值表(东方财富)' COMMENT '参考名称',
                              ...
                              `INCOME_PER_10K` DECIMAL(10, 4) COMMENT '每万份收益(元)',
    ...
    def fetch_fund_hist_data(self, fund_code):
        ...
        try:
            ...
            df = df.rename(
                columns={
                    "净值日期": "VALUE_DATE",
                    "每万份收益": "INCOME_PER_10K",
                    "7日年化收益率": "ANNUAL_YIELD_7D",
            ...
            # 添加系统字段
            df["REFERENCE_CODE"] = "MONEY_FUND_HIST"
            df["REFERENCE_NAME"] = "货币型基金历史净值表(东方财富)"
            df["IS_ACTIVE"] = 1
            df["DATA_SOURCE"] = "东方财富"
            ...
            self.logger.info(f"成功获取货币型基金[{fund_code}]{len(df)}条历史数据")
            return df

        except Exception as e:
            self.logger.error(f"获取货币型基金[{fund_code}]历史数据失败: {e}", exc_info=True)
            return None
    ...
    def run(self, fund_codes=None):
        ...
        Args:
            fund_codes: 基金代码列表，如果为None则从数据库获取所有货币型基金代码
        ...
        try:
            self.logger.info("=" * 50)
            self.logger.info("开始执行货币型基金历史数据更新")

            # 创建表（如果不存在）
            self.create_table_if_not_exists()

            # 如果未指定基金代码，则从数据库获取所有货币型基金代码
            if fund_codes is None:
                fund_codes = self._get_all_money_fund_codes()
                if not fund_codes:
            ...
            self.logger.info(f"货币型基金历史数据更新完成，共处理{total_count}条数据")
    ...
    def _get_all_money_fund_codes(self):
        """
        从数据库获取所有货币型基金代码
        ...
        try:
            ...
            self.logger.error(f"获取货币型基金代码列表失败: {e}")
            return []
...
if __name__ == "__main__":
    ...
    fund_codes = [
        "000009",
        "000010",
    ]  # 可以指定要更新的基金代码，如果为None则更新所有货币型基金

    data_updater = MoneyFundHistEm()
