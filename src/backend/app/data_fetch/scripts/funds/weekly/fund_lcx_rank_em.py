...
class FundLcxRankEm(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        # 理财基金排行数据表
        self.table_name = "FUND_LCX_RANK_EM"

        # 表结构定义
        self.create_table_sql = """
                            CREATE TABLE `FUND_LCX_RANK_EM` (
                              `R_ID` VARCHAR(64) NOT NULL COMMENT '主键ID',
                              `REFERENCE_CODE` VARCHAR(50) DEFAULT 'FUND_LCX_RANK' COMMENT '参考编码',
                              `REFERENCE_NAME` VARCHAR(100) DEFAULT '理财基金排行数据表(东方财富)' COMMENT '参考名称',
                              ...
                              PRIMARY KEY (`R_ID`),
                              UNIQUE KEY `IDX_FUND_LCX_RANK_UNIQUE` (`FUND_CODE`, `TRADE_DATE`),
                              KEY `IDX_FUND_CODE` (`FUND_CODE`),
                              KEY `IDX_TRADE_DATE` (`TRADE_DATE`),
                              KEY `IDX_IS_PURCHASABLE` (`IS_PURCHASABLE`),
                              KEY `IDX_IS_ACTIVE` (`IS_ACTIVE`)
                            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='理财基金排行数据表(东方财富)';
                            """
    ...
    def parse_int(self, value: Any, default: int = None) -> int | None:
        """解析整数值"""
        if pd.isna(value) or value in ("---", "", "--", "-"):
            return default
        try:
    ...
    def fetch_lcx_rank_data(self) -> pd.DataFrame | None:
        """
        获取理财基金排行数据

        Returns:
            pd.DataFrame: 处理后的理财基金排行数据
        """
        try:
            self.logger.info("开始获取理财基金排行数据...")

            # 获取数据
            df = self.fetch_ak_data("fund_lcx_rank_em")

            if df is None or df.empty:
                self.logger.warning("未获取到理财基金排行数据")
                return None

            return df

        except Exception as e:
            self.logger.error(f"获取理财基金排行数据失败: {e}", exc_info=True)
            return None

    def process_lcx_rank_data(self, df: pd.DataFrame) -> pd.DataFrame | None:
        """处理理财基金排行数据"""
        if df is None or df.empty:
            return None
        ...
        try:
            ...
            if "IS_PURCHASABLE_STR" in df.columns:
                df["IS_PURCHASABLE"] = df["IS_PURCHASABLE_STR"].apply(
                    lambda x: 1 if self.parse_int(x, 0) == 1 else 0
                )
            ...
            # 添加系统字段
            df["REFERENCE_CODE"] = "FUND_LCX_RANK"
            df["REFERENCE_NAME"] = "理财基金排行数据表(东方财富)"
            df["IS_ACTIVE"] = 1
            df["DATA_SOURCE"] = "东方财富"
            ...
            self.logger.info(f"成功处理{len(df)}条理财基金排行数据")
            return df

        except Exception as e:
            self.logger.error(f"处理理财基金排行数据失败: {e}", exc_info=True)
            return None

    def save_lcx_rank_data(self, df: pd.DataFrame) -> bool:
        """
        保存理财基金排行数据到数据库
        ...
        """
        if df is None or df.empty:
            self.logger.warning("没有理财基金排行数据需要保存")
            return False
        ...
        if df.empty:
            self.logger.info("没有新的理财基金排行数据需要保存")
            return True
    ...
    def _get_existing_ids(self) -> set:
        ...
        try:
            self.connect_db()
            sql = "SELECT R_ID FROM FUND_LCX_RANK_EM WHERE IS_ACTIVE = 1"
    ...
    def run(self):
        ...
        try:
            self.logger.info("=" * 50)
            self.logger.info("开始执行理财基金排行数据更新")

            # 创建表（如果不存在）
            self.create_table_if_not_exists()

            # 获取数据
            df = self.fetch_lcx_rank_data()

            if df is None or df.empty:
                self.logger.error("未获取到有效的理财基金排行数据")
                return False

            # 处理数据
            processed_df = self.process_lcx_rank_data(df)

            if processed_df is None or processed_df.empty:
                self.logger.error("处理理财基金排行数据失败")
                return False

            # 保存数据
            success = self.save_lcx_rank_data(processed_df)

            if success:
                self.logger.info(f"成功保存{len(processed_df)}条理财基金排行数据")
            else:
                self.logger.error("保存理财基金排行数据失败")
