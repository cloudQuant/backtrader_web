...
class GradedFundHistEm(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        # 分级基金历史数据表
        self.table_name = "GRADED_FUND_HIST_EM"

        # 表结构定义
        self.create_table_sql = """
                            CREATE TABLE `GRADED_FUND_HIST_EM` (
                              `R_ID` VARCHAR(64) NOT NULL COMMENT '主键ID',
                              `REFERENCE_CODE` VARCHAR(50) DEFAULT 'GRADED_FUND_HIST' COMMENT '参考编码',
                              `REFERENCE_NAME` VARCHAR(100) DEFAULT '分级基金历史数据表(东方财富)' COMMENT '参考名称',
                            ...
                            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='分级基金历史数据表(东方财富)';
                            """

    def parse_growth_rate(self, value):
        """
        解析增长率字符串，如'1.72'，返回1.72
    ...
    def fetch_fund_hist_data(self, fund_code):
        ...
        try:
            ...
            # 添加系统字段
            df["REFERENCE_CODE"] = "GRADED_FUND_HIST"
            df["REFERENCE_NAME"] = "分级基金历史数据表(东方财富)"
            df["IS_ACTIVE"] = 1
            df["DATA_SOURCE"] = "东方财富"
            ...
            self.logger.info(f"成功获取分级基金[{fund_code}]{len(df)}条历史数据")
            return df

        except Exception as e:
            self.logger.error(f"获取分级基金[{fund_code}]历史数据失败: {e}", exc_info=True)
            return None
    ...
    def run(self, fund_codes=None):
        ...
        Args:
            fund_codes: 基金代码列表，如果为None则从数据库获取所有分级基金代码
        ...
        try:
            self.logger.info("=" * 50)
            self.logger.info("开始执行分级基金历史数据更新")

            # 创建表（如果不存在）
            self.create_table_if_not_exists()

            # 如果未指定基金代码，则从数据库获取所有分级基金代码
            if fund_codes is None:
                fund_codes = self._get_all_graded_fund_codes()
                if not fund_codes:
            ...
            self.logger.info(f"分级基金历史数据更新完成，共处理{total_count}条数据")
    ...
    def _get_all_graded_fund_codes(self):
        """
        从数据库获取所有分级基金代码
        ...
        try:
            ...
            self.logger.error(f"获取分级基金代码列表失败: {e}")
            return []
...
if __name__ == "__main__":
    ...
    fund_codes = [
        "150232",
        "150174",
    ]  # 可以指定要更新的基金代码，如果为None则更新所有分级基金

    data_updater = GradedFundHistEm()
