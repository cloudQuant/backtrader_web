...
class StockHotRankEm(AkshareToMySql):
    """Stock Hot Rank Em"""

    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "STOCK_HOT_RANK_EM"
        self.create_table_sql = """
    CREATE TABLE IF NOT EXISTS `STOCK_HOT_RANK_EM` (
    ...
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Stock Hot Rank Em'
    """
    ...
    def fetch_rank(page_size: int = 100) -> pd.DataFrame:
        ...
        if quote_df.empty:
            ...
            result = rank_df.merge(
                quote_df[["code6", "股票名称", "最新价", "涨跌幅"]],
                on="code6",
                how="left",
            )
            result = result.rename(columns={"rk": "当前排名", "sc": "代码"})
            result["涨跌额"] = result["最新价"] * result["涨跌幅"] / 100
        result = result[
            [
                column
                for column in ("当前排名", "代码", "股票名称", "最新价", "涨跌额", "涨跌幅")
                if column in result.columns
            ]
        ]
        return StockHotRankEm.normalize_columns(result)
    ...
    def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
        if df.empty:
            return df
        df = df.copy()
        for column in ("当前排名", "最新价", "涨跌额", "涨跌幅"):
    ...
    def fetch_data(self, **kwargs):
        ...
        Args:
            **kwargs: Parameters to pass to ak.stock_hot_rank_em
