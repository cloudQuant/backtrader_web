"""
Stock Board Concept Hist Em

数据源: AkShare
函数: stock_board_concept_hist_em
频率: daily
"""
...
class StockBoardConceptHistEm(AkshareToMySql):
    """Stock Board Concept Hist Em"""

    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "STOCK_BOARD_CONCEPT_HIST_EM"
        self.create_table_sql = """
    CREATE TABLE IF NOT EXISTS `STOCK_BOARD_CONCEPT_HIST_EM` (
    ...
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Stock Board Concept Hist Em'
    """
    ...
    def _resolve_board_code(self, symbol):
        ...
        listing = ak.stock_board_concept_name_em()
        matched = listing[listing["板块名称"] == symbol]
        if matched.empty:
            raise ValueError(f"Unknown Eastmoney concept board: {symbol}")
        row = matched.iloc[0]
        return str(row["板块代码"]), str(row["板块名称"])
    ...
    def _fetch_daily_from_trends(self, symbol, start_date=None, end_date=None):
        try:
            ...
            self.logger.warning(f"Resolve Eastmoney concept board failed: {exc}")
            return pd.DataFrame()
        ...
        try:
            ...
            self.logger.warning(f"Eastmoney concept trends2 fallback failed: {exc}")
            return pd.DataFrame()
        ...
        if not grouped.empty:
            self.logger.warning(
                "Eastmoney concept kline returned empty; populated "
                "STOCK_BOARD_CONCEPT_HIST_EM from same-source trends2 aggregation"
            )
    ...
    def fetch_data(self, **kwargs):
        ...
        Args:
            **kwargs: Parameters to pass to ak.stock_board_concept_hist_em

        Returns:
            pd.DataFrame: Fetched data
        """
        try:
            kwargs.setdefault("symbol", "数据要素")
            kwargs.setdefault("period", "daily")
            kwargs.setdefault("adjust", "")
            kwargs.setdefault("_call_timeout", 8)
            # Fetch data from AkShare
            try:
                df = self.fetch_ak_data("stock_board_concept_hist_em", **kwargs)
            except Exception as fetch_exc:
                self.logger.warning(
                    f"Eastmoney concept kline fetch failed, trying same-source trends2 "
            ...
            if "symbol" not in df.columns:
                df["symbol"] = str(kwargs.get("symbol") or "数据要素")
            if "name" not in df.columns:
                df["name"] = str(kwargs.get("symbol") or "数据要素")
...
def main():
    ...
    script = StockBoardConceptHistEm()
    script.run()
