"""
Stock Gpzy Distribute Statistics Bank Em

数据源: AkShare
函数: stock_gpzy_distribute_statistics_bank_em
频率: daily
"""
...
class StockGpzyDistributeStatisticsBankEm(AkshareToMySql):
    """Stock Gpzy Distribute Statistics Bank Em"""

    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "STOCK_GPZY_DISTRIBUTE_STATISTICS_BANK_EM"
        self.create_table_sql = """
    CREATE TABLE IF NOT EXISTS `STOCK_GPZY_DISTRIBUTE_STATISTICS_BANK_EM` (
    ...
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Stock Gpzy Distribute Statistics Bank Em'
    """
    ...
    def fetch_data(self, **kwargs):
        ...
        Args:
            **kwargs: Parameters to pass to ak.stock_gpzy_distribute_statistics_bank_em

        Returns:
            pd.DataFrame: Fetched data
        """
        try:
            # 东方财富当前 PFORG_TYPE 使用行业二级名称，旧筛选值“银行”已返回空。
            df = self._fetch_distribution(str(kwargs.pop("pforg_type", "银行Ⅱ")))
...
def main():
    ...
    script = StockGpzyDistributeStatisticsBankEm()
    script.run()
