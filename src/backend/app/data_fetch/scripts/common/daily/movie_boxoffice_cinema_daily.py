"""
Movie Boxoffice Cinema Daily

数据源: 艺恩 Endata
函数: movie_boxoffice_cinema_daily
频率: daily
"""
...
class MovieBoxofficeCinemaDaily(AkshareToMySql):
    """Movie Boxoffice Cinema Daily"""

    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "MOVIE_BOXOFFICE_CINEMA_DAILY"
        self.create_table_sql = """
    CREATE TABLE IF NOT EXISTS `MOVIE_BOXOFFICE_CINEMA_DAILY` (
        ...
        UNIQUE KEY uk_cinema_date (`CinemaID`, `data_date`),
        INDEX idx_data_date (`data_date`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Movie Boxoffice Cinema Daily'
    """

    def fetch_data(self, **kwargs):
        """Fetch daily cinema box-office data from the current Endata API.
        ...
        """
        try:
            df = EndataYienClient().fetch_cinema_day(kwargs.get("date"))
            ...
            self.save_data(
                ...
                unique_keys=["CinemaID", "data_date"],
            )
...
def main():
    ...
    script = MovieBoxofficeCinemaDaily()
    script.fetch_data()
