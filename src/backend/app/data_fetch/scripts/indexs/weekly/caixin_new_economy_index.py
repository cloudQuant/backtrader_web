# 4064_caixin_new_economy_index.py
import argparse
import logging
import sys
...
class CaixinNewEconomyIndex(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "CAIXIN_NEW_ECONOMY_INDEX"
        self.create_table_sql = """
            CREATE TABLE `CAIXIN_NEW_ECONOMY_INDEX` (
                `R_ID` VARCHAR(50) PRIMARY KEY,
                `TRADE_DATE` DATE NOT NULL COMMENT '日期',
                `NEW_ECONOMY_INDEX` DECIMAL(12, 6) COMMENT '中国新经济指数',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='财新中国新经济指数';
        """

    def fetch_index_data(self):
        """Fetch Caixin New Economy Index data"""
        try:
            self.logger.info("Fetching Caixin New Economy Index data")

            df = self.fetch_ak_data("index_nei_cx")

            if df is None or df.empty:
                self.logger.warning("No data found for Caixin New Economy Index")
                return pd.DataFrame()
            ...
            self.logger.error(
                f"Error fetching Caixin New Economy Index data: {str(e)}", exc_info=True
            )
            return pd.DataFrame()

    def run(self, update_all=False):
        """Run the Caixin New Economy Index update"""
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Caixin New Economy Index records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Caixin New Economy Index Data")
    ...
    try:
        ...
        fetcher = CaixinNewEconomyIndex(logger=logger)
