# 4070_caixin_quality_factor_index.py
import argparse
import logging
import sys
...
class CaixinQualityFactorIndex(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "CAIXIN_QUALITY_FACTOR_INDEX"
        self.create_table_sql = """
            CREATE TABLE `CAIXIN_QUALITY_FACTOR_INDEX` (
                `R_ID` VARCHAR(50) PRIMARY KEY,
                `TRADE_DATE` DATE NOT NULL COMMENT '日期',
                `QUALITY_FACTOR_INDEX` DECIMAL(12, 6) COMMENT '高质量因子指数',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='财新高质量因子指数';
        """

    def fetch_index_data(self):
        """Fetch Caixin Quality Factor Index data"""
        try:
            self.logger.info("Fetching Caixin Quality Factor Index data")

            df = self.fetch_ak_data("index_qli_cx")

            if df is None or df.empty:
                self.logger.warning("No data found for Caixin Quality Factor Index")
                return pd.DataFrame()
            ...
            self.logger.error(
                f"Error fetching Caixin Quality Factor Index data: {str(e)}",
                exc_info=True,
            )
            return pd.DataFrame()

    def run(self, update_all=False):
        """Run the Caixin Quality Factor Index update"""
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Caixin Quality Factor Index records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Caixin Quality Factor Index Data")
    ...
    try:
        ...
        fetcher = CaixinQualityFactorIndex(logger=logger)
