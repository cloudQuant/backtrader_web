...
class StockHkIndexDailySina(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "STOCK_HK_INDEX_DAILY_SINA"
        self.create_table_sql = """
            CREATE TABLE `STOCK_HK_INDEX_DAILY_SINA` (
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='港股指数日线行情表(新浪财经)';
        """
    ...
    def get_hk_index_codes(self):
        ...
        """
        table_name = "STOCK_HK_INDEX_SPOT_SINA"
    ...
    def fetch_index_daily(self, symbol):
        """Fetch daily historical data for a Hong Kong index from Sina Finance.

        Args:
            symbol (str): Index code (e.g., 'HSI', 'HSCEI')
        ...
        try:
            ...
            df = self.fetch_ak_data("stock_hk_index_daily_sina", symbol=symbol)
    ...
    def run(self, symbol=None):
        ...
        Args:
            symbol (str, optional): Index code (e.g., 'HSI', 'HSCEI'). If None, fetches all indices.

        Returns:
            bool: True if successful, False otherwise
        """
        try:
            self.logger.info("Starting Hong Kong index daily data update")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch historical daily Hong Kong index data from Sina Finance"
    )
    parser.add_argument(
        "--symbol",
        type=str,
        required=False,
        help="港股指数代码，例如: HSI, HSCEI。如果不指定，将处理所有指数",
    )

    try:
        args = parser.parse_args()
        fetcher = StockHkIndexDailySina(logger=logger)
