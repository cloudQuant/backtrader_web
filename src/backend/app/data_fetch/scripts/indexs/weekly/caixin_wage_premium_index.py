# 4068_caixin_wage_premium_index.py
import argparse
import logging
import sys
...
class CaixinWagePremiumIndex(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "CAIXIN_WAGE_PREMIUM_INDEX"
        self.create_table_sql = """
            CREATE TABLE `CAIXIN_WAGE_PREMIUM_INDEX` (
                `R_ID` VARCHAR(50) PRIMARY KEY,
                `TRADE_DATE` DATE NOT NULL COMMENT '日期',
                `WAGE_PREMIUM_LEVEL` DECIMAL(12, 6) COMMENT '新经济入职工资溢价水平',
                `CHANGE_VALUE` DECIMAL(12, 6) COMMENT '变化值',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='财新新经济入职工资溢价水平';
        """

    def fetch_index_data(self):
        """Fetch Caixin Wage Premium Level data"""
        try:
            self.logger.info("Fetching Caixin New Economy Wage Premium Level data")

            df = self.fetch_ak_data("index_awpr_cx")

            if df is None or df.empty:
                self.logger.warning("No data found for Caixin New Economy Wage Premium Level")
                return pd.DataFrame()
            ...
            df = df.rename(
                columns={
                    "日期": "TRADE_DATE",
                    "新经济入职工资溢价水平": "WAGE_PREMIUM_LEVEL",
                    "变化值": "CHANGE_VALUE",
                }
            )
            ...
            self.logger.error(
                f"Error fetching Caixin New Economy Wage Premium Level data: {str(e)}",
                exc_info=True,
            )
            return pd.DataFrame()

    def run(self, update_all=False):
        """Run the Caixin Wage Premium Level update"""
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Caixin New Economy Wage Premium Level records")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update Caixin New Economy Wage Premium Level Data"
    )
    ...
    try:
        ...
        fetcher = CaixinWagePremiumIndex(logger=logger)
