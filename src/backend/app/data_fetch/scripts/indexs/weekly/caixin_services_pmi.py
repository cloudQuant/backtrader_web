...
class CaixinServicesPmi(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "CAIXIN_SERVICES_PMI"
        self.create_table_sql = """
            CREATE TABLE `CAIXIN_SERVICES_PMI` (
                `R_ID` VARCHAR(50) PRIMARY KEY,
                `TRADE_DATE` DATE NOT NULL COMMENT '日期',
                `SERVICES_PMI` DECIMAL(5, 2) COMMENT '服务业PMI',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='财新中国服务业PMI';
        """

    def fetch_pmi_data(self):
        """Fetch Caixin China Services PMI data"""
        try:
            self.logger.info("Fetching Caixin China Services PMI data")

            df = self.fetch_ak_data("index_pmi_ser_cx")

            if df is None or df.empty:
                self.logger.warning("No data found for Caixin China Services PMI")
                return pd.DataFrame()
            ...
            self.logger.error(f"Error fetching Caixin Services PMI data: {str(e)}", exc_info=True)
            return pd.DataFrame()

    def run(self, update_all=False):
        """Run the Caixin Services PMI update"""
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Caixin Services PMI records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Caixin China Services PMI Data")
    ...
    try:
        ...
        fetcher = CaixinServicesPmi(logger=logger)
