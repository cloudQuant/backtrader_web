# 4057_caixin_manufacturing_pmi.py
import argparse
import logging
import sys
...
class CaixinManufacturingPmi(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "CAIXIN_MANUFACTURING_PMI"
        self.create_table_sql = """
            CREATE TABLE `CAIXIN_MANUFACTURING_PMI` (
                `R_ID` VARCHAR(50) PRIMARY KEY,
                `TRADE_DATE` DATE NOT NULL COMMENT '日期',
                `MANUFACTURING_PMI` DECIMAL(5, 2) COMMENT '制造业PMI',
                `CHANGE_VALUE` DECIMAL(5, 2) COMMENT '变化值',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='财新中国制造业PMI';
        """

    def fetch_pmi_data(self):
        """Fetch Caixin China Manufacturing PMI data"""
        try:
            self.logger.info("Fetching Caixin China Manufacturing PMI data")

            df = self.fetch_ak_data("index_pmi_man_cx")

            if df is None or df.empty:
                self.logger.warning("No data found for Caixin China Manufacturing PMI")
                return pd.DataFrame()
            ...
            self.logger.error(
                f"Error fetching Caixin Manufacturing PMI data: {str(e)}", exc_info=True
            )
            return pd.DataFrame()

    def run(self, update_all=False):
        """Run the Caixin Manufacturing PMI update"""
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Caixin Manufacturing PMI records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Caixin China Manufacturing PMI Data")
    ...
    try:
        ...
        fetcher = CaixinManufacturingPmi(logger=logger)
