...
class CaixinBasicIndex(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "CAIXIN_BASIC_INDEX"
        self.create_table_sql = """
            CREATE TABLE `CAIXIN_BASIC_INDEX` (
                `R_ID` VARCHAR(50) PRIMARY KEY,
                `TRADE_DATE` DATE NOT NULL COMMENT '日期',
                `BASIC_INDEX` DECIMAL(10, 2) COMMENT '基础指数',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='财新基础指数';
        """

    def fetch_index_data(self):
        """Fetch Caixin Basic Index data"""
        try:
            self.logger.info("Fetching Caixin Basic Index data")

            df = self.fetch_ak_data("index_bi_cx")

            if df is None or df.empty:
                self.logger.warning("No data found for Caixin Basic Index")
                return pd.DataFrame()
            ...
            self.logger.error(f"Error fetching Caixin Basic Index data: {str(e)}", exc_info=True)
            return pd.DataFrame()

    def run(self, update_all=False):
        """Run the Caixin Basic Index update"""
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Caixin Basic Index records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Caixin Basic Index Data")
    ...
    try:
        ...
        fetcher = CaixinBasicIndex(logger=logger)
