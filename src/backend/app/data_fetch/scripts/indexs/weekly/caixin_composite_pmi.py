...
class CaixinCompositePmi(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "CAIXIN_COMPOSITE_PMI"
        self.create_table_sql = """
            CREATE TABLE `CAIXIN_COMPOSITE_PMI` (
                `R_ID` VARCHAR(50) PRIMARY KEY,
                `TRADE_DATE` DATE NOT NULL COMMENT '日期',
                `COMPOSITE_PMI` DECIMAL(5, 2) COMMENT '综合PMI',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='财新中国综合PMI';
        """

    def fetch_pmi_data(self):
        """Fetch Caixin China Composite PMI data"""
        try:
            self.logger.info("Fetching Caixin China Composite PMI data")

            df = self.fetch_ak_data("index_pmi_com_cx")

            if df is None or df.empty:
                self.logger.warning("No data found for Caixin China Composite PMI")
                return pd.DataFrame()
            ...
            self.logger.error(f"Error fetching Caixin Composite PMI data: {str(e)}", exc_info=True)
            return pd.DataFrame()

    def run(self, update_all=False):
        """Run the Caixin Composite PMI update"""
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Caixin Composite PMI records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Caixin China Composite PMI Data")
    ...
    try:
        ...
        fetcher = CaixinCompositePmi(logger=logger)
