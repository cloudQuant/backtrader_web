...
class CaixinIndustryIndex(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "CAIXIN_INDUSTRY_INDEX"
        self.create_table_sql = """
            CREATE TABLE `CAIXIN_INDUSTRY_INDEX` (
                `R_ID` VARCHAR(50) PRIMARY KEY,
                `TRADE_DATE` DATE NOT NULL COMMENT '日期',
                `INDUSTRY_INDEX` DECIMAL(10, 2) COMMENT '产业指数',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='财新产业指数';
        """

    def fetch_index_data(self):
        """Fetch Caixin Industry Index data"""
        try:
            self.logger.info("Fetching Caixin Industry Index data")

            df = self.fetch_ak_data("index_ii_cx")

            if df is None or df.empty:
                self.logger.warning("No data found for Caixin Industry Index")
                return pd.DataFrame()
            ...
            self.logger.error(f"Error fetching Caixin Industry Index data: {str(e)}", exc_info=True)
            return pd.DataFrame()

    def run(self, update_all=False):
        """Run the Caixin Industry Index update"""
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Caixin Industry Index records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Caixin Industry Index Data")
    ...
    try:
        ...
        fetcher = CaixinIndustryIndex(logger=logger)
