...
class SWIndustryFirstInfo(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "SW_INDUSTRY_FIRST_INFO"
        self.create_table_sql = """
            CREATE TABLE `SW_INDUSTRY_FIRST_INFO` (
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='申万一级行业信息';
        """

    def fetch_industry_info(self):
        """
        Fetch Shenwan First-Class Industry Information

        Returns:
            DataFrame containing industry information
        """
        try:
            self.logger.info("Fetching Shenwan First-Class Industry Information")

            # Fetch data using parent class method
            df = self.fetch_ak_data("sw_index_first_info")

            if df is None or df.empty:
                self.logger.warning("No Shenwan industry data found")
                return pd.DataFrame()
            ...
            self.logger.error(f"Error fetching Shenwan industry data: {str(e)}", exc_info=True)
            return pd.DataFrame()

    def run(self):
        """
        Main method to run the Shenwan industry info update
        ...
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Shenwan industry records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Shenwan First-Class Industry Information")
    ...
    try:
        ...
        fetcher = SWIndustryFirstInfo(logger=logger)
        success = fetcher.run()
