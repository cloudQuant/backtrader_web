...
class OptionKCBQVIX(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "OPTION_KCB_QVIX"
        self.create_table_sql = """
            CREATE TABLE `OPTION_KCB_QVIX` (
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='科创板期权波动率指数(QVIX)';
        """

    def fetch_qvix_data(self):
        """
        Fetch STAR Market (KCB) Option Volatility Index (QVIX) data

        Returns:
            DataFrame containing QVIX data
        """
        try:
            self.logger.info("Fetching STAR Market (KCB) Option Volatility Index (QVIX) data")
            ...
            df = raw_df.iloc[:, [0, 83, 84, 85, 86]].copy()
            ...
            if df is None or df.empty:
                self.logger.warning("No KCB QVIX data found")
                return pd.DataFrame()
            ...
            self.logger.error(f"Error fetching KCB QVIX data: {str(e)}", exc_info=True)
            return pd.DataFrame()

    def run(self, update_all=False):
        """
        Main method to run the KCB QVIX data update
        ...
        try:
            ...
            if df.empty:
                self.logger.error("No KCB QVIX data to save")
                return False
            ...
            #         if df.empty:
            #             self.logger.info("No new KCB QVIX data to update")
            #             return True
            ...
            if not df.empty:
                ...
                self.logger.info(f"Saved {len(df)} KCB QVIX records")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update STAR Market (KCB) Option Volatility Index (QVIX) data"
    )
    ...
    try:
        ...
        fetcher = OptionKCBQVIX(logger=logger)
