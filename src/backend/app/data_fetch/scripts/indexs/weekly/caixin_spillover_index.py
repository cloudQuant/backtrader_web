# 4061_caixin_spillover_index.py
import argparse
import logging
import sys
...
class CaixinSpilloverIndex(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "CAIXIN_SPILLOVER_INDEX"
        self.create_table_sql = """
            CREATE TABLE `CAIXIN_SPILLOVER_INDEX` (
                `R_ID` VARCHAR(50) PRIMARY KEY,
                `TRADE_DATE` DATE NOT NULL COMMENT '日期',
                `SPILLOVER_INDEX` DECIMAL(10, 2) COMMENT '溢出指数',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='财新溢出指数';
        """

    def fetch_index_data(self):
        """Fetch Caixin Spillover Index data"""
        try:
            self.logger.info("Fetching Caixin Spillover Index data")

            df = self.fetch_ak_data("index_si_cx")

            if df is None or df.empty:
                self.logger.warning("No data found for Caixin Spillover Index")
                return pd.DataFrame()
            ...
            self.logger.error(
                f"Error fetching Caixin Spillover Index data: {str(e)}", exc_info=True
            )
            return pd.DataFrame()

    def run(self, update_all=False):
        """Run the Caixin Spillover Index update"""
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Caixin Spillover Index records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Caixin Spillover Index Data")
    ...
    try:
        ...
        fetcher = CaixinSpilloverIndex(logger=logger)
