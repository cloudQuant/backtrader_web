...
class Option50IndexQVIX(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "OPTION_50INDEX_QVIX"
        self.create_table_sql = """
            CREATE TABLE `OPTION_50INDEX_QVIX` (
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='上证50股指期权波动率指数(QVIX)';
        """

    def fetch_qvix_data(self):
        """
        Fetch SSE 50 Index Option Volatility Index (QVIX) data

        Returns:
            DataFrame containing QVIX data
        """
        try:
            self.logger.info("Fetching SSE 50 Index Option Volatility Index (QVIX) data")
            ...
            df = raw_df.iloc[:, [0, 79, 80, 81, 82]].copy()
            ...
            if df is None or df.empty:
                self.logger.warning("No 50 Index QVIX data found")
                return pd.DataFrame()
            ...
            self.logger.error(f"Error fetching 50 Index QVIX data: {str(e)}", exc_info=True)
            return pd.DataFrame()

    def run(self, update_all=False):
        """
        Main method to run the 50 Index QVIX data update
        ...
        try:
            ...
            if df.empty:
                self.logger.error("No 50 Index QVIX data to save")
                return False
            ...
            #         if df.empty:
            #             self.logger.info("No new 50 Index QVIX data to update")
            #             return True
            ...
            if not df.empty:
                ...
                self.logger.info(f"Saved {len(df)} 50 Index QVIX records")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update SSE 50 Index Option Volatility Index (QVIX) data"
    )
    ...
    try:
        ...
        fetcher = Option50IndexQVIX(logger=logger)
