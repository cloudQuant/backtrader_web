# 4066_caixin_capital_input_index.py
import argparse
import logging
import sys
...
class CaixinCapitalInputIndex(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "CAIXIN_CAPITAL_INPUT_INDEX"
        self.create_table_sql = """
            CREATE TABLE `CAIXIN_CAPITAL_INPUT_INDEX` (
                `R_ID` VARCHAR(50) PRIMARY KEY,
                `TRADE_DATE` DATE NOT NULL COMMENT '日期',
                `CAPITAL_INPUT_INDEX` DECIMAL(12, 6) COMMENT '资本投入指数',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='财新资本投入指数';
        """

    def fetch_index_data(self):
        """Fetch Caixin Capital Input Index data"""
        try:
            self.logger.info("Fetching Caixin Capital Input Index data")

            df = self.fetch_ak_data("index_ci_cx")

            if df is None or df.empty:
                self.logger.warning("No data found for Caixin Capital Input Index")
                return pd.DataFrame()
            ...
            self.logger.error(
                f"Error fetching Caixin Capital Input Index data: {str(e)}",
                exc_info=True,
            )
            return pd.DataFrame()

    def run(self, update_all=False):
        """Run the Caixin Capital Input Index update"""
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Caixin Capital Input Index records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Caixin Capital Input Index Data")
    ...
    try:
        ...
        fetcher = CaixinCapitalInputIndex(logger=logger)
