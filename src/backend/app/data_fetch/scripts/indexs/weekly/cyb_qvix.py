...
class OptionCYBQVIX(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "OPTION_CYB_QVIX"
        self.create_table_sql = """
            CREATE TABLE `OPTION_CYB_QVIX` (
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='创业板期权波动率指数(QVIX)';
        """

    def fetch_qvix_data(self):
        """
        Fetch ChiNext (CYB) Option Volatility Index (QVIX) data

        Returns:
            DataFrame containing QVIX data
        """
        try:
            self.logger.info("Fetching ChiNext (CYB) Option Volatility Index (QVIX) data")
            ...
            df = raw_df.iloc[:, [0, 71, 72, 73, 74]].copy()
            ...
            if df is None or df.empty:
                self.logger.warning("No CYB QVIX data found")
                return pd.DataFrame()
            ...
            self.logger.error(f"Error fetching CYB QVIX data: {str(e)}", exc_info=True)
            return pd.DataFrame()

    def run(self, update_all=False):
        """
        Main method to run the CYB QVIX data update
        ...
        try:
            ...
            if df.empty:
                self.logger.error("No CYB QVIX data to save")
                return False
            ...
            #         if df.empty:
            #             self.logger.info("No new CYB QVIX data to update")
            #             return True
            ...
            if not df.empty:
                ...
                self.logger.info(f"Saved {len(df)} CYB QVIX records")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update ChiNext (CYB) Option Volatility Index (QVIX) data"
    )
    ...
    try:
        ...
        fetcher = OptionCYBQVIX(logger=logger)
