...
class SWIndustryThirdInfo(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "SW_INDUSTRY_THIRD_INFO"
        self.create_table_sql = """
            CREATE TABLE `SW_INDUSTRY_THIRD_INFO` (
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='申万三级行业信息';
        """

    def fetch_industry_info(self):
        """
        Fetch Shenwan Third-Class Industry Information

        Returns:
            DataFrame containing industry information
        """
        try:
            self.logger.info("Fetching Shenwan Third-Class Industry Information")

            # Fetch data using parent class method
            df = self.fetch_ak_data("sw_index_third_info")

            if df is None or df.empty:
                self.logger.warning("No Shenwan third-class industry data found")
                return pd.DataFrame()
            ...
            self.logger.error(
                f"Error fetching Shenwan third-class industry data: {str(e)}",
                exc_info=True,
            )
            return pd.DataFrame()
    ...
    def run(self):
        """
        Main method to run the Shenwan third-class industry info update
        ...
        try:
            ...
            if not df.empty:
                ...
                self.logger.info(f"Updated {len(df)} Shenwan third-class industry records")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update Shenwan Third-Class Industry Information")
    ...
    try:
        ...
        fetcher = SWIndustryThirdInfo(logger=logger)
        success = fetcher.run()
