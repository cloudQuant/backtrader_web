...
class Option50ETFMinQVIX(AkshareToMySql):
    def __init__(self, db_config=DB_CONFIG, logger=None):
        super().__init__(db_config, logger)
        self.table_name = "OPTION_50ETF_MIN_QVIX"
        self.create_table_sql = """
            CREATE TABLE `OPTION_50ETF_MIN_QVIX` (
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='50ETF期权波动率指数-分时数据';
        """

    def fetch_min_qvix_data(self):
        """
        Fetch 50ETF Option Volatility Index (QVIX) minute-level data

        Returns:
            DataFrame containing QVIX minute data with date and time
        """
        try:
            self.logger.info("Fetching 50ETF Option Volatility Index (QVIX) minute data")

            # Fetch data using parent class method
            df = self.fetch_ak_data("index_option_50etf_min_qvix")

            if df is None or df.empty:
                self.logger.warning("No QVIX minute data found")
                return pd.DataFrame()
            ...
            self.logger.error(f"Error fetching QVIX minute data: {str(e)}", exc_info=True)
            return pd.DataFrame()

    def run(self):
        """
        Main method to run the QVIX minute data update
        ...
        try:
            ...
            if df.empty:
                self.logger.error("No QVIX minute data to save")
                return False
            ...
            if not df.empty:
                ...
                self.logger.info(f"Saved {len(df)} QVIX minute records for {today}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update 50ETF Option Volatility Index (QVIX) minute data"
    )
    ...
    try:
        ...
        fetcher = Option50ETFMinQVIX(logger=logger)
        success = fetcher.run()
