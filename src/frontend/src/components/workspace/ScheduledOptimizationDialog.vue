    ...
    <div class="space-y-4">
      <div class="grid grid-cols-1 gap-3 md:grid-cols-4">
        <div class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
          ...
          <div class="text-xs text-slate-400">定时优化调度</div>
        </div>
        <div class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
          <div class="text-xs text-slate-500">执行频率</div>
          <div class="mt-1 text-lg font-semibold text-slate-700">{{ frequencyLabel }}</div>
          <div class="text-xs text-slate-400">自动触发周期</div>
        </div>
        <div class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
          <div class="text-xs text-slate-500">执行时间</div>
          <div class="mt-1 text-lg font-semibold text-slate-700">{{ form.execution_time }}</div>
          <div class="text-xs text-slate-400">工作区计划时点</div>
        </div>
        <div class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
          <div class="text-xs text-slate-500">作用范围</div>
          <div class="mt-1 text-lg font-semibold text-slate-700">{{ scopeLabel }}</div>
          <div class="text-xs text-slate-400">已选 {{ store.selectedUnitIds.length }} 个单元</div>
        </div>
      </div>

      <div class="rounded-xl border border-slate-200 bg-white px-4 py-4">
        <el-form label-width="110px" class="space-y-2">
          ...
          <el-form-item label="作用范围">
            <el-select v-model="form.unit_scope" class="w-48">
              <el-option label="全部交易单元" value="all" />
              <el-option label="仅当前选中单元" value="selected" />
            </el-select>
          </el-form-item>
