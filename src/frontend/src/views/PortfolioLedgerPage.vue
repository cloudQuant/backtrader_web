<template>
  <div class="space-y-4">
    <div>
      <h2 class="text-2xl font-bold">组合账本</h2>
      <p class="text-sm text-gray-500 mt-1">独立于旧组合聚合接口的账本式组合管理。</p>
    </div>

    <el-card>
      <template #header>
        <div class="font-bold">账本概览</div>
      </template>
      <div class="space-y-3">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
          <el-input v-model="portfolioName" placeholder="组合名称" />
          <el-input v-model="baseCurrency" placeholder="基础货币" />
          <el-input v-model="sourceType" placeholder="来源类型" />
          <el-button type="primary" :loading="loading" @click="createPortfolio">创建账本</el-button>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-6 gap-3">
          <el-input v-model="txSymbol" placeholder="标的代码" />
          <el-select v-model="txTradeType">
            <el-option label="buy" value="buy" />
            <el-option label="sell" value="sell" />
          </el-select>
          <el-input-number v-model="txQuantity" :min="1" />
          <el-input-number v-model="txPrice" :min="0" :step="1" />
          <el-input v-model="txTradeDate" placeholder="交易日 YYYY-MM-DD" />
          <el-button :loading="loading || !portfolioId" @click="importTransaction">导入交易</el-button>
        </div>
        <div class="flex gap-3 flex-wrap">
          <el-button :loading="loading || !portfolioId" @click="refreshPortfolio">刷新数据</el-button>
          <el-button :loading="loading || !portfolioId" @click="backfillOnly">回填快照</el-button>
        </div>
        <div v-if="portfolioId" class="text-sm text-gray-500">当前账本：{{ portfolioId }}</div>
        <div v-if="portfolio" class="text-sm text-gray-500">交易数：{{ portfolio.transaction_count ?? 0 }}</div>
        <div v-if="exportPayload" class="text-sm text-gray-500">导出 schema：{{ exportPayload.schema_version }}</div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h3 class="font-medium mb-2">持仓</h3>
            <el-table :data="holdings">
              <el-table-column prop="symbol" label="标的" />
              <el-table-column prop="quantity" label="数量" />
              <el-table-column prop="cost_basis" label="成本" />
            </el-table>
          </div>
          <div>
            <h3 class="font-medium mb-2">快照</h3>
        ...
        <div>
          <h3 class="font-medium mb-2">风险分析</h3>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <el-card shadow="never">
              <div class="text-sm text-gray-500 mb-2">VaR / CVaR</div>
              <div v-if="varCvar" class="space-y-1 text-sm">
            ...
            <el-card shadow="never">
              <div class="text-sm text-gray-500 mb-2">仓位建议</div>
              <div v-if="positionSizing" class="space-y-1 text-sm">
            ...
            <el-card shadow="never">
              <div class="text-sm text-gray-500 mb-2">基准指标</div>
              <div v-if="benchmarkMetrics" class="space-y-1 text-sm">
        ...
        <div>
          <h3 class="font-medium mb-2">交易</h3>
          <el-table :data="transactions">
            <el-table-column prop="symbol" label="标的" />
            <el-table-column prop="trade_type" label="方向" />
            <el-table-column prop="quantity" label="数量" />
            <el-table-column prop="price" label="价格" />
            <el-table-column prop="trade_date" label="交易日" />
          </el-table>
        </div>
      </div>
    </el-card>
