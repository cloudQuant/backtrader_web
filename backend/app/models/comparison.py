...
class Comparison(Base):
    ...
    name = Column(String(200), nullable=False, description="对比名称")
    description = Column(Text, nullable=True)
    type = Column(String(20), default=ComparisonType.METRICS, nullable=False)  # 对比类型

    # 对比的回测任务 ID 列表
    backtest_task_ids = Column(JSON, nullable=False, description="回测任务 ID 列表")

    # 对比结果（JSON 存储）
    comparison_data = Column(JSON, nullable=False, description="对比结果")

    # 标记
    is_favorite = Column(Boolean, default=False, nullable=False, description="是否收藏")
    is_public = Column(Boolean, default=False, nullable=False, description="是否公开")
...
class ComparisonShare(Base):
    ...
    can_edit = Column(Boolean, default=False, nullable=False, description="是否允许编辑")
    created_at = Column(DateTime, default=datetime.utcnow)
