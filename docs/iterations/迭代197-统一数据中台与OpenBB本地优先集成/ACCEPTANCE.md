...
**编号消歧：**本文统一矩阵的 `AC-36`（正式 ID `AC-197-MATRIX-036`）只表示 sealed URI、Explorer token 与运行环境隔离的 G1/G3 验收。它不映射到相邻本地数据平台包的 `AC-197-036`，也不能由 B2 或 research-binding 的离线 fixture 取得任何通过证据；在具备真实隔离环境前，该矩阵项必须保持 `NOT_RUN`。
