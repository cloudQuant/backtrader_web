...
所有仓库内变更先进入 `feature/iteration-195-pr-governance-*` 分支并 PR → `dev`。外部设置只在对应仓库 PR 合并且 D 门签署后由管理员执行。每个提交使用显式路径 allowlist；禁止 `git add .`、清理他人改动或把外部设置截图当作代码提交。
...
5. 将 Issue Template 转为 Issue Forms：bug form 采集版本/环境/最小复现/期望与实际；feature form 采集问题、用户、范围与替代方案；`config.yml` 将一般问题指向 Discussions，并把安全问题指向 D7 已确认的私密通道。
...
/Users/yunjinqi/opt/anaconda3/bin/conda run -n base python -m pytest \
  scripts/ci/tests/test_pr_template_contract.py -q
/Users/yunjinqi/opt/anaconda3/bin/conda run -n base python scripts/ci/check_pr_template.py
...
`check_pr_template.py` 在非 PR 环境可保持其既有“未提供 PR_BODY”退出语义；测试必须显式传入 PR_BODY/CHANGED_FILES，不可把本地空环境当作通过证据。
...
- manifest 缺一个分支、错误 enforcement、遗漏 required check 或无效 bypass actor 时产生可读 diff；
...
3. 两个 Ruleset JSON 是**归一化的 desired state**，不复制 GitHub API 返回中的 ID、时间戳、actor 数据或 secrets。每个 manifest 必须包含 target ref、enforcement、bypass 原则、PR 审批、conversation resolution、force/delete 防护、code-owner review 与 required-check 占位来源。required check 名称只能在 D4 的真实 Check Run 证据得到后填写。
...
/Users/yunjinqi/opt/anaconda3/bin/conda run -n base python -m pytest \
  scripts/ci/tests/test_governance_contract.py \
  scripts/ci/tests/test_verify_github_governance.py -q
/Users/yunjinqi/opt/anaconda3/bin/conda run -n base ruff check scripts/ci
...
先以 `PR Governance` 的 report-only shadow 运行 10 个工作日。若 GitHub 支持 evaluate，则可同步 evaluate；若不支持，禁止把“无 evaluate”当作跳过观察期的理由。观察期无未解释阻断后，由 D3 指定管理员按 manifest 在 GitHub UI/API 应用 `dev`，再用 verifier/API 读回。`master` 只在三类演练完成后应用。外部设置的截图/API 摘要写入本迭代 evidence，不混入 JSON manifest。
...
git add .github/CODEOWNERS .github/governance/README.md \
  ...
  .github/governance/rulesets/master.json scripts/ci/governance_contract.py \
...
在内部草稿 PR 中演练 3 个情形：普通 R1 → `dev`、R2 → `dev`、`hotfix/master-*` → `master`。记录实际 check display name、事件重跑行为、所请求的 review 与可读阻断消息，供 D4 和 Task 3 使用。
...
**Owner:** 发布/平台 owner
**前置:** D1、D5、D6；Task 2 已消除 fork 特权路径
**Files:**
...
在非生产 release 演练中，创建一个已合并到 `master` 的测试 release PR 和符合策略的测试 tag；验证 tag 不在 `master` HEAD 时发布被阻断，正确 tag 的 artifact 包含实际 digest。不得用生产 Docker Hub 凭据在未批准环境中演练。
...
- Create: `docs/iterations/迭代195-社区PR治理与双分支协作/evidence/dry-run-matrix.md`
...
先激活 `dev`，复验一轮普通/R2 PR；再激活 `master`。每次外部设置后立即运行：
...
| Ruleset | desired manifest、API/界面读回、verifier 输出 | `master`、`dev` 都由 active 规则覆盖，或未启用原因有时限记录。 |
...
5. 当默认分支为 `master` 时，安全的治理工作流骨架已先经 `release/governance-bootstrap` PR 进入默认分支；`dev` 与 `master` 的 active Ruleset 已由 API 读回并与 desired manifest 比对，或外部能力/人员阻塞被如实保留；
...
在满足上述条件前，本文件仍是待实施的治理设计，不能宣称仓库已具备可规模化、可审计的社区 PR 治理能力。
