...
> 计划日期：2026-08-23
> 最近验收：2026-08-24
> 状态：**仓库侧候选实现已复核 / 运营验收待完成**（Task 0 已完成；D4、D7 仍阻塞；D3、D6 为有到期日的例外）
...
7. 每个受信任的 `pull_request_target` 事件（PR 打开、同步、编辑、标签变化）都生成同一个 `Governance Gate` check，而不是被条件跳过。`pull_request_review` 的 submitted/edited/dismissed 不能直接加入该特权 workflow：GitHub 会从 PR merge ref 读取该类 event 的 workflow，fork 可篡改 Gate 实现。review 变化先通过维护者重跑已有受信任 Gate，或后续经独立、受信任的 GitHub App/webhook relay 接入；在真实 bridge 证据出现前，D4 保持阻塞。
...
3. `pr-governance.yml` 只监听 `pull_request_target` 的 opened/synchronize/reopened/edited/ready_for_review/labeled/unlabeled 事件。它只读取 GitHub API metadata 与以 base SHA checkout 的受信任基线配置，始终生成稳定命名的 `Governance Gate` 结果；不得触碰 PR head。**不得**在同一 Gate workflow 加入 `pull_request_review`：该 event 使用 PR merge ref 的 workflow 文件，不能证明 Gate 实现来自受信任基线。review submitted/edited/dismissed 后由维护者重跑同一受信任 Gate；若需要自动重跑，另行引入经过安全评审的 GitHub App/webhook relay，并将其 actor、权限和实际 Check Run 证据记录为新的 D4 子门。若默认分支仍是 `master`，必须先完成 D0 的 `release/governance-bootstrap` PR，否则 `pull_request_target` 不会使用 `dev` 中刚合入的 workflow、脚本和 manifest。
...
在内部草稿 PR 中演练 3 个情形：普通 R1 → `dev`、R2 → `dev`、`hotfix/master-*` → `master`。把 PR URL、commit SHA、实际 check display name、受信任 `pull_request_target` 事件重跑行为、review 后维护者重跑（或已批准 GitHub App/webhook relay）的证据、所请求的 review 与可读阻断消息写入 `evidence/dry-run-matrix.md`。D4 决策人据此解除或继续阻塞 D4；只有解除后才能返回 Task 3 Step 5 回填并应用 required checks。
