...
| AO-15 | B2 多记录本地事实基础 | `20260911_market_data_semantic_record_keys`、`SemanticRecordKey`、`B2SliceSelector`、`B2ReportSelector`、`MarketDataStore` 与内部 `MultiRecordLocalQueryService`；覆盖 record-key identity、PIT/revision 坐标、严格本地 completeness、exact-event SQL 与无公共路由边界。四个 B2 family 仍未配置。 |
...
- 四个 B2 多记录 family 的真实 provider、family-specific dimension/selector manifest、durable zero-record receipt、由 `MarketDataAccessGrant` 派生的 public authorization、MySQL/PostgreSQL writer locking/规模、以及浏览器/API/数据库三方链路；L-197-36 的内部 Store/reader 不能替代这些证据。
...
| E-197-19 | B2 多记录本地事实基础 | semantic record key、singleton migration、`(series,event,key,revision)` 唯一性、V3 revision identity、严格 selector completeness、exact-event Store read、internal local-only cursor 与 source-filter containment | `PASS`（L-197-36 本地开发回归）；公开启用与真实环境验收仍 `NOT_RUN / NO-GO` | 需要冻结候选上的 SQLite 升级/重放及 MySQL/PostgreSQL 离线方言/guard 契约证据、无 route/provider/lease 静态边界、真实 MySQL/PostgreSQL writer-drain/concurrency/scale、真实 provider、durable zero-record receipt、MarketDataAccessGrant 派生授权及浏览器/API/数据库三方证据。 |
...
| AC-197-036 | 在隔离 SQLite 中执行 B2 foundation migration：升级 `20260911_market_data_semantic_record_keys`，核验历史 singleton 回填、新 `(series,event,key,revision)` 唯一约束/PIT index/检查约束和空表 downgrade 拒绝；对 MySQL/PostgreSQL 方言、反射和 maintenance-fence 做离线单元契约验证。随后同一 event 写入至少两条不同 option-like dimensions，校验独立 revision 更正、B2 缺 dimension、singleton family 多 dimension、重复 coordinate、超大 key/batch identity、篡改 V3 key 及将有效 V1/V2 hash 置于 non-singleton row；对 slice/report selector 验证缺失/重复/额外/empty/类型敏感值/zero certificate event binding；以 internal reader 验证 strict `local_only`、exact event SQL、frozen visibility anchor、HMAC cursor selector/access/PIT/expiry bind、完整页与 incomplete 空结果、nonempty verified-source allowlist 和无 route/provider/lease wiring。 | `semantic_record_key` 只能由 Store 以 family/version/dimensions 派生，legacy `source_record_key` 不能升级为 identity；同一 event 的不同键并存、同一键修订独立递增且 V1/V2 只能是 singleton。selector 必须精确匹配 expected key manifest，空结果没有 durable zero receipt 时保持 incomplete。内部 reader 只读取 anchor 内的 local facts，向 Store 传 `exact_event_at`，不触网、不补齐、不渲染 partial；它的 allowlist 仅强制 verified-source filter，任何未来 public wrapper 必须由 `MarketDataAccessGrant` 派生并绑定 policy/access-grant descriptor。四个 B2 family 继续 `DATA_FAMILY_UNCONFIGURED`，不得添加 HTTP/provider/frontend/scheduler。 | `test_multi_record_identity.py`、`test_multi_record_completeness.py`、`test_multi_record_store.py`、`test_multi_record_query_service.py`、`test_multi_record_migration.py`、`test_storage_models.py`、`test_store.py`、`test_query_service.py`、`test_dataset_contracts.py`；L-197-36 | `PASS`（本地开发基础）；真实 MySQL/PostgreSQL migration/writer concurrency/scale、真实 provider、durable zero receipt、grant-derived public authorization、页面/API/数据库三方与生产验收均 `NOT_RUN / NO-GO` |

**AC-197-036 判定边界：**本项只验收可复核的 local persistence/read foundation，不把 generic dimension normalizer 当作四个产品的业务 contract。任何 family-specific alias/strike/expiry/warehouse/entity 规则、expected manifest 签发、source completeness evidence、许可证、provider adapter、route、scheduler、公共 RBAC 或页面状态均不在本项成功范围。MySQL current read 使用精确 `(event,key)` locking predicate 并有单 chunk 行数上限，但真实 MySQL REPEATABLE READ 写者等待、锁范围、50,000 record batch 和历史 revision 压力仍需独立验收。
...
| MySQL 准生产升级、UTC session、PIT 与恢复演练 | `NOT_RUN` | 需要经授权的可恢复数据库副本、每连接 UTC 验证、维护窗口和跨连接证据；fresh upgrade 必须按第 7.1 节在 writer drain 下同时设置十一项 `MARKET_DATA_*_MAINTENANCE_FENCE=confirmed`，其中包括 shared-binding、visibility-anchor、source-receipt-evidence、source-governance、fetch-lease、exact-identity、constraint-portability、shared-source-payload、capability-ledger、publication-release-hold 和 semantic-record-key。fetch lease/capability ledger 的 MySQL downgrade 均硬拒绝，非空 release hold 或非空 observation fact table 的 semantic-record-key downgrade 也拒绝。MySQL `DATETIME` 时区语义不能靠 SQLite 推定。 |
...
| 196/197 Alembic 单 head 合并 | `PASS`（本地隔离 SQLite） | `20260909_ai_research_market_data_merge` 后依次追加 binding、consumer、shared payload、`20260910_market_data_capability_ledger`、`20260911_market_data_deferred_publications` 与 `20260911_market_data_semantic_record_keys`，当前唯一 head 为后者；ledger 不 seed 活动能力，release hold 也不发布任何 receipt，semantic key 只为历史行回填 fixed singleton，SQLite 非空 downgrade 与 MySQL fail-closed downgrade 均有本地回归。MySQL/PostgreSQL 的真实升级仍为 `NOT_RUN`。 |
...
迭代 196 已冻结并作为本候选的基线。迁移图以 `20260909_ai_research_market_data_merge` 显式合并 196 研究审批 head 和 197 数据中台 head，随后依次追加 `20260909_market_data_research_bindings`、`20260909_market_data_research_binding_consumers`、`20260910_market_data_shared_source_payloads`、`20260910_market_data_capability_ledger`、`20260911_market_data_deferred_publications` 与 `20260911_market_data_semantic_record_keys`。本地 `alembic heads` 已证明唯一 head；它不能替代可恢复 MySQL/PostgreSQL 副本或真实页面环境。

| 整合闸门 | 通过条件 | 当前状态 |
| --- | --- | --- |
| IG-196-01：接口冻结 | 196 的研究、策略、回测输入输出与数据工件 schema 已冻结，并有冻结收据。 | `PASS`（集成基线） |
| IG-196-02：迁移单头 | 196 与 197 的 Alembic 链已合并，`alembic heads` 恰一个 head，隔离升级演练通过。 | `PASS`（当前 head 为 `20260911_market_data_semantic_record_keys`；MySQL/PostgreSQL 仍 `NOT_RUN`） |
