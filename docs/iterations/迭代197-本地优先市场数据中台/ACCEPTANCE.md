...
| L-197-26 | 2026-09-10，当前待提交的策略页资产意图与 196/197 provenance 候选：`test_iteration197_acceptance_runner.py`、`test_scope_manifest.py`、`test_ai_research_direction_a.py` 与三个研究授权服务边界；策略页 Vitest、`npm run typecheck`、`npm run build`、目标 Ruff、scope manifest 验证，以及 socket-guarded `AC-01:fund:G1`。 | `PARTIAL`（局部离线）：验收运行器/范围清单 `39 passed`，Direction A `22 passed`，服务边界 `3 passed`；策略页 `142 passed`、typecheck/build 退出码 0、Ruff 与 diff 检查通过。G0 验证确认 196 真实候选 `3ebe7717a0bfe7ebf1cde2dfc501d6842034c253` 是 `fec74728ad4469ae6481b134323a6dd7d1401d32` 的第二父，原收据与更正收据 SHA 分别为 `f47d8ff435a36f53a43049a209c0b614f4bde59f8c50e2dd76d56385e42646d1`、`a02197ce712fdd43e7f85aeb88420865ab161d83d29b85e80959305e4af27f39`；scope SHA 为 `2950d1ac6f05d3a468060d07d4b2289363f21413d026e20ef844f9c8018961df`。`AC-01:fund:G1` 为 `PASS`（2 tests、0 socket attempt），但 CLI 按 G2 pending 返回 exit 3/`NOT_RUN`。 | 只证明候选的显式七资产客户端意图、mandate 精确匹配、默认无 marker 的兼容回退、G0 来源链与离线防出网边界；不得把 G1 切片或本地构建升级为正式完整 case、真实 AkShare/OpenBB、数据库、浏览器或生产验收。 |
