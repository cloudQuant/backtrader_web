...
5. 在迭代 196 未冻结其研究/回测工件契约前，197 不接管策略页面的生产读取路径。
...
1. 验收在单独的 197 工作树或其待合并候选提交执行，不能在迭代 196 的未提交工作树中混跑。
2. Python 命令使用项目约定环境：
...
| L-197-08 | 2026-09-09，代码候选 `c29d2d72`（包含前端 `0289dfdd`）执行完整后端命令、全中台 Ruff/`compileall`、`alembic heads`、一次性 SQLite `upgrade head`，以及三份 v2 前端测试、typecheck、build、lint。 | `PASS`：后端 `468 passed, 62 warnings`（107.08s）；Ruff 与 `compileall` 退出码 0；独立链唯一 head 为 `20260909_market_data_exact_identity_collation`，临时 SQLite 升级到该 revision 且 `asset_instruments`、`md_fetch_leases`、identity/lookup 表存在；前端 163 tests、typecheck/build 通过，lint 为 0 errors/1,225 warnings。 | 覆盖 B1 三个候选 family 的精确 route/合同、reference-series calendar-grid 上限、`DATA_KIND_COVERAGE_UNSUPPORTED` 的多记录拒绝、市场页显式选择/在途请求作废/必填字段拒绝及范围清单。它仍只是独立工作树中的离线开发证据；E-197-01 至 E-197-10、真实 provider/多方言/多 worker、浏览器灰度和 196 整合状态均不变。 |
| L-197-09 | 2026-09-09，候选代码提交 `a37f0514`、`b3c52283`、`b978b3a7`：完整后端命令、目标 Ruff/`compileall`、`alembic heads`、临时 SQLite `upgrade head`，以及三份 v2 前端测试、typecheck、build、lint。 | `PASS`：后端 `473 passed, 62 warnings`（99.83s）；Ruff 与 `compileall` 退出码 0；独立链唯一 head 为 `20260909_market_data_exact_identity_collation`，临时 SQLite 升级到该 revision 且 `asset_instruments`、`md_fetch_leases`、`md_instrument_lookup_keys`、`md_observation_revisions` 存在；前端 166 tests、typecheck/build 通过，lint 为 0 errors/1,225 warnings。独立代码审查未发现 P0/P1。 | 覆盖 reference-series 的显式 calendar grid 与跨 session 本地复读、页面对标的/市场/周期/日期窗口/family 的在途响应作废，以及三个 B1 family 四个语义轴的 exact binding；FX 的 `currency=null`、`unit=null` 经 JSON 保留，省略或变更任一轴在 provider I/O 前拒绝。它仍只是独立工作树中的离线开发证据；真实 AkShare/OpenBB、MySQL/PostgreSQL、多 worker、浏览器灰度和 196 整合状态均不变。 |
...
当前状态：`NOT_RUN`。一次真实 `yfinance` 小窗口调用收到上游 HTTP 429 限流，不能作为 OpenBB route、持久化、索引/interval 修正或隔离的成功证据；待上游恢复、限频窗口允许且许可确认后，需重新执行第 4、5、7 步。临时 fake runner 的协议测试也不等同于本机 OpenBB、`openbb-docs`、`agents-for-openbb` checkout 或 GitHub 社区仓库中的任一扩展已安装、可用或获授权。

## 9. 迭代 196 整合闸门

迭代 197 可以在独立工作树完成中台实现和离线回归，但不能在 196 尚未完成时把以下事项标为完成。当前迁移图的确定事实是：197 `20260908_market_data_catalog` 和 196 `20260904_ai_research_protocol_v2` 都以 `20260811_asset_research_task_leases` 为 `down_revision`，而 196 另有后续审批修订。因此把两条链简单合并将出现两个 Alembic head；197 工作树里单独看到的单链结果不能用来掩盖该问题。

| 整合闸门 | 通过条件 | 当前状态 |
| --- | --- | --- |
| IG-196-01：接口冻结 | 196 的研究、策略、回测输入输出与数据工件 schema 已冻结，并有批准的兼容性说明。 | `BLOCKED` |
| IG-196-02：迁移单头 | 196 与 197 的 Alembic 链已合并，`alembic heads` 恰一个 head，升级演练通过。 | `BLOCKED` |
| IG-196-03：工件绑定 | 策略/研究结果保存 canonical identity、metadata version、数据系列、来源策略、日历版本、PIT 截止点、修订/快照和工件指纹。 | `BLOCKED` |
| IG-196-04：页面灰度 | `/data/market` 在功能开关下使用 v2，`/investment/strategies` 在严格工件链可用后消费 v2；两个页面均无旧样例/模糊回退。 | `BLOCKED` |
| IG-196-05：端到端回归 | 相同用户权限、相同请求在灰度前后均可解释；开关关闭可回到受支持的旧接口，不删除 197 的证据事实。 | `BLOCKED` |

解除以上阻塞前，197 的 API 只能保持默认关闭或用于隔离验证。任何“页面已经接入”结论都必须附上前端 build、浏览器用例、API trace、数据库回执和已冻结 196 工件的一致性证据。

IG-196-02 的唯一可接受处置是：在 196 冻结后将 197 重基到冻结 head，或创建带两个 `down_revision` 的显式 merge revision；随后在空库及可恢复 MySQL/PostgreSQL 副本上记录唯一 `alembic heads` 和 `alembic upgrade head` 输出。不能通过任选一个 head、`stamp` 掉另一个分支或先上线独立 197 链来解除阻塞。
