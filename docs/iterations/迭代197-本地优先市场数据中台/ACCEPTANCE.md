...
> 当前结论：**尚未达到发布验收条件。** 迭代 196 已冻结并接入 `dev`；本地自动化只记录当前候选的开发回归，真实环境和页面灰度证据仍须独立记录。没有命令输出、数据库快照或外部回执的项目不得标记为 `PASS`。本次 OpenBB yfinance fork 记录是文档与离线构件候选更新：没有安装扩展、构建镜像、创建 permit/route、发起真实 OpenBB/yfinance 网络调用或写入市场数据。当前 `/api/v1/data/kline` 仍直接调用 AkShare，未通过本文件的受治理兼容验收；AC-197-031 和 L-197-25 在本次只作为 `NOT_RUN` 的实现/证据占位。
...
| AO-14 | `/api/v1/data/kline` 的受治理 legacy projection | 当前路由仍直接调用 AkShare，故本对象尚未实现。正式候选必须先新增拟议 `stock.kline_legacy` family 及其拟议 `market-data-kline-v1` server-owned full-OHLCV contract，再由 v2 access/capability/contract/query/read-back 链投影既有 JSON；当前代码没有该 family/version，见 AC-197-031。 |
...
- 当前 `/api/v1/data/kline` 的受治理替换：现有路由直接 AkShare 的行为既不是本地优先，也不具备 `data:read`、capability、exact qfq contract、durable lease、receipt、publication 或本地 reread 证据。AC-197-031 尚未执行，不能用现有接口测试或历史 K 线 mock 代替。
...
| E-197-17 | `/api/v1/data/kline` 受治理兼容桥 | 拟议的受限 family/version pair（保留 `{stock.realtime: market-data-family-v1}`，只新增 `{stock.kline_legacy: market-data-kline-v1}`）、full-OHLCV contract、`Asia/Shanghai` 参数/窗口限额、`data:read`、effective capability、exact `qfq` contract、local-first、receipt/publication、本地 reread、accepted-event-key/field-quality/唯一序列化、legacy projection 与拒绝矩阵；当前代码尚无该 pair/family/version。 | `NOT_RUN` | 冻结候选上的定向后端回归、静态禁止 direct AkShare/`MarketInstrumentService` 调用的检查，以及独立验收库中的 local hit、受控补齐与 local-only reread 证据。当前 direct-AkShare 路由不能充当本项证据。 |
...
| L-197-25 | **预留本地候选证据位（AC-197-031）**：拟新增的受限 `stock.kline_legacy / market-data-kline-v1` pair、`MarketDataFamilyContractVersion` coexist migration、`Asia/Shanghai` 参数 profile、v2 compatibility bridge 与定向回归尚未实施；本记录不对应命令、提交或测试输出。 | `NOT_RUN`。 | 该行仅防止把现有 `/api/v1/data/kline` 的 direct-AkShare 行为或历史 K 线 mock 误作为候选证据。实施后必须以冻结候选 SHA、明确命令、stdout 摘要、静态 direct-call 禁止检查、独立验收库的 local hit/受控补齐/local-only reread、pair coexist、`Asia/Shanghai` date/window limit、accepted-event-key、field-quality、单页和唯一序列化拒绝矩阵替换本占位；在此之前 AC-197-031 不得标为 `PASS`。 |
...
| AC-197-031 | **拟新增受治理 `/api/v1/data/kline` compatibility bridge。** 在 current route 仍 direct AkShare 的基线先执行静态审计；随后，在冻结候选和隔离验收库中验证：无 `data:read`；query capability 缺失/关闭/过期/非法；拟新增 `stock.kline_legacy / market-data-kline-v1` pair 缺失、现有 `{stock.realtime: market-data-family-v1}` token/binding 被错误转换、自由/跨 family version、非精确 identity 或非 `qfq`；daily 不是 `Asia/Shanghai` ISO 标签或超过 366 日，weekly 不是周一至周日或超过 260 完整周，monthly 不是月初至月末或超过 120 完整月，以及无效/倒置日期；本地完整、无日历/缺行/缺字段/未发布/不完整 coverage；批准 gap 的 v2 provider→receipt→事务 A/B→local-only reread；以及 execution 返回 `next_cursor`、部分页或空 provider 内存结果；故意制造 `coverage.accepted_event_keys` 的缺失、额外、重复或重排，以及 OHLCV/`change_pct` 的布尔值、`NaN`、正负无穷或空文本；再让首个 v2 请求携带非空 cursor；并构造同一日期多行、日期未按 `Asia/Shanghai` 升序、array/record 索引错位或长度/count 不等、OHLC/change 不是两位小数 JSON number、change 被写成比例而非百分比点、以及 volume 为负、小数、布尔或超出 JSON safe-integer 范围。 | 当前代码因没有拟新增 family、contract、profile、route 或 bridge 而必须保持 `NOT_RUN`；现有 direct-AkShare `/kline` 不可作为正例。正式实现后：无 `data:read` 为 403 且零 control-plane/data/provider I/O；日期/period/window/page 输入为 422；capability、family/version pair、contract/route、calendar、coverage、accepted-event-key 精确性、字段质量、唯一序列化、publication、首请求 cursor 或 legacy pagination 不满足为 503。唯一成功路径必须是 `data:read → effective capability → exact qfq contract → v2 local_first → (获批缺口才 provider/lease/receipt/publication) → local_only reread → 完整 OHLCV + change_pct projection`，且由 server-registry 签发正确 family/version pair，reread 的无重复 canonical observation event-key 序列与 `coverage.accepted_event_keys` 精确相等，首请求无 cursor、单页覆盖完整按 `Asia/Shanghai` 生成的 server-owned legacy window 并返回 `next_cursor=null`。投影 dates 为 `Asia/Shanghai` `YYYY-MM-DD` 升序，每个 accepted event 恰一行，`count` 与全部 arrays/records 长度相等且同索引对齐；OHLC/change 为两位小数 JSON number，change 为百分比点，volume 为非负 exact JSON safe integer。不得截断、伪造 `change`、直接返回 provider DTO，或调用/import AkShare、`MarketInstrumentService`、遗留专表作为任何 fallback。 | 未来 `test_legacy_kline_v2_bridge.py`、family/contract/query-service/provider/store 回归、pair coexist、`Asia/Shanghai` date/window、accepted-event-key/field-quality/cursor/serialization 拒绝回归、静态调用图检查与独立验收库证据；本地候选占位为 L-197-25。 | `NOT_RUN` |

**AC-197-031 判定细则（仅适用于未来冻结候选；当前均为 `NOT_RUN`）：**

1. server registry 只可签发 `{stock.realtime: market-data-family-v1}` 或拟新增 `{stock.kline_legacy: market-data-kline-v1}` 中与目标 family 精确匹配的一对；旧 v1 token、cursor、contract 和 binding 必须仍按旧 pair 验证，不能转换或重签。任何自由、跨 family 或错配 pair 都必须在 control-plane、事实或 provider I/O 前返回 503。
...
- [ ] AC-197-031 已证明受限 family/version coexistence（保留 `{stock.realtime: market-data-family-v1}`，仅新增 `{stock.kline_legacy: market-data-kline-v1}`，既有 v1 token/binding 不转换）、`Asia/Shanghai` daily/weekly/monthly 标签和 366 日/260 完整周/120 完整月边界、完整 OHLCV + `change_pct` profile、无重复 observation event-key 与 `coverage.accepted_event_keys` 精确相等、统一 field-quality、首请求无 cursor/单页完整窗口，以及 dates/records/arrays 的一一对应、两位小数 JSON number/百分比点 change/nonnegative exact safe-integer volume 序列化。authorized capability→exact `qfq` contract→v2 `local_first`→receipt/publication→本地 reread→legacy projection 闭环均已在冻结候选和独立验收库证明；当前 direct-AkShare `/kline` 不可计入。
