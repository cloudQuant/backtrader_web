...
| E-197-06 | `/data/market`、`/investment/strategies` 端到端回归 | 页面灰度、授权、回退防护、196 工件绑定 | `NOT_RUN`（正式浏览器/API/数据库三方）；L-197-34 仅为 `/data/market` 的静态 fixture 浏览器协议回归。 | 保存浏览器/API/数据库三方一致证据；196 冻结和桥接不等于浏览器或真实数据通过。 |
