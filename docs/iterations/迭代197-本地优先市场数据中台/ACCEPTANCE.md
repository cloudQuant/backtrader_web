...
> 文档状态：196/197 当前实现候选；历史本地回归、legacy K 线增量、public HTTP + SQLite local-first 闭环及行情页静态浏览器协议回归均已记录（2026-09-11）
> 适用工作树：当前迭代 197 实现候选；文中较早候选 SHA、分支名与测试计数均为历史快照
> 当前结论：**尚未达到发布验收条件。** 迭代 196 已冻结并接入 `dev`；本地自动化只记录当前候选的开发回归，真实环境和页面灰度证据仍须独立记录。没有命令输出、数据库快照或外部回执的项目不得标记为 `PASS`。本次 OpenBB yfinance fork 记录是文档与离线构件候选更新：没有安装扩展、构建镜像、创建 permit/route、发起真实 OpenBB/yfinance 网络调用或写入市场数据。`/api/v1/data/kline` 的受治理 bridge 已完成 L-197-25 冻结本地回归；L-197-33 已在 ASGI + SQLite + 无网络 fixture 中验证 public `/queries` 的 local hit、精确补齐和独立 `local_only` 重读；L-197-34 已在静态 Vite 预览与无网络 Playwright route fixture 中验证 `/data/market` 的请求意图；L-197-35 已在前端与后端 fixture 中验证策略研究的严格本地门控；L-197-36 记录 B2 semantic-record-key/Store/local-reader/migration 的早期离线基础，L-197-37 再记录内部 durable receipt/manifest、双 publication 可见性和 receipt-bound strict reader 的本地回归。它们都不替代真实 provider、独立验收数据库、浏览器/API/数据库三方或生产证据，本文件不将本地结果表述为正式 `PASS`。

> L-197-38 将统一矩阵的 `AC-17` OpenBB runner 证据纳入七个资产的干净候选 `G1` 记录：每个 slice 的本地 G1 通过且未观察到 Python child socket attempt，但 `G3` 仍未运行，顶层结果必须保持 `NOT_RUN`。该记录不改变 E-197-05、E-197-09 或发布签收状态。

> L-197-39 将统一矩阵 offline G1 映射从 7 个 case 扩展到 24 个 case（新增 `AC-03`—`AC-08`、`AC-10`—`AC-14`、`AC-16`、`AC-18`、`AC-22`、`AC-30`、`AC-37`、`AC-40` 的确定性 pytest 驱动），并在干净候选上一次执行全部 55 个映射 slice：合计 115 个 JUnit testcase、0 failures/errors/skips、0 socket attempt。无 offline 驱动的 120 个资产 slice（含刻意留空的 `AC-36`）按设计 fail-closed 为 `NOT_RUN`；所有 G2/G3/G4 仍 pending，顶层保持 exit 3 / `NOT_RUN`。该记录不解除任何真实环境或发布闸门。

> L-197-40 把受审核 AkShare 验收 harness 扩展到四个 B1 family 并执行 `fund.liquidity`、`fund.nav`、`fx.range` 三条路线的真实受限探测：三者均如实 `FAIL`（间歇空返回/限流、上游字段漂移稳定码、零行快照），无 observation/publication/`local_only` 复读产生；`E-197-04` 保持 `FAIL`。详见第 8.1 节与[收据](evidence/2026-09-11-l197-40-akshare-b1-live-probes.txt)。
...
| AO-14 | `/api/v1/data/kline` 的受治理 legacy projection | 候选代码已用私有 `stock.kline_legacy / market-data-kline-v1` full-OHLCV + `change_pct` contract 保留既有 JSON 形状，并由 `data:read → capability → exact contract → local_first →（获批缺口才 receipt/publication）→ local_only reread → fail-closed projection` 链路执行。该 pair 不能由 public `/queries`、`query-contract` 或 bundle 选择；handler/bridge 不得直接 import/call AkShare、`MarketInstrumentService` 或遗留专表。见 AC-197-031；真实 provider、独立数据库、浏览器和生产验收仍未运行。 |
| AO-15 | B2 多记录本地事实与内部 completeness evidence | `20260911_market_data_semantic_record_keys`、`20260911_market_data_b2_completeness_evidence`、`SemanticRecordKey`、四个 versioned B2 dimensions contract、`B2SliceSelector`、`B2ReportSelector`、immutable receipt/manifest、`MarketDataStore` 与内部 `MultiRecordLocalQueryService`；覆盖 record-key identity、PIT/revision 坐标、同 source/exact-event manifest、双 publication visibility、durable zero evidence、receipt-bound strict local read 与无公共路由边界。四个 B2 family 仍未配置。 |
...
- `/api/v1/data/kline` bridge 的真实来源、独立验收数据库、浏览器和生产验证：候选代码已具备 `data:read`、capability、exact qfq contract、durable lease、receipt/publication 与本地 reread 边界，且 AC-197-031 的冻结本地回归已记录；真实 local hit/受控补齐/local-only reread 和外部验收仍未完成，不能用历史 K 线 mock 代替。

- 四个 B2 多记录 family 的真实 provider-backed family/source completeness assertion、真实 zero result、由 `MarketDataAccessGrant` 派生的 public authorization、MySQL/PostgreSQL writer locking/规模、以及浏览器/API/数据库三方链路；L-197-37 的内部 dimensions/receipt/reader 机制不能替代这些证据。
...
7. 若需启用 OpenBB，`OPENBB_MARKET_DATA_RUNNER` 必须精确为绝对 Python、`-I`、`-S`、绝对 runner 脚本四段；父进程侧、非秘密的 `OPENBB_MARKET_DATA_RUNNER_PROTOCOL` 也必须精确为 `openbb-market-data-v2`，缺失、v1 或未知值必须在任何 spawn 前以 `OPENBB_RUNNER_PROTOCOL_UNSUPPORTED` 拒绝，且不得转交 runner。每个真实请求先从已验证四段命令内部派生固定 `--protocol-self-check`：关闭 stdin，要求 manifest/OpenBB-import 前的 v2 JSONL transport receipt，并通过继承 ACK fd 保持 preflight leader 至 parent cleanup；timeout、旧 JSON-load peer、v1/无效 receipt 或 cleanup failure 必须为 `OPENBB_RUNNER_PROTOCOL_UNATTESTED`，正常请求不得启动。该固定派生 flag 不是命令第五段，不能由运维配置，也不能启用 route。恰为 `yfinance` 的 `OPENBB_ALLOWED_PROVIDERS`、独立且绝对存在的 `OPENBB_RUNNER_HOME` 与 `OPENBB_RUNNER_WORKDIR` 也须由 runner 运维方审核。所有 runner CLI 入口必须在 manifest、distribution metadata、candidate identity 或动态扩展导入前验证 `-I -S`；普通 Python 启动仅可返回最小 `OPENBB_RUNTIME_ISOLATION_UNATTESTED` 阻断结果。后二者不可缺失、回退为临时目录/主应用 HOME/工作树，主应用进程也不能把自身的数据库凭据、项目工作树或服务账户权限作为 runner 前置条件。当前 permit matrix 为空。fork `24d06a7657ab9e19d07b5ba4f801394a440287a1` / `openbb-yfinance 1.6.3.post1` 仅是 `1d`、UTC 日对齐、最长 3650 天、包含式 OpenBB 日终到排他 yfinance `end` 的构件候选；在完整隔离导入闭包、不可变镜像、AGPL-3.0-only 许可证和最小出网审计完成前，正常请求必须在动态扩展导入前拒绝，不能用这些配置启用 route。
...
同一冻结候选执行 legacy K 线 bridge 的定向回归；本次命令、stdout 摘要和结果已记录在 L-197-25：

```bash
/Users/yunjinqi/opt/anaconda3/bin/conda run -n base python -m pytest -q \
  tests/market_data_platform/test_legacy_kline_api.py \
  tests/market_data_platform/test_legacy_kline_projection.py \
  ...
  tests/test_backward_compat_iter170.py \
...
# legacy K 线 handler 的 AST/source 回归和 bridge/projection 的静态扫描
# 都必须证明没有 direct AkShare 或 MarketInstrumentService fallback。
/Users/yunjinqi/opt/anaconda3/bin/conda run -n base python -m pytest -q \
  tests/market_data_platform/test_legacy_kline_api.py \
  -k no_direct_akshare_or_market_instrument_fallback \
  -p no:cacheprovider --tb=short
! rg -n '(^|[^[:alnum:]_])(akshare|MarketInstrumentService)([^[:alnum:]_]|$)' \
  app/services/market_data/legacy_kline_bridge.py \
  app/services/market_data/legacy_kline_projection.py
```
...
| E-197-17 | `/api/v1/data/kline` 受治理兼容桥 | 已实现受限 private family/version pair（保留 `{stock.realtime: market-data-family-v1}`，只新增 `{stock.kline_legacy: market-data-kline-v1}`）、full-OHLCV + `change_pct` contract、`Asia/Shanghai` 参数/窗口限额、`data:read`、effective capability、exact `qfq` contract、`local_first`、receipt/publication、`local_only` reread、coverage/event/field/唯一序列化和 legacy projection 拒绝矩阵。private pair 不能由 public `/queries`、`query-contract` 或 bundle 选择。 | 候选代码 `DONE`；L-197-25 冻结本地回归 `PASS`；正式验收 `NOT_RUN / NO-GO` | 已记录定向后端回归与 handler/bridge direct AkShare/`MarketInstrumentService` 禁止检查；独立验收库的 local hit、受控补齐和 `local_only` reread、真实 provider、浏览器与生产证据仍待完成。 |
| E-197-18 | public `/api/v1/data/queries` 本地优先 HTTP 闭环 | 当前 `data:read` 用户、真实 FastAPI ASGI 路由、每个请求新的 SQLite session、真实 resolver/Store/publication/lease，与精确绑定 `stock.realtime / market-data-family-v1` 的 range-bounded provider fixture。先验证窄窗口 `local_only` 命中零新 fetch，再以宽窗口 `local_first` 只补 `[2026-09-02, 2026-09-03)` 缺口，最后用独立宽窗口 `local_only` 重读持久化 revision/source snapshot。 | `PASS`（L-197-33 本地开发回归）；正式候选、浏览器、真实 AkShare/OpenBB、MySQL/PostgreSQL 与部署验收仍 `NOT_RUN`。 | 同一冻结候选的目标 pytest stdout、无网络 fixture、三次 HTTP response receipt/revision/source snapshot 对应关系见 L-197-33 的受版本控制收据；实际 capability ledger、浏览器和真实数据库仍需独立证据。 |

| E-197-19 | B2 多记录本地事实与 internal durable completeness evidence | semantic record key、singleton migration、`(series,event,key,revision)` 唯一性、V3 revision identity、四个 versioned dimensions contract、immutable receipt/child manifest、同 source/exact-event proof、publication/recovery integrity、strict selector completeness、exact-event/source SQL、receipt-bound cursor 与 source-filter containment | `PASS`（L-197-36、L-197-37 本地开发回归）；公开启用与真实环境验收仍 `NOT_RUN / NO-GO` | 需要冻结候选上的 SQLite 升级/重放及 MySQL/PostgreSQL 离线方言/guard 契约证据、无 route/provider/lease 静态边界、真实 MySQL/PostgreSQL writer-drain/concurrency/scale、真实 provider-backed family/zero assertion、MarketDataAccessGrant 派生授权及浏览器/API/数据库三方证据。 |

`E-197-01`、`E-197-02`、`E-197-03` 与 `E-197-11` 的状态仅表示正式候选；本地工作树回归单列于下节。`E-197-04` 已有一次真实子用例失败，不能以其它离线通过记录覆盖为 `NOT_RUN` 或 `PASS`。`E-197-14` 的局部 G1 通过只证明离线 test slice；`E-197-15` 的失败阻断发布签收，但不改变用户已授权的候选提交与 `dev` 合并。`E-197-18` 的 ASGI/SQLite fixture 闭环及 L-197-34 的静态浏览器 fixture 均不启用部署开关，也不替代浏览器/API/数据库三方、真实 provider、真实方言或发布签收。任何本地命令退出码、冻结收据和已合并的 Alembic 图均不能升级为真实环境或发布签收。
...
| L-197-23 | 2026-09-10，当前 `dev` 工作树，以隔离临时 SQLite 运行 `/Users/yunjinqi/opt/anaconda3/bin/conda run --no-capture-output -n base python scripts/accept_iteration197_akshare_stock_liquidity.py --live --trading-date 2024-09-03`。 | `PASS`（真实 AkShare 单路线、非生产临时库）：`stock.liquidity` / `akshare-stock-liquidity-primary-v1` 发起 1 次 provider 请求，返回、标准化、持久化各 1 条 observation；首次 `local_first` coverage 为 `complete`，有 1 个 source snapshot 和 1 个 revision；独立新 session 的 `local_only` coverage 为 `complete`、0 provider 调用、0 persisted fetch；临时数据库已删除，输出不包含原始载荷或凭据。 | 仅证明 2024-09-03 的一只 CN-SSE 股票日线流动性路线在 disposable SQLite 中完成 provider→store→local-only 闭环。它不推翻 L-197-11 对 2024-09-02 的零行结果，也不证明其它 AkShare route、真实生产来源授权、MySQL/PostgreSQL、跨 worker、浏览器或发布验收。 |
| L-197-24 | 2026-09-10，当前 `dev` completion candidate：`pytest -q tests/market_data_platform tests/test_config.py -p no:cacheprovider --tb=short`；市场页两份 Vitest 文件、`npm run typecheck`、`npm run build`；`alembic heads`、隔离 SQLite `upgrade head`、scope manifest 重新生成/验证、目标 Ruff 和 `git diff --check`。 | `PASS`（仅本地）：后端 `717 passed, 95 warnings`（217.09s）；前端 `87 passed`、typecheck/build 退出码 0；唯一 Alembic head 为 `20260910_market_data_capability_ledger`，临时 SQLite 从空库升级后存在 `md_capability_ledger_entries`；scope manifest 返回 `SCOPE_MANIFEST_GENERATED` 与 `SCOPE_MANIFEST_VALID`，canonical SHA-256 为 `e06de85ea5a48a265c7737cba9ec1ebdd1b583b45699ec3e9873fca1bf2b1265`；OpenBB 回归验证三个 runner 入口先检查 `-I -S`，普通 Python 不读取 manifest/metadata 且不发起 Python socket 连接。Ruff 与 diff 检查通过。 | 证明 durable capability ledger、local-read/online-I/O 分界、受限 B1 UI allowlist、未配置家族的 pre-catalog 拒绝、本地 Python 启动拒绝，以及 MySQL downgrade fail-closed / PostgreSQL evidence-lock 的迁移边界；不证明真实 OpenBB/route permit、动态扩展闭包、OS/container/egress/凭据隔离、真实 provider、MySQL/PostgreSQL、浏览器 E2E 或发布验收。E-197-05、E-197-06、E-197-07、E-197-09 和相关 `NO-GO` 状态不变。 |
| L-197-25 | **当前 legacy K 线实现候选 `ada3c0fec90b00f2b84d0fb3b58ec5057c38e98f` 的本地回归（AC-197-031）**：冻结后执行 `pytest -q tests/market_data_platform/test_legacy_kline_api.py tests/market_data_platform/test_legacy_kline_projection.py tests/market_data_platform/test_dataset_contracts.py tests/market_data_platform/test_legacy_contract.py tests/market_data_platform/test_query_service.py tests/market_data_platform/test_akshare_provider.py tests/test_data_api.py tests/test_backward_compat_iter170.py tests/test_api_routes.py -p no:cacheprovider --tb=short`；并执行完整 `pytest -q tests/market_data_platform tests/test_config.py -p no:cacheprovider --tb=short`、目标 Ruff、`alembic heads`、handler/bridge 静态禁止检查与 `git diff --check`。 | `PASS`（仅本地自动化）：K 线组合 `282 passed, 3 warnings`（82.65s）；完整中台/配置 `811 passed, 95 warnings`（245.63s）；目标 Ruff、唯一 Alembic head `20260910_market_data_capability_ledger`、handler/bridge 直接 AkShare/`MarketInstrumentService` 禁止检查和差异检查通过。 | 证明本候选的 private pair 控制面拒绝、pair coexist、冻结 identity alias、`Asia/Shanghai` date/window（含可构造性极值）、coverage/event-key、field-quality、单页及唯一序列化拒绝矩阵。它不证明独立验收库的 local hit/受控补齐/`local_only` reread，也不证明真实 provider、数据库、浏览器或生产验收；AC-197-031 仍为正式 `NOT_RUN / NO-GO`。 |
| L-197-26 | 2026-09-10，策略页资产意图、196/197 provenance 与 strict-bars family 候选：`test_iteration197_acceptance_runner.py`、`test_scope_manifest.py`、`test_ai_research_direction_a.py` 与三个研究授权服务边界；另以 `test_research_binding.py` 覆盖 server-owned seven-asset family、六类 ready 资产实际 binding，以及 crypto snapshot family 注入回放拒绝。策略页 Vitest、`npm run typecheck`、`npm run build`、目标 Ruff、scope manifest 验证，以及 socket-guarded `AC-01:fund:G1`。 | `PARTIAL`（局部离线）：既有验收运行器/范围清单 `39 passed`，Direction A `22 passed`，服务边界 `3 passed`；本次 strict-bars 组合为 `68 passed, 10 warnings`，策略页为 `148 passed`，typecheck/build 退出码 0、Ruff 与 diff 检查通过。严格路径将 stock/futures/bond/fund/option/fx 分别固定到其 `*.realtime` bars family，crypto 固定到未配置的 `crypto.range`；crypto 日/周/月在 contract/query/provider/artifact 前失败，页面识别 FastAPI `detail.code` 与规范化 `details.code`，且不查询 facts 或 legacy。G0 验证确认 196 真实候选 `3ebe7717a0bfe7ebf1cde2dfc501d6842034c253` 是 `fec74728ad4469ae6481b134323a6dd7d1401d32` 的第二父，原收据与更正收据 SHA 分别为 `f47d8ff435a36f53a43049a209c0b614f4bde59f8c50e2dd76d56385e42646d1`、`a02197ce712fdd43e7f85aeb88420865ab161d83d29b85e80959305e4af27f39`；scope SHA 为 `2950d1ac6f05d3a468060d07d4b2289363f21413d026e20ef844f9c8018961df`。`AC-01:fund:G1` 为 `PASS`（2 tests、0 socket attempt），但 CLI 按 G2 pending 返回 exit 3/`NOT_RUN`。 | 只证明候选的显式七资产客户端意图、mandate 精确匹配、默认无 marker 的兼容回退、strict-bars 选择/拒绝、G0 来源链与离线防出网边界；不得把 G1 切片或本地构建升级为正式完整 case、真实 AkShare/OpenBB、数据库、浏览器或生产验收。 |
| L-197-33 | 2026-09-11，冻结实现候选 `2dfb691bf52ada3c707c8af4c4ae5f7b14b9413c`：干净 detached worktree 的 `git status --short` 为空；执行 `/Users/yunjinqi/opt/anaconda3/bin/conda run --no-capture-output -n base python -m pytest -q tests/market_data_platform/test_local_first_persistence.py -p no:cacheprovider --tb=short`。新用例 `test_public_http_local_first_persists_exact_gap_then_independent_local_only_rereads` 使用实际 ASGI `POST /api/v1/data/queries`、认证 header、当前 `data:read` 角色、真实 catalog/identity/calendar/Store/publication/lease，以及只按请求半开窗口返回数据的 fixture provider。 | `PASS`（本地开发）：目标用例 `4 passed, 1 warning in 2.49s`；同一干净候选完整 `tests/market_data_platform` 为 `898 passed, 102 warnings in 278.74s`。三次 HTTP 请求依次验证窄窗口 `local_only` 的 `fetches=[]` 和零 provider 调用、宽窗口 `local_first` 只补 `[2026-09-02, 2026-09-03)`、精确 route/family/version 且只写入 1 条 revision，以及新 HTTP session 的宽窗口 `local_only` 返回相同 revision/source snapshot 对并保持 `fetches=[]`。请求时钟递增，覆盖 publication 的 post-commit 可见性边界。 | 完整原始 stdout、候选 SHA、clean 状态、cwd、Python/FastAPI/SQLAlchemy 版本、SQLite fixture、命令和退出码见 [冻结收据](evidence/2026-09-11-l197-33-local-first-http-sqlite.txt)，SHA-256 为 `8fa3a9dec151c8f4d884a3770af1a22aac869896af261e500212e067d3612d4a`。provider 为无网络 deterministic fixture，capability evaluation 仅在 FastAPI test dependency override 中开启，不写 durable capability ledger 或部署设置。该记录不证明真实 AkShare/OpenBB、浏览器页面、MySQL/PostgreSQL、跨进程 worker、真实 deployment capability 或发布验收；E-197-04 至 E-197-10 的外部状态不变。 |
| L-197-34 | 2026-09-11，冻结实现候选 `7bc3aa620f25aa463bc9ca8f12f8e153baf490f7`：干净 detached worktree 的 `git status --short` 为空；在静态 Vite preview 和 Chromium 下执行 `market_data_local_first.spec.ts`，并复跑行情页组件测试、typecheck 与 production build。该浏览器用例仅允许明确列出的页面 data API 请求，并拒绝其它 `/api/v1/data/**` 路径；它验证初始请求与显式查询使用相同 `stock.realtime / market-data-family-v1` 合同，且 mode 依次为 `local_only`、`local_first`。 | `PASS`（本地静态浏览器协议）：Playwright `1 passed (2.9s)`；`DataPage.test.ts` 为 `62 passed`；typecheck/build 均退出 0。用例等待页面把 fixture provider 回执呈现为“已获取并入库”，断言 canonical identity、dataset、kind、frequency、required fields、全部语义轴、source policy、family/version、purpose/consistency 与相同窗口，并确认 bars card 是 K 线兼容数据而非实时行情。 | 完整命令、stdout、候选 SHA、clean 状态、Node/Playwright 版本和 SHA-256=`74edc7bb2b42e06f550cda6e3ec7f5f6b9fa26fe79abef38700c02098f056580` 见[冻结收据](evidence/2026-09-11-l197-34-market-page-static-preview.txt)。测试使用无网络 Playwright route fixture，Node `v25.1.0` 超出 package 声明的 `<25` 上界；它不证明真实 FastAPI、RBAC、持久化、AkShare/OpenBB、MySQL/PostgreSQL、策略页或发布验收。E-197-06 保持 `NOT_RUN`。 |
| L-197-35 | 2026-09-11，候选提交 `16fa23a24007d78798dd3f264fee301627ff3122`：对策略研究的新建、run-record 续跑与 task snapshot 续跑，强制 fresh capability 决策；在 durable query-v2 + research bridge 有效时，前端先以冻结提交快照执行 `local_only + research + strict` 预检，后端四条共享入口再拒绝无 binding intent 的旧请求。 | `PASS`（本地开发）：定向后端回归 `9 passed, 207 deselected, 1 warning in 4.97s`；策略页组件回归 `158 passed`，`vue-tsc --noEmit` 与 production build 退出 0，目标 Ruff 与 diff 检查通过。覆盖 incomplete、query error、contract null/fallback、capability 失败/并发状态竞态、冻结 symbol/asset/period/window、三类前端动作和四条服务端路径；bridge 关闭保持 legacy。 | 完整命令、工作树、退出码、范围、独立审查和残余边界见[冻结收据](evidence/2026-09-11-l197-35-strategy-strict-local-gate.txt)。所有数据/能力均为 fixture；未执行真实 FastAPI 浏览器/API/数据库三方、capability ledger、AkShare/OpenBB、MySQL/PostgreSQL 或部署。桥接由关闭切换为有效时仍须在部署层做请求排空/围栏；E-197-06 及正式 strict research 验收保持 `NOT_RUN`。 |
| L-197-36 | 2026-09-11，代码候选 `acece716c5e4fb146edc1568555f56c9e8dba77d`：实现 B2 semantic record key、singleton 兼容迁移、`(series,event,key,revision)` 修订坐标、V3 identity、selector completeness planner 与未接入路由的严格本地 reader。 | `PASS`（本地开发基础）：聚焦矩阵 `266 passed, 66 warnings in 90.64s`；目标 Ruff/格式检查、`alembic heads`（唯一 head：`20260911_market_data_semantic_record_keys`）和差异检查均通过。独立代码及迁移审查无 Critical/Important 阻断项。 | 完整命令、范围、stdout 摘要和未运行边界见[冻结收据](evidence/2026-09-11-l197-36-b2-record-foundation.txt)。四个 B2 family 仍为 `DATA_FAMILY_UNCONFIGURED`；没有 public route/provider/frontend/scheduler、真实 provider、真实 MySQL/PostgreSQL/MariaDB、durable zero-record receipt、grant-derived public authorization 或浏览器/API/数据库三方验收。 |
| L-197-37 | 2026-09-11，B2 evidence 候选 `e2bfc19e20ecb5f130d898ae3f1615b5118e031b`（实现范围 `b4f5905f..e2bfc19e`）：新增四个 versioned B2 dimensions contract、immutable receipt/manifest schema、server-owned manifest issuance、publication/recovery integrity、receipt-bound strict reader、cursor evidence binding，并将所有迁移图断言和 Iteration 197 acceptance runner sealed head 推进到 `20260911_market_data_b2_completeness_evidence`。 | `PASS`（本地开发）：`tests/market_data_platform tests/test_config.py` 为 `1030 passed, 137 warnings in 347.87s`；B2 重点回归、跨域迁移图回归、Ruff lint、单头检查和临时 SQLite fresh upgrade 均通过。完整回归先暴露两组 stale Alembic-head 断言，已修复并重跑为绿。 | 完整命令、独立 SQLite table check、审查结论、范围和未运行边界见[冻结收据](evidence/2026-09-11-l197-b2-family-evidence.txt)。本地结果只证明内部 mechanism；四个 B2 family 仍为 `DATA_FAMILY_UNCONFIGURED`，没有 public route/provider/frontend/scheduler、真实 provider、真实 MySQL/PostgreSQL/MariaDB、grant-derived public authorization 或浏览器/API/数据库三方验收。 |
| L-197-38 | 2026-09-11，干净 `dev` 候选 `70e94148b8494ddb222d555513f809d76ca478c8`：统一矩阵 `AC-17` 的 `--mode offline --asset-type all`。运行器为 stock/fund/option/crypto 绑定空 permit matrix，为 futures/bond/fx 绑定不等价上下文拒绝；每个 asset 都追加 copied real-runner 的 artifact-unattested 回归及 instrumented source-level socket regression。 | `PASS`（仅本地 G1 slice）：七个 asset slice 各 `3` 个 JUnit testcase，均为 0 failures/errors/skips，Python child socket audit `network_attempt_count=0`；每个 slice 的 `G1=PASS`。运行器顶层按预期返回 exit `3` / `NOT_RUN`，因为 `G3` 仍在 `remaining_gates`。 | 见 [L-197-38 冻结收据](evidence/2026-09-11-l197-38-openbb-ac17-g1.txt)，结果 SHA-256=`7cc69085c2cd28dd4c5d1e4768b56351776df8907cc3d90962af2d5c44dc22ec`。copied/instrumented runner 只证明协议、拒绝次序和 Python 源级 socket regression，不证明 runner/OCI 构件身份或 OS/container egress 隔离；没有安装 OpenBB、创建 permit/route、执行真实网络或写库。E-197-05 保持 `BLOCKED`，E-197-09 保持 `NOT_RUN`，不构成真实 OpenBB 或发布验收。 |
| L-197-39 | 2026-09-11，干净候选 `98644bc8ee8e0280625fcd7d61c56fa76542222a`（映射扩展提交本身；detached worktree 执行）：`case_mapping_version` 升级为 `iteration197-unified-matrix-id-v4-offline-g1-coverage`，为 17 个此前无驱动的 G1 case 绑定精确 pytest node，随后以 `--mode offline` 显式 24-case 选择执行全部映射资产 slice。 | `PASS`（仅本地 G1 slice）：55 个映射 slice 全部 `OFFLINE_PYTEST_PASSED`，合计 115 testcase、0 failures/errors/skips、0 socket attempt；120 个无驱动资产 slice（含 `AC-36` 全部 7 个）fail-closed `NOT_RUN`；不传 `--case` 的全量选择被 `ACCEPTANCE_CASE_MODE_INCOMPATIBLE` fail-closed 拒绝执行。顶层 exit `3` / `NOT_RUN`（G2/G3 pending）。 | 见 [L-197-39 冻结收据](evidence/2026-09-11-l197-39-offline-g1-matrix-coverage.txt)，结果 SHA-256=`d15cd2c96b57ce48568b9ba8350df464f4d9883051899b697850a8dc59b4687a`，`case_map_sha256=2f7664e65312c53cf10cea44a5bcac4615f9e928935ad7d80ec902c6d9840b1a`。全部为确定性离线 fixture；机制类测试按 fixture 资产映射、不以股票冒充其它资产；`AC-18` 的 Retry-After/额度/熔断合同尚无 offline 测试，本记录只覆盖超时/上限面。真实 provider、数据库、多进程、浏览器与 G2/G3/G4 均保持 `NOT_RUN` / `BLOCKED`。 |
| L-197-40 | 2026-09-11 晚间，候选 `ce99d78b3e5a44c0ff9d0d5b50b0a34e4e06f634`：AkShare 验收 harness 从单 family 扩展为四个受审核 B1 profile（离线 13 passed，含三新 profile 全链闭环与 production 轴镜像断言），随后在隔离 runner 下对 `fund.liquidity`、`fund.nav`、`fx.range` 各执行一次真实小窗口 `--live` 探测（每路线恰一次网络调用、一次性 SQLite、终态删除）。 | `FAIL`（如实记录）：`fund.liquidity` 为 `AKSHARE_ZERO_USABLE_OBSERVATIONS`（上游对同机由早前 8 行真实数据转为持续 0 行，跨标的/窗口/等待重试均空，判间歇空返回/限流）；`fund.nav` 为 `AKSHARE_RUNNER_ERROR`（akshare 1.18.64 `fund_etf_fund_info_em` 稳定抛列数不匹配，上游字段漂移；adapter 稳定码拒绝、`persisted_fetch_count=0`）；`fx.range` 为 `AKSHARE_ZERO_USABLE_OBSERVATIONS`（`forex_hist_em("USDCNH")` 两次均 0 行快照形态）。三者 coverage 均 `incomplete`，零 observation/publication/复读。 | 见 [L-197-40 收据](evidence/2026-09-11-l197-40-akshare-b1-live-probes.txt)。`E-197-04` 保持 `FAIL`：四个实际启用 AkShare 路线的真实子用例均已执行且全部 FAIL，失败均来自上游当前返回形态；本地控制面由离线回归证明。上游恢复前不得升级为 `PASS`，也不得以 fixture 替代重跑。 |
| L-197-41 | 2026-09-11 深夜，候选 `83d01792…`+`d46e1a84…`：新增 `scripts/verify_iteration197_mysql_acceptance.py`（E-197-07 MySQL 侧核心 harness，离线 6 passed），在本机 Homebrew MySQL 9.4.0 上以 root 管理 URL `--apply` 执行：UUID 临时库 → 完整迁移链 → 结构/UTC/collation/PIT/downgrade 断言 → finally 删除。验收过程以 TDD 修复四个真实 MySQL 方言缺陷（allocator AUTO_INCREMENT+CHECK 3818、`constraint_name_portability` 与 `holdout_executions` 的 CHECK 反射文本规范化、`approval_authority` 的 `_utf8mb4'...'` 引入符与 `grant` 保留字别名）。 | `PASS`（E-197-07 MySQL 侧核心）：迁移链升级到唯一 head `20260911_market_data_b2_completeness_evidence`；16 必需表、9 个 `utf8mb4_bin` 身份列、7 个 `DATETIME(6)` PIT 列实测通过；`RB0`/`rb0` 各自解析且 `Rb0` 稳定拒绝；`published_at=…772868+00:00` 前 1µs 不可见、恰在时刻可见；非空库 `downgrade -1` 被拒且 3 行身份保留。修复后 `tests/market_data_platform tests/test_config.py` 为 `1175 passed`、196 迁移回归 `153 passed, 3 skipped`、目标 Ruff 与单 head 检查通过。 | 见 [L-197-41 收据](evidence/2026-09-11-l197-41-mysql-core-acceptance.txt)。root 凭据仅命令行使用、未落盘。E-197-07 的 PostgreSQL 侧（`verify_iteration197_postgres_acceptance.py`）仍未运行；MySQL 侧多 worker/fault takeover（E-197-08）、备份恢复（E-197-45）与 BLOB drift 的真实演练未包含，相应项保持原状态。 |
...
L-197-38 使用 `iteration197-unified-matrix-id-v3-ac17-openbb-runner-g1` 映射，在干净候选中将 `AC-17` 的七个资产 G1 slice 都绑定到精确 pytest node 与 JUnit 收据。其结果只补足本地 fail-closed/协议回归的可复核性：每个 case 仍同时要求 G3，故任何一个或全部 G1 slice 的 `PASS` 都不能改写整体 `NOT_RUN`，更不能替代真实 OpenBB runtime、构件/隔离、provider→Store→`local_only` 复读、许可证或外部验收。

L-197-39 将该映射升级为 `iteration197-unified-matrix-id-v4-offline-g1-coverage`：24 个 offline 兼容 case、55 个资产 slice 全部绑定精确 pytest node，一次性干净候选执行给出可复核的 G1 台账。映射遵循三条边界：机制类测试按其 fixture 实际资产登记，不以股票 fixture 冒充其它资产；`AC-36` 刻意保持无 offline 驱动（sealed URI/隔离边界只能由真实环境证明）；`AC-18` 只映射已存在的超时/上限测试，Retry-After、共享额度与熔断预算合同在具备确定性 offline 测试前不声明覆盖。任何 G1 slice 的 `PASS` 都不改变所属 case 的 G2/G3/G4 要求，也不解除 E-197 系列正式项。
...
**编号消歧：**本节的 `AC-197-036` 是本地 B2 persistence/read/evidence mechanism 的开发验收编号，不是统一矩阵的 `AC-36` 或正式 ID `AC-197-MATRIX-036`。它没有统一验收运行器映射，不能产生 sealed URI、Explorer token、provider 或运行环境隔离的 G1/G3 通过证据；这些边界仍由相邻 [统一验收文档](../迭代197-统一数据中台与OpenBB本地优先集成/ACCEPTANCE.md) 的 `AC-36` 单独验证。
...
| AC-197-008 | 本地完整、local-only 缺口、local-first 缺口、日历未知、在线关闭和严格历史查询分别执行。 | 完整本地不触网；`local_only` 永不触网；成功补齐后只从本地返回；未知日历不声称完整；严格历史不以实时获取污染回放。L-197-33 额外以 public HTTP + SQLite fixture 验证窄窗口 local hit、宽窗口精确补齐和独立 `local_only` reread；它不替代真实来源或正式候选重跑。 | `test_query_service.py`、`test_local_first_persistence.py` | `NOT_RUN` |
...
| AC-197-021 | 对同一 snapshot 的多条 `option_chain`（不同 expiry/strike/right）和同一 report date 的多条 `position_report`（不同 reporting entity/rank）发起请求；分别尝试未绑定和绑定到目前未配置家族的路径。 | **公共产品仍为 NO-GO。** 公共 HTTP 未绑定请求在 catalog/identity/provider 前以 schema HTTP 422 拒绝；抵达 resolver 的未绑定内部请求以 `DATA_FAMILY_BINDING_REQUIRED` 拒绝；已绑定的未配置家族以 `DATA_FAMILY_UNCONFIGURED` 拒绝。L-197-36/37 已完成 internal local record-key、版本化 dimensions、immutable receipt/manifest、PIT selection、durable zero certificate、receipt-bound reader 和无路由边界，但没有注册 family/source/provider/route，且不能作为 public authorization 或真实数据证据。未来启用仍须证明当前 grant-derived authorization、provider-backed source completeness/zero assertion、provider → Store → strict PIT replay 和真实页面链路；本地单行或空响应不能作为通过证据。 | `test_query_resolution.py`、`test_query_api.py`、`test_multi_record_*.py`；未来多记录端到端回归 | `PARTIAL / NO-GO` |
...
| AC-197-031 | **已实现受治理 `/api/v1/data/kline` compatibility bridge。** 在冻结候选与隔离验收库中验证：无 `data:read`；public `/queries`、`query-contract` 或 bundle 试图选择 private `stock.kline_legacy / market-data-kline-v1`；query capability 缺失/关闭/过期/非法；private pair 缺失、现有 `{stock.realtime: market-data-family-v1}` token/binding 被错误转换、自由/跨 family version、非精确 identity、同一 token 命中多个 current/published frozen display/exchange identity 或非 `qfq`；daily 不是 `Asia/Shanghai` ISO 标签或超过 366 日，weekly 不是周一至周日或超过 260 完整周，monthly 不是月初至月末或超过 120 完整月，以及无效/倒置日期；本地完整、无日历/缺行/缺字段/未发布/不完整 coverage；批准 gap 的 v2 provider→receipt→事务 A/B→local-only reread；以及 execution 返回 `next_cursor`、部分页或 provider 内存结果；故意制造 `expected_event_keys != accepted_event_keys`、missing/gap、accepted event 越过 sealed window、accepted key 缺失/额外/重复或重排，以及 OHLCV/`change_pct` 的布尔值、`NaN`、正负无穷或空文本；再让首个 v2 请求携带非空 cursor；并构造同一日期多行、日期未按 `Asia/Shanghai` 升序、array/record 索引错位或长度/count 不等、OHLC/change 不能按两位小数 JSON number 安全投影、change 被写成比例而非百分比点、signed zero，以及 volume 为负、小数、布尔或超出 JSON safe-integer 范围。 | 候选代码已实现：无 `data:read` 为 403 且零 control-plane/data/provider I/O；日期/period/window/page 输入为 422；capability、family/version pair、contract/route、calendar、coverage、event-key 精确性、字段质量、唯一序列化、publication、首请求 cursor 或 legacy pagination 不满足为 503。legacy token 只可逐字符匹配唯一的 current/published frozen `display_symbol` 或 `details.exchange_symbol`，二者候选并集若有不同 canonical identity 则拒绝，provider 继续使用已解析 frozen display symbol。public `/queries`、`query-contract` 与 bundle 不得选择/签发/暴露 private pair。唯一成功路径是 `data:read → effective capability → exact qfq contract → v2 local_first → (获批缺口才 provider/lease/receipt/publication) → local_only reread → 完整 OHLCV + change_pct projection`；reread 必须为 complete、expected/accepted event-key 精确相等、无 missing/gap、accepted event 均在 sealed window 内，且 response 无重复 canonical event-key 序列与 accepted 序列精确相等。首请求无 cursor、单页覆盖完整 server-owned legacy window 并返回 `next_cursor=null`。投影 dates 为 `Asia/Shanghai` `YYYY-MM-DD` 升序，每个 accepted event 恰一行，`count` 与全部 arrays/records 长度相等且同索引对齐；OHLC/change 用 `ROUND_HALF_UP` 投影为两位小数 JSON number，change 为百分比点且拒绝 signed zero，volume 为非负 exact JSON safe integer。不得截断、伪造 `change`、直接返回 provider DTO，或在 handler/bridge 调用/import AkShare、`MarketInstrumentService`、遗留专表作为任何 fallback。 | L-197-25 的 `282 passed, 3 warnings` 定向组合与 `811 passed, 95 warnings` 冻结中台/配置回归；family/contract/query-service/provider/store、pair coexist、identity alias collision、`Asia/Shanghai` date/window、coverage/event-key/field-quality/cursor/serialization 拒绝与静态调用图检查均已执行。 | 候选代码 `DONE`；本地回归证据 `PASS`（L-197-25）；正式验收 `NOT_RUN / NO-GO` |
| AC-197-032 | 对 `STOCK_ZH_A_HIST` 提交非 allowlisted 表、缺失/错误 read-scope receipt、`SELECT *` 或扩大/重排投影、未封存 batch、不同 raw hash、不同物理 schema hash、不同 read-authorization receipt 的相同行、相同 raw 行但不同 source revision、calendar PIT/事件映射、identity revision 或 metadata version 的旧 scope receipt、错误/过期或跨 target/raw-batch/scope/source-receipt 重放的 write permit、多 target 缺少任一 permit、两个 bar 共用一个 observation revision、聚合 revision ID 不变但两个 bar 的 revision/source binding 被交换、任一 publication receipt 早于 Store local receipt，以及 caller 伪造早于或晚于 extraction 的 `retrieved_at`。还要让 reread 将 source extraction time 冒充 Store-owned local availability、篡改 provenance `provider_available_at`（包括格式合法但更早的 timestamp），并在独立 MySQL/PostgreSQL 验收库执行真实表导入、current authorization/lease、deferred publication/quarantine、canonical publication 与 local-only PIT reread。 | 当前候选必须在读表前获得 gate 核验的 source registry/provider/route/物理 schema/read-authorization 回执；固定八列投影后，raw batch 与完整解释范围分别哈希，batch receipt 必须绑定二者和 read-authorization identity。写前必须有逐 target 的 permit，绑定 canonical ID、current authorization descriptor、resolved context、lease key/fence、raw batch、scope 与 source receipt；错误、缺失或跨边界重放的 permit 均在 writer 前失败。sealed `extracted_at` 作为 Provider observation 上游 availability：新写入 Store v2 的 `market-data-observation-revision-v2` identity 必须将规范化 UTC `provider_available_at` 封入 `revision_key_sha256`，reread 重建/验证该 identity 后才返回 `LocalObservationRevision.source_available_at`。任何 v2 provenance 缺失、坏格式、无时区、晚于 canonical local `available_at` 或改成另一合法更早 timestamp 均失败关闭；历史 v1 identity 因未封存该字段只能返回 `source_available_at=None`，future adapter 必须拒绝。canonical `available_at` 必须保留传给 Store 的可信本地 receipt，不可采用 `PersistedProviderFetch.received_at` 的 publication 时间；writer 为每条 bar 回显唯一 revision-to-source-snapshot binding，且每个 source snapshot 的 publication receipt `visible_at` 不得早于该 local receipt。写后以可见本次 publication 的新 anchor local-only reread 逐条核对业务字段、source/local availability 和 binding。future adapter 必须逐 target 使用 resolved context、current `MarketDataAccessAuthorizer`、`MarketDataSourceAuthorization`、fetch lease 和 `MarketDataStore.stage_provider_result_for_deferred_release`；private staged reread、current authorization/lease guard 与 `promote_deferred_provider_result` 都必须在受控流程成功，且 guard、promotion 与 receipt seal 必须在同一事务 B 原子完成，才可使 receipt 可见。验证失败必须 `quarantine_deferred_provider_result` 或保持 `DEFERRED`；不得走普通 publish/recovery。 | L-197-32 原有协议回归为 `51 passed, 1 warning`；延期发布基础设施另有 SQLite/fixture 本地开发回归（durable hidden、generic publish/recovery 拒绝、stale-session race、guarded promotion、guard transaction loss、quarantine、迁移和 bootstrap）。其中 promotion 可见性断言只使用未经 source-authorization 筛选的规范化 Store read，不是 v2、页面或策略授权读取证据。完整协议、约束和启用步骤见 [遗留日线导入边界](LEGACY_STOCK_DAILY_IMPORT_GUARD.md)。 | 候选协议与 durable deferred-release 基础设施 `PASS`（本地开发回归）；真实导入、真实 provider/数据库/页面/策略验收 `NOT_RUN / NO-GO` |

| AC-197-036 | 在隔离 SQLite 中执行 B2 foundation/evidence migrations：升级 `20260911_market_data_semantic_record_keys` 与 `20260911_market_data_b2_completeness_evidence`，核验历史 singleton 回填、新 `(series,event,key,revision)` 唯一约束/PIT index/检查约束、immutable receipt/child manifest、非空 evidence downgrade 拒绝，以及 MySQL/PostgreSQL 方言、反射和 maintenance-fence 离线契约。随后同一 event 写入至少两条不同 B2 dimensions，校验独立 revision 更正、四个 exact family/version dimensions、B2 缺 dimension、singleton family 多 dimension、重复 coordinate、超大 key/batch identity、篡改 V3 key 及将有效 V1/V2 hash 置于 non-singleton row。签发时必须由服务端派生 selector/manifest/receipt，验证 restage 幂等、缺失/重复/额外/空/type-sensitive selector、zero-evidence event binding、source event key-set mismatch、parent/child/source raw-SQL 篡改、pending source/B2 publication 隐藏及恢复/直接发布重检。以 internal reader 验证 strict `local_only`、exact event/source SQL、同 anchor 双 publication、current verified-source filter、receipt-bound selector/zero certificate、HMAC cursor selector/access/PIT/receipt/source/expiry bind、完整页与 incomplete 空结果，以及无 route/provider/lease wiring。 | `semantic_record_key` 只能由 Store 以 family/version/dimensions 派生，legacy `source_record_key` 不能升级为 identity；同一 event 的不同键并存、同一键修订独立递增且 V1/V2 只能是 singleton。B2 receipt 的 parent、child manifest、zero state 与同 source/exact-event key set 必须在签发、发布/恢复和读取时重算；source receipt 与 B2 receipt 均须在同一 anchor 可见且当前来源授权有效。请求 selector 仅作 lookup，reader 只读取 receipt 的 source snapshot；证据缺失为 `DURABLE_SELECTOR_EVIDENCE_MISSING`，不触网、不补齐、不渲染 partial。内部 allowlist 仅强制 verified-source filter，任何未来 public wrapper 必须由 `MarketDataAccessGrant` 派生并绑定 policy/access-grant descriptor。四个 B2 family 继续 `DATA_FAMILY_UNCONFIGURED`，不得添加 HTTP/provider/frontend/scheduler。 | `test_multi_record_contracts.py`、`test_multi_record_evidence.py`、`test_multi_record_evidence_models.py`、`test_multi_record_evidence_migration.py`、`test_multi_record_durable_reader.py`、`test_multi_record_identity.py`、`test_multi_record_completeness.py`、`test_multi_record_store.py`、`test_multi_record_query_service.py`、`test_multi_record_migration.py`、`test_storage_models.py`、`test_store.py`、`test_dataset_contracts.py`；L-197-36、L-197-37 | `PASS`（内部 SQLite/local-development）；真实 MySQL/PostgreSQL migration/writer concurrency/scale、真实 provider-backed source/zero assertion、grant-derived public authorization、页面/API/数据库三方与生产验收均 `NOT_RUN / NO-GO` |

**AC-197-036 判定边界：**本项验收可复核的 internal local persistence/read/evidence mechanism，不把它描述成任一 B2 产品的公开可用性。四个 versioned dimensions、server-derived expected manifest、durable zero receipt、source-event proof 和 receipt-bound strict reader 已在 SQLite/local-development 验证；但 family-specific provider semantics、真实 source completeness/zero assertion、许可证、`MarketDataAccessGrant` public authorization、provider adapter、route、scheduler、公共 RBAC 或页面状态均不在本项成功范围。MySQL current read 使用精确 `(event,key)` locking predicate 并有单 chunk 行数上限，但真实 MySQL REPEATABLE READ 写者等待、锁范围、50,000 record batch 和历史 revision 压力仍需独立验收。


`AC-197-032` 本次基础设施收据：延期发布/迁移/bootstrap/迁移图复审焦点为 `32 passed, 9 warnings`，普通 Store/recovery 为 `49 passed, 1 warning`，最终 `tests/market_data_platform` 为 `897 passed, 102 warnings`（278.13 秒）。其读取断言只覆盖未经 source-authorization 筛选的规范化 Store，因此不改变该 AC 对真实来源、真实库、v2、页面和策略的 `NOT_RUN / NO-GO` 判定。

**AC-197-031 判定细则（适用于当前实现候选；本次冻结本地回归为 `PASS`，真实 provider/数据库/浏览器/生产验收仍为 `NOT_RUN / NO-GO`）：**

1. server registry 只可解析 `{stock.realtime: market-data-family-v1}` 或 `{stock.kline_legacy: market-data-kline-v1}` 中与目标 family 精确匹配的一对；旧 v1 token、cursor、contract 和 binding 必须仍按旧 pair 验证，不能转换或重签。private K 线 pair 只能由 server-owned bridge 使用，public `/queries`、`query-contract` 与 bundle 均不得选择、签发或暴露它。legacy token 只可逐字符匹配唯一 current/published frozen stock identity 的 `display_symbol` 或 `details.exchange_symbol`；两字段并集的多 canonical collision、pending/历史 revision 或大小写不等均失败关闭。任何自由、跨 family、private public-input 或错配 pair 都必须失败关闭。
2. `daily` 的唯一目标窗口是 `[start@00:00 Asia/Shanghai, (end+1 calendar day)@00:00 Asia/Shanghai)` 转换后的 UTC 半开窗口，且不超过 366 日历日；`weekly` 只接受周一 start、周日 end，唯一目标窗口是同一 `[start@00:00 Asia/Shanghai, (end+1 calendar day)@00:00 Asia/Shanghai)` 形式的完整周并映射 `1w`，最多 260 个完整周；`monthly` 只接受月初 start、月末 end，唯一目标窗口是 `[start@00:00 Asia/Shanghai, (end 所在月的下月 1 日)@00:00 Asia/Shanghai)` 形式的完整月并映射 `1mo`，最多 120 个完整月。任何不符合上述 profile 的日期、`period`、范围或分页输入为 422；首个 v2 请求之外的 cursor、底层 cursor 或 partial page 不是可恢复输入，必须为 503。
3. 唯一成功结果先由持久化后的 `local_only` reread 证明 `coverage=complete`、`expected_event_keys == accepted_event_keys`、无 missing key、无 coverage gap、每个 accepted event 都在 sealed window 内，并证明无重复、按 canonical 顺序的 response event-key 序列与 `coverage.accepted_event_keys` 精确相等，再投影。dates 必须为严格递增、无重复的 `Asia/Shanghai` `YYYY-MM-DD`；每个 accepted event 恰好一行，两个 accepted event 不得投影为同一日期；`count == len(records) == len(dates) == len(ohlc) == len(volumes)`，并且 `records[i]` 与 `dates[i]`、`ohlc[i]=[open, close, low, high]`、`volumes[i]` 为同一 event 的同一值。显示的 OHLC/change 以 `ROUND_HALF_UP` 生成两位小数 JSON number，change 为百分比点且不能产生 signed zero；该显示投影不得降低存储精度。volume 只接受非负、精确且处于 JSON safe-integer 范围的整数，不能以 `int()` 截断。任何 field-quality、coverage/event 或序列化检查失败均为 503。
...
当前候选有十个 `ready` family：六个 `*.realtime` 的 `market.bars`，以及 `stock.liquidity`、`fund.liquidity`、`fund.nav` 三个日线 `reference_series` 和 `fx.range` 的完整 OHLC 日线。后面四个只在行情页明确选择、server bundle/contract 精确绑定、来源策略匹配时进入候选代码路径；它们没有扩展策略页的 strict bars/PIT 预检。其它七个 B1 family、快照和四个 B2 多记录产品仍为 `unconfigured`。本表的“本地命中/受控缺口补齐”仍是正式验收要求，**不是当前真实环境通过声明**；AC-197-036 已将 B2 事实/selector/local reader 基础标为本地开发通过，但 AC-197-021 的公开产品启用仍为 `PARTIAL / NO-GO`，直至 family/source/authorization 和真实端到端证据完成。
...
5. 对七类资产逐一验证 server-owned strict-bars family 选择。六类已有 bars 资产必须仍为各自的 `*.realtime`；crypto 必须选择 `crypto.range`，不得将 `crypto.realtime / quote_snapshot` 当作 backtest bars。若 `crypto.range` 尚未配置，浏览器必须以 `DATA_FAMILY_UNCONFIGURED` 显示 unsupported，且不调用事实查询或 legacy fallback；服务端必须以 `MARKET_DATA_BINDING_STRICT_BARS_FAMILY_UNCONFIGURED` 在 query-contract、query service、provider、CSV 或 binding artifact 之前拒绝。该离线回归只证明选择和拒绝边界，不证明 crypto OpenBB route、真实数据、持久化、PIT 回放或浏览器 E2E。

L-197-35 进一步记录策略研究执行前的本地开发门控：当 durable query-v2 与 research bridge 均有效时，新建运行、run-record 续跑和 task snapshot 续跑必须先按**冻结提交快照**发起 `local_only + purpose=research + consistency=strict` 预检，只有完整的 `strict_local` 结果才能提交或续跑；未配置、契约/查询失败、响应不匹配、覆盖不完整、能力读取失败及中止都必须在动作 API 前阻断。后端在四条共享研究入口中独立评估 durable capability，桥接有效而请求未带 server-owned binding intent 时必须在 mandate、任务状态、快照、研究服务和 artifact binder 前返回稳定拒绝。该记录只覆盖无网络 fixture 与本地组件/API-route 回归，不能替代真实 capability ledger、真实 PIT 数据、浏览器/API/数据库三方、部署切换围栏或生产验收。
...
3. 设置经运维审核的四段 `OPENBB_MARKET_DATA_RUNNER`、精确 `OPENBB_MARKET_DATA_RUNNER_PROTOCOL=openbb-market-data-v2`、恰为 `yfinance` 的 `OPENBB_ALLOWED_PROVIDERS`、独立绝对存在的 runner `HOME` 与 `OPENBB_RUNNER_WORKDIR`。记录实际 `cwd`、镜像 digest、允许 provider、挂载和启动命令哈希，但不记录密钥。逐次真实请求前还须记录固定派生 `--protocol-self-check` 的无网络 v2 receipt 摘要及其 ACK cleanup 成功，证明声明没有替代 transport 证据；任何 HOME/workdir 临时回退、主应用工作树、主应用 HOME、缺失/旧 protocol 声明、失败预检或秘密挂载均失败关闭。
...
迭代 196 已冻结并作为本候选的基线。迁移图以 `20260909_ai_research_market_data_merge` 显式合并 196 研究审批 head 和 197 数据中台 head，随后依次追加 `20260909_market_data_research_bindings`、`20260909_market_data_research_binding_consumers`、`20260910_market_data_shared_source_payloads`、`20260910_market_data_capability_ledger`、`20260911_market_data_deferred_publications`、`20260911_market_data_semantic_record_keys` 与 `20260911_market_data_b2_completeness_evidence`。本地 `alembic heads` 已证明唯一 head；它不能替代可恢复 MySQL/PostgreSQL 副本或真实页面环境。
...
- [ ] 所有已定义的 AC-197 条目（当前至 AC-197-036）均有对应证据；开发回归、候选验收和真实验证的边界清楚可查。
- [ ] AC-197-031 已证明受限 family/version coexistence（保留 `{stock.realtime: market-data-family-v1}`，仅新增 `{stock.kline_legacy: market-data-kline-v1}`，既有 v1 token/binding 不转换）、public 入口拒绝 private pair、`Asia/Shanghai` daily/weekly/monthly 标签和 366 日/260 完整周/120 完整月边界、完整 OHLCV + `change_pct` profile、`coverage=complete`、expected/accepted event-key 精确相等且无 missing/gap、统一 field-quality、首请求无 cursor/单页完整窗口，以及 dates/records/arrays 的一一对应、`ROUND_HALF_UP` 两位小数 JSON number/百分比点 change/no signed zero/nonnegative exact safe-integer volume 序列化。authorized capability→exact `qfq` contract→v2 `local_first`→receipt/publication→本地 reread→legacy projection 闭环均已在冻结候选和独立验收库证明；handler/bridge 的 direct-AkShare、`MarketInstrumentService` 与遗留专表旁路均不可计入。
- [ ] AC-197-029/030 的预捕获批次与共享原始载荷边界已经按记录执行；在公开 `stock.valuation` 开通前，真实来源、日线 calendar、数据库、页面和策略闸门均有独立证据。
- [ ] AC-197-032 在独立验收库证明受控遗留表导入的逐 target current authorization/lease、物理 schema/read-authorization/source-batch/import-scope/source-receipt 回执、写前 permit、逐 bar 的唯一 revision/source binding、每个 publication receipt 不早于 Store local receipt、Store v2 `revision_key_sha256` 对 `provider_available_at` 的封存与重建验证、v1 `source_available_at=None` 的 adapter 拒绝、source/local availability 分离、quarantine/deferred publication、canonical publication 和 strict local-only PIT reread；在此之前 `STOCK_ZH_A_HIST` 不得成为任何 v2、页面或策略的已验证来源。
- [ ] AC-197-036 在冻结候选和可恢复 MySQL/PostgreSQL 副本证明 semantic-record-key migration 的 writer drain/backfill/恢复、精确 `(event,key)` writer 锁范围与规模、family-specific dimensions/expected manifest/durable zero receipt、grant-derived public authorization、provider → Store → strict PIT replay 和浏览器/API/数据库三方链路；在此之前四个 B2 family 保持 `DATA_FAMILY_UNCONFIGURED`，内部 local reader 不得被接入 public route。
