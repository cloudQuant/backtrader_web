...
本次交付包含方案文档、隔离工作树中的实现候选和本地自动化验收。仍不安装或启用真实 OpenBB 扩展、不执行共享数据库迁移、不抓取真实数据、不打开生产开关、不合并到共享分支，也不把局部测试当作生产验收证据。
...
| 工作包 | 目标 | 与 196 的关系 |
| --- | --- | --- |
| WP-197-01 | 冻结 `MD-197-SCOPE-MANIFEST`、数据目录、精确身份、日历和 source policy。 | 候选实现与离线验证完成；页面基线版本须记录。 |
| WP-197-02 | 建立 canonical series、来源快照、revision、visibility receipt、PIT/coverage/cursor 和授权模型。 | 候选实现与离线验证完成；可在隔离数据库验证。 |
| WP-197-03 | 建立 AkShare route registry、OpenBB JSON runner、幂等/lease/fencing 与本地回读。 | 候选实现与离线验证完成；真实 route/许可按验收逐项启用。 |
