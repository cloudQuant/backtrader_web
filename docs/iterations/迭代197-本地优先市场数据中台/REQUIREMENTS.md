...
- 旧 `/api/v1/data/kline` 当前仍在路由内直接调用 AkShare，尚未接入本地优先、`data:read`、capability ledger、精确 contract、lease、receipt 或写后本地复读链路。它是本迭代已知的未治理兼容缺口，不能被标记为 v2、local-first 或已验收。保留路由注册不等于允许它继续成为在线旁路；其替换必须满足 FR-01A 与 AC-197-031，完成前不得将现有直接 AkShare 行为扩展到任何页面、策略或新调用方。
...
`/api/v1/data/kline` 的兼容响应可以保留 `symbol`、`count`、`kline.{dates,ohlc,volumes}` 与 `records` 的既有 JSON 形状，但取得和投影数据的路径必须更换为受治理的 v2 闭环。为避免把当前 `stock.realtime` 的仅 `close` contract 静默升级为 OHLCV，本迭代的**拟议实施设计**选择新增 server-owned `stock.kline_legacy` family，并为该 family 拟议 `family_contract_version=market-data-kline-v1`；不得原地扩大已有 `stock.realtime` 的字段承诺或让客户端自行选择 family。当前代码尚未配置该 family 或该 contract version，故本条不是现有能力声明。

当前 v2 wire schema 的 `MarketDataFamilyContractVersion` 只允许全局 `market-data-family-v1`，registry 也硬比较该常量。正式实现前必须把这项全局限制迁移为受限、server-registry 约束的 `(family_id, family_contract_version)` pair：至少永久保留 `{stock.realtime: market-data-family-v1}`，并且只为拟新增 family 允许 `{stock.kline_legacy: market-data-kline-v1}`。`query-bundle` 与 `query-contract` 只能签发 registry 中的正确 pair；不得接受自由字符串、客户端版本选择或把 `market-data-kline-v1` 用于其他 family。既有 v1 token、contract 和 binding 不转换、不重签为新 pair、也不放宽其比较；任一错配必须在控制面、事实、provider I/O 前以 HTTP 503 失败关闭。
...
legacy projection 是一个完整性断言，不是对 v2 行的格式转换：在持久化后的 v2 `local_only` reread 中，先拒绝响应 observation 自身的重复 event key，再按 contract 的规范顺序排序，并与 `coverage.accepted_event_keys` 做逐项精确相等比较；任一缺失、额外、重复或排序后不相等均不得投影。`next_cursor=null` 只能说明当前响应没有续页，不能证明 legacy 窗口未被截断；桥接的首个 v2 请求必须显式无 cursor，且该单次响应覆盖完整 server-owned legacy window。`open`、`high`、`low`、`close`、`volume`、`change_pct` 必须通过同一 field-quality 规则：拒绝布尔值、`NaN`、正负无穷、空/纯空白文本和其他不可用数值，不能依赖 Python/JSON 的隐式转换或因质量问题保留部分行。

投影在存储之后执行，不得为显示精度改写 observation。每个 accepted event 恰好投影一行；其 event timestamp 转为 `Asia/Shanghai` 后生成严格递增、无重复 `YYYY-MM-DD` 的 `dates`，两个 accepted event 不得投影为同一日期。`count == len(records) == len(dates) == len(ohlc) == len(volumes)`，且同一索引上的 `records[i].date == dates[i]`，`records[i]` 的 open/close/low/high/volume 与 `ohlc[i]=[open, close, low, high]`、`volumes[i]` 相同。OHLC 与 `change` 必须按 server-owned 的既有两位小数 JSON number 投影；`change` 的单位为百分比点，`1.23` 表示 `+1.23%`，不得写成 `0.0123`。`volume` 必须为非负、语义上精确的整数，并处于 server-owned JSON safe-integer 范围 `0..9007199254740991`；不得通过 `int()` 或等效截断把小数、布尔值或超范围值伪造成成交量。任一序列化不变量或 field-quality 违反均为 HTTP 503。

正式桥接按以下顺序执行，且每一步都由服务端控制：

1. 先构建 current principal 并强制 `data:read`；无权限为 HTTP 403，且不得读取 capability、catalog、identity、calendar、事实或 provider。
2. 读取 effective v2 capability。缺失、关闭、过期或格式无效的 query capability 一律为 HTTP 503；不得回退当前 `/kline` 的直接 AkShare 调用、遗留 warehouse 或 `MarketInstrumentService`。
3. 由服务端将 legacy display token 解析为唯一、逐字符匹配的 frozen identity，并签发上述 exact `qfq` contract。不能从后缀、市场、代码前缀或邻近标的猜测 identity；缺 family、contract、route、完整字段合同或精确 binding 时为 HTTP 503。
