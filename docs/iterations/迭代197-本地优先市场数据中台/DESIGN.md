...
`B2SliceSelector` 与 `B2ReportSelector` 是纯领域 completeness contract：调用方必须声明 selector dimensions 和预期 semantic-key hash 集合，规划器才会给出 `COMPLETE`；缺失、重复、额外、未声明、空结果或类型不精确 selector 匹配均为 `INCOMPLETE`。空结果只能由绑定 selector digest 与 `event_at` 的 zero-record certificate 证明；本轮 certificate 只是内存领域对象，没有 durable receipt，`MultiRecordLocalQueryService` 不接受它，因此空切片仍不能返回 complete。selector manifest 单独受 4 MiB/50,000 key 上限约束，不能借单个 record-key 的 16 KiB 限制制造大对象。

内部 `MultiRecordLocalQueryService` 只读取已经持久化且在冻结 visibility anchor 内可见的 Store facts，强制 `local_only + strict`、精确 `knowledge_cutoff` 和单一 `event_at`。它将 `exact_event_at` 传至 Store SQL，避免先读取整个 context 时间窗再丢弃其它 event；selector、series semantic identity、PIT anchor、access-binding digest、最后 record coordinate 和原始 cursor 过期时间均被 HMAC cursor 绑定。任何 incomplete 结果返回空 observations 和无 cursor，不能触发 AkShare、OpenBB、lease 或网络回退。其非空 `allowed_source_registry_ids` 只确保 Store 走 verified-source filter；它不是授权器。未来公开 wrapper 必须从已验证的 `MarketDataAccessGrant` 派生该 allowlist，并把 policy/access-grant descriptor hash 一同绑定，不能直接接受或构造这个内部 DTO。
