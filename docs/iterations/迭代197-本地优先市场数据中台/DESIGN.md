...
#### 3.2.1 多记录本地事实基础；公开产品仍为 NO-GO

`20260911_market_data_semantic_record_keys` 已把 `md_observation_revisions` 从仅按 `(series_id, event_time, revision_number)` 编号的单记录形状扩展为可保存同一 snapshot/report date 多条事实的**本地基础**。其后继 `20260911_market_data_b2_completeness_evidence` 新建 append-only 的 `md_b2_completeness_receipts` 与 `md_b2_completeness_manifest_entries`：parent 固定 family/version、selector dimensions/digest、exact event、source snapshot、manifest digest、expected count、可选 zero-evidence digest 和 receipt digest；child 固定每个 expected semantic-key digest。非空 manifest 与 zero evidence 的状态互斥，二者均由 receipt hash 和独立 `b2_completeness_receipt` publication 保护。它仍不是对四个 B2 产品的公开启用：`futures.inventory`、`option.derivative`、`option.risk_surface` 和 `crypto.cme_position` 仅有 `unconfigured` registry entry，仍没有可执行的 public family binding、source policy、provider route、HTTP DTO、页面、scheduler 或 capability permit，公共 resolver 继续返回 `DATA_FAMILY_UNCONFIGURED`。

每条 observation 现在有服务端拥有的 `semantic_record_key` 及其 SHA-256。既有行和所有单记录写入使用固定 singleton canonical JSON/hash；迁移只为缺失键的历史行回填该固定值。非 singleton 键以 `record_identity_contract_version`、精确 `family_id`、精确 `family_contract_version` 和已深度冻结、类型敏感的业务 dimensions 构成 canonical JSON。`true` 不等于 `1`、`1` 不等于 `1.0`、`-0.0` 不等于 `0.0`，嵌套 mapping/sequence 也按相同 canonical JSON 比较；不做 strike rounding、别名、大小写或其它产品语义猜测。`source_record_key` 仍只是上游 trace metadata，绝不能作为事实身份或绕过该键。

数据库唯一约束改为 `(series_id, event_time, semantic_record_key_sha256, revision_number)`，并有 `(series_id, event_time, semantic_record_key_sha256, available_at)` 的 PIT 索引。新写入的 V3 revision identity 绑定 semantic-key hash；V1/V2 identity 只可验证 migration singleton，不能被伪造为 B2 行。读取在 `(event_at, semantic_record_key_sha256)` 坐标内选择字段/质量可用的最新可见 revision，输出排序固定为 event、canonical key、revision 和 immutable ID。写入时 Provider DTO 只能给出 dimensions，Store 依 family/contract 自行派生键；B2 family 缺 dimensions、单记录 family 带 dimensions、同一 receipt 重复坐标、超出单键 16 KiB、单批 identity 10 MiB 或超过 50,000 条均在 source receipt 前失败。MySQL/InnoDB 的 ordinal 分配在 series lock 后以精确 `(event,key)` locking read 读取，并对单 chunk 历史行数设上限；真实 MySQL 并发、规模和故障接管演练仍为 `NOT_RUN`。

`B2SliceSelector` 与 `B2ReportSelector` 是纯领域 completeness contract，但 strict reader 不信任调用方给出的 expected hash：issuer 从受审核的 versioned dimensions 重新派生 selector、semantic keys、manifest digest 与 receipt digest，并在事务 A 先校验同一 `source_snapshot_id` 的精确 `event_at` key 集合。随后 receipt、normalized child manifest entries 和 pending `b2_completeness_receipt` publication 一同 durable；事务 B 才使 receipt 可见。恢复、直接发布和读取都重算 parent digest、child manifest、zero state 及 source-event 集合；篡改、待发布、来源 publication 不可见或当前 verified-source 授权失效均 fail closed。空结果只接受服务端散列、绑定 selector digest 与 `event_at` 的 zero evidence，且只能由已发布 durable receipt 重建 `ZeroRecordCertificate`。selector manifest 单独受 4 MiB/50,000 key 上限约束，不能借单个 record-key 的 16 KiB 限制制造大对象。

内部 `MultiRecordLocalQueryService` 只读取已经持久化且在冻结 visibility anchor 内可见的 Store facts，强制 `local_only + strict`、精确 `knowledge_cutoff` 和单一 `event_at`。请求 selector 只作 receipt lookup；Store 要求 B2 receipt 和它绑定的 source snapshot publication 都在同一 anchor 可见、来源仍为当前 verified/allowlisted，然后从 durable parent/child rows 重建 selector、manifest 和可选 zero certificate。reader 只把 `exact_event_at` 与 `exact_source_snapshot_id` 传至 Store SQL，避免先读取整个 context 时间窗、跨来源 dedupe 或把另一 source 的修订拼成完整性。cursor 除 selector、series semantic identity、PIT anchor、access-binding digest、最后 record coordinate 和原始 expiry 外，还 HMAC 绑定 receipt ID/hash 与 source snapshot ID；重查 evidence 漂移稳定拒绝。任何 evidence 缺失返回 `DURABLE_SELECTOR_EVIDENCE_MISSING`、空 observations 和无 cursor，不能触发 AkShare、OpenBB、lease 或网络回退。其非空 `allowed_source_registry_ids` 只确保 Store 走 verified-source filter；它不是授权器。未来公开 wrapper 必须从已验证的 `MarketDataAccessGrant` 派生该 allowlist，并把 policy/access-grant descriptor hash 一同绑定，不能直接接受或构造这个内部 DTO。
...
目录已预注册 `market.valuation`、`market.liquidity`、`market.settlement`、`market.bond_reference`、`market.fund_nav` 与 `market.fx_reference`，并与已有 `market.bars`、`market.quote_snapshot` 共用不可变 revision 存储绑定。此次还注册了私有 `market.stock_valuation_captured_snapshot`：其 schema 明示 `internal_only=true`、`valuation_snapshot / snapshot`、`collector_observed`，并与公开 `market.valuation / reference_series / 1d` 分离。它没有 family、route、freshness、coverage 或 legacy bridge，因此不能由 DTO 扩展或页面改动升级为公开能力。family DTO 只允许已审核的单记录 calendar-grid 或 snapshot-freshness 形状，registry 继续以有限白名单核对 family、dataset、kind、频率、必需字段和 coverage model；所以更改一个 UI 卡片或 DTO 不能把尚未开通的 B1/B2 产品变成可执行请求。

启用任何上述家族前，必须同时具备：

1. 针对每个 family 审核并签发维度 schema、selector/expected-record manifest 与 durable zero-record receipt，不能把当前泛型 normalizer 当作业务字典；
2. 由 `MarketDataAccessGrant`、policy/access-grant descriptor 和 source authorization 构造公开访问绑定，并实现 public route、catalog/contract、capability 和审计；
3. 已批准且 `ready` 的 source policy/family contract、provider adapter、lease/receipt/publication 链，以及“provider → Store → strict local-only PIT replay”覆盖同一 snapshot/report date 多行的真实端到端测试；
4. 在可恢复 MySQL/PostgreSQL 副本执行 writer-drain migration、跨连接 PIT、精确 record-coordinate 并发/规模和降级演练，再进行页面灰度。
...
### 5.1.3 A 股历史日线遗留表受控导入候选

`legacy_stock_daily_import.py` 是未注册的 protocol seam，不是 provider 或 migration。它先以 `LegacyStockDailyReadScopeReceipt` 锁定受 gate 核验的物理 schema、source registry/provider/route 和 read-authorization receipt identity，再将固定八列的 raw source batch 与 `LegacyStockDailyImportScope` 分开哈希。scope 使用 canonical JSON 绑定 logical projection、attestation 的稳定语义、读表前 calendar snapshot/PIT/EventKey map、frozen identity revision/metadata version、`bars/1d/qfq` 合同和 read-authorization receipt identity；raw hash 只绑定所选原始单元格。

source-batch receipt 必须同时回显 raw hash、scope hash、物理 schema hash、route、read-authorization receipt identity 和 extraction time。writer 前会复核这些证据；`extracted_at` 只成为 normalized source bars 的 `source_available_at`。随后 gate 必须为每个 canonical target 签发写前 permit，分别绑定 target、当前 write-authorization descriptor、resolved-context digest、lease key/fence、raw batch hash、scope hash 和 source receipt ID；一个 target 的 context/lease 不能写入 sibling target。canonical writer 的结果还须回显逐 target permit、source-batch/scope/source-receipt digest，以及每条 `(canonical_id,event_at)` 到唯一 revision/source-snapshot 的不可变映射；同一 observation revision 不得被两条 bar 共用，一个 source snapshot 不可跨 target 证明事实，且每个 source snapshot 的 publication receipt `visible_at` 不得早于 Store local receipt。adapter 将 `extracted_at` 作为 `ProviderMarketObservation.available_at` 传入 `MarketDataStore.persist_provider_result`。新写入的 Store v2 revision identity（`market-data-observation-revision-v2`）必须将规范化 UTC `provider_available_at` 封入 `revision_key_sha256`；Store 在返回 `LocalObservationRevision.source_available_at` 前重建并验证该 identity，故把 provenance 改成另一格式合法但更早的来源时间也失败关闭。没有这项封存的历史 v1 identity 只能返回 `source_available_at=None`，future adapter 必须拒绝，不能由其 provenance 猜测来源时间。adapter 同时必须保留传给 `persist_provider_result(received_at=...)` 的可信本地 receipt 作为 `local_observation_available_at`，不能将 `PersistedProviderFetch.received_at` 的 publication 时间当作本地 receipt。写后 protocol 在一个新 visibility anchor 下，以 strict `local_only` reread 对照相同业务字段、source/local availability 和逐 bar revision/source mapping。当前 Store 会在该独立 reread 前完成 publication，因此 concrete adapter 还必须实现 deferred-publication/quarantine，使失败的 import 不会短暂进入产品可读集合。当前没有 concrete reader/gate/writer、`ResolvedMarketDataQueryContext`、`MarketDataSourceAuthorization`、`MarketDataFetchLeaseHandle` 或上述 visibility gate 接线，故该设计为 `NO-GO`；详细接口、负例和 future adapter 步骤见 [受控导入边界](LEGACY_STOCK_DAILY_IMPORT_GUARD.md)。

### 5.2 OpenBB

`OpenBBSubprocessProvider` 只执行运维配置的命令，不通过 shell 拼接，且只接受恰为“绝对 Python 可执行文件、`-I`、`-S`、绝对 runner 脚本”的四段命令；wrapper、模块模式、相对路径与额外参数均在父进程拒绝。配置该命令时，父进程侧的非秘密声明 `OPENBB_MARKET_DATA_RUNNER_PROTOCOL` 必须精确为 `openbb-market-data-v2`；它不转交 runner，缺失、v1 或未知值在任何 `Popen` 前稳定拒绝为 `OPENBB_RUNNER_PROTOCOL_UNSUPPORTED`。该声明只是部署约束，不是 transport 证据：每次正常请求前，父进程从已验证四段命令内部派生唯一的 `--protocol-self-check` 预检，使用关闭的 stdin 和父进程创建、仅为该 child 继承的 ACK fd。预检在不读 manifest、不导入 OpenBB、不访问网络的分支输出精确 v2 JSONL receipt（protocol、self-check 和 transport version、`ready`），随后等待 ACK；父进程在 receipt 后按 kill → reaping/waitpid → bounded drain → `killpg(..., 0)` 确认的顺序结束其独立 session。预检超时、旧 `json.load(stdin)` peer、无效/非 v2 receipt 或清理失败均为 `OPENBB_RUNNER_PROTOCOL_UNATTESTED`，且正常请求不启动。固定派生 flag 不是用户或运维可加入命令的参数，四段命令契约不变。正常协议包含版本、关联请求 ID 和完整请求 DTO；运行器必须回显它们。Web 进程对超时、非零退出、超大输出、无效 JSON、错配 ID、重复/越界事件全部拒绝。
...
```mermaid
flowchart LR
  L[GET /api/v1/data/kline] --> A[data:read principal]
  ...
  W --> R
  R --> V[accepted events + quality + serialization + single page]
  V --> O[legacy JSON projection]
```
...
启用时，`/ai-research/run` 在创建任何 workspace 前绑定，`/ai-research/tasks` 在生成 task ID 后、写入 request snapshot 或派发后台任务前绑定。服务端按资产意图先选择 strict-bars family：`stock`、`futures`、`bond`、`fund`、`option`、`fx` 为各自 `*.realtime`，`crypto` 为 `crypto.range`；随后才重新解析 exact contract、当前 `data:read` 权限和 source registry，构造唯一允许的 `local_only + backtest + strict + knowledge_cutoff` 查询。若映射 family 不为 `ready`、不是 `bars` 或不支持所选周期，绑定在 contract、provider、CSV 和 binding artifact 写入前失败关闭（当前 crypto 返回 `MARKET_DATA_BINDING_STRICT_BARS_FAMILY_UNCONFIGURED`）；它既不降级到 `crypto.realtime` quote snapshot，也不尝试 AkShare/OpenBB。它要求完整覆盖、无 fetch、无分页和完整有限的 OHLC；不完整本地数据绝不触发 AkShare/OpenBB。
...
196 的冻结候选已作为集成基线。`20260909_ai_research_market_data_merge` 使用两个 `down_revision` 显式合并 196 研究审批链与 197 数据中台链；`20260909_market_data_research_bindings` 创建 binding receipt，`20260909_market_data_research_binding_consumers` 继续创建 scope、consumer 和 revocation receipt，`20260910_market_data_shared_source_payloads` 再只创建 shared payload 与子引用表，`20260910_market_data_capability_ledger` 创建独立的 append-only capability ledger，`20260911_market_data_deferred_publications` 新增受控 legacy-import receipt 的 release-hold 子表，`20260911_market_data_semantic_record_keys` 以固定 singleton 回填历史 observation 并替换事实唯一约束，`20260911_market_data_b2_completeness_evidence` 最后追加 B2 receipt/manifest evidence 表。不得任选一个历史 head、`stamp` 掉另一个分支或直接对生产库运行旧独立链。shared payload revision 不回填历史内联 receipt，也不重建 `md_source_snapshots`；其 downgrade 只有两张新表均为空时允许。fetch lease 与 capability ledger migration 都不会重置或重解释既有 evidence；capability ledger 的 SQLite/PostgreSQL downgrade 仅在 ledger 表为空时允许，而 fetch lease/capability ledger 的 MySQL downgrade 因无法原子地证明空表均始终拒绝。release-hold downgrade 也必须在 hold 表为空时才允许；semantic-record-key downgrade 只允许在事实表为空时执行，MySQL 因无法原子证明空表而拒绝；B2 evidence downgrade 只有 receipt 与 manifest 两表均为空时才允许，MySQL 同样 fail closed。候选发布仍须在空数据库执行 `alembic heads`（恰一个 head）和 `alembic upgrade head`，再在可恢复的 MySQL/PostgreSQL 副本做同样演练；详情和证据格式见 [验收文档](ACCEPTANCE.md#7-数据库迁移与灾备验收)。

### 7.2 发布前操作顺序

1. 复核 196 冻结基线、建立 196/197 集成候选并完成单 head 迁移修订；记录候选 SHA、`git status --short`、`alembic heads` 和备份标识。
2. 在空库和经批准的可恢复副本执行 `alembic upgrade head`；审计 `dg_*`/`md_*` 的列、索引、外键、检查约束、时间字段与遗留 AkShare 表的行数/校验和。MySQL fresh upgrade 前必须先 drain market-data writer，并在同一受控命令设置 `MARKET_DATA_SHARED_BINDING_MAINTENANCE_FENCE=confirmed`、`MARKET_DATA_VISIBILITY_ANCHOR_MAINTENANCE_FENCE=confirmed`、`MARKET_DATA_SOURCE_RECEIPT_EVIDENCE_MAINTENANCE_FENCE=confirmed`、`MARKET_DATA_SOURCE_GOVERNANCE_MAINTENANCE_FENCE=confirmed`、`MARKET_DATA_FETCH_LEASE_MAINTENANCE_FENCE=confirmed`、`MARKET_DATA_EXACT_IDENTITY_MAINTENANCE_FENCE=confirmed`、`MARKET_DATA_CONSTRAINT_NAME_PORTABILITY_MAINTENANCE_FENCE=confirmed`、`MARKET_DATA_SHARED_SOURCE_PAYLOAD_MAINTENANCE_FENCE=confirmed`、`MARKET_DATA_CAPABILITY_LEDGER_MAINTENANCE_FENCE=confirmed`、`MARKET_DATA_PUBLICATION_RELEASE_HOLD_MAINTENANCE_FENCE=confirmed`、`MARKET_DATA_SEMANTIC_RECORD_KEY_MAINTENANCE_FENCE=confirmed` 和 `MARKET_DATA_B2_COMPLETENESS_EVIDENCE_MAINTENANCE_FENCE=confirmed`。这些有界 `GET_LOCK` 只串行化迁移运行，不能替代停止 writer；fetch lease/capability ledger 的 MySQL downgrade 都直接拒绝，非空 release hold、非空事实表或非空 B2 receipt/manifest 的 downgrade 也拒绝。审计四个身份字段实际为 `utf8mb4_bin`，PostgreSQL 为 `C`，再验证每个应用连接的 UTC session time zone 与跨连接 PIT 读取。
...
必须记录但不暴露敏感值的指标包括：本地命中率、按 `(market, data_kind, frequency)` 分组的日历未知率和 `CALENDAR_GRID_UNAVAILABLE`、每提供方请求/失败/延迟、写入行数、质量拒绝原因、因字段集选择旧但完整修订的数量、索引回填进度、来源策略或用途拒绝、provider 活动预检拒绝、receipt/request 错配、shared payload 新建/复用、内容完整性冲突、B2 receipt/manifest/source-event integrity rejection 与 `DURABLE_SELECTOR_EVIDENCE_MISSING`、冻结游标读取、singleflight leader/follower 数量、fetch-lease acquire owner/follower/conflict、expiry takeover、fence lost、release lost 与数据库 UTC clock 失败，以及 OpenBB 协议、静态构件未认证、空 permit、日对齐/3650 天窗口拒绝、原始载荷 hash 和受控 HOME/工作目录失败。数据质量告警以稳定机器码聚合，而不是解析异常文本。
