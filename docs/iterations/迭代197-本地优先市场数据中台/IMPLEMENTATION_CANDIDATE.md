...
逐 route 的 `declared / installed / verified / authorized / effective` capability ledger 仍未落库；目前的静态 policy、provider active 与授权不是安装/验证证据。因而所有 online 路由继续默认关闭，OpenBB permit matrix 继续为空；该账本及其 immutable evidence、generation fence、receipt digest 绑定是未来启用任何在线 route 的前置迁移，不能用环境变量、candidate artifact 或测试替身绕过。`MarketDataQueryService` 的 lease 注入 seam 只用于受控测试组合，生产 API 由 `MarketDataStore.fetch_lease_manager()` 组成；下一轮应收紧该构造边界。旧 raw AkShare helper 图仍留在 `MarketInstrumentService` 内但已无 public `lookup` 调用边，未来必须删除、隔离或以同一拒绝边界封住，不能重新接线为 online fallback。
