...
> 记录日期：2026-09-10<br>
> 文档性质：当前迭代 197 实现候选审计，不是生产发布证明。<br>
> 设计基线：迭代 196 已冻结并已接入 `dev`；本候选尚未进入 release 签收，本文件不解除真实数据、数据库、浏览器和部署验收闸门。

## 1. 阅读规则和状态含义

本文件把当前迭代 197 候选中已经存在的实现，与尚未完成的真实环境验收明确分开。`DONE` 只表示候选代码已具备；只有记录了本次命令和输出的定向自动化才可作为本地回归证据，二者都不表示迁移、真实来源、生产数据库、浏览器 E2E 或策略工件验收已经完成。

`/api/v1/data/kline` 已使用受治理 legacy bridge，保留旧 JSON 形状但不再在 handler 或 bridge 中直接 import/call AkShare。它先执行 `data:read`，以 server-owned `stock.kline_legacy / market-data-kline-v1` exact `qfq` contract 进入 `local_first`，缺口只经批准 route、lease、receipt 与 A/B publication 补齐，随后以同一 sealed request 的 `local_only` reread 做 coverage、event、field 与 serialization fail-closed 检查后投影。`MarketDataFamilyContractVersion` 和 registry 已按受限 pair 共存：保留 `{stock.realtime: market-data-family-v1}`，只为 `stock.kline_legacy` 允许 `market-data-kline-v1`，既有 v1 token/binding 不转换。该 family 是私有 bridge 输入，不出现在 public bundle，`POST /api/v1/data/queries` 与 `query-contract` 均不能选择或签发它。候选代码完成不等于真实 provider/数据库/浏览器/生产验收；这些外部证据仍是 `NOT_RUN` / `NO-GO`。

| 状态 | 含义 |
| --- | --- |
| `DONE` | 当前候选已有实现；本地测试证据必须逐行写明已执行命令与输出，否则标为 `PENDING`。仍可能有外部验收未运行。 |
...
| legacy `/api/v1/data/kline` 受治理兼容桥 | `DONE`（候选代码；L-197-25 本地回归已记录） | 私有 `stock.kline_legacy / market-data-kline-v1` 已与 `{stock.realtime: market-data-family-v1}` 共存，且旧 v1 binding 不转换。仅 server-owned bridge 可解析 K 线 pair；public `/queries`、`query-contract` 与 bundle 不能选择或暴露它。legacy token 必须逐字符匹配唯一 current/published frozen `display_symbol` 或 `details.exchange_symbol`，多 canonical collision、pending/历史 revision 与大小写漂移均拒绝。bridge 强制 `data:read`、严格 `Asia/Shanghai` daily/weekly/monthly window、`local_first → receipt/publication（仅获批缺口）→ local_only reread`，并对 coverage/event/field/serialization 失败关闭。它不复用仅保证 `close` 的 `stock.realtime`，不以 `MarketInstrumentService`、遗留专表或 provider DTO fallback，也不在 handler/bridge 直接 import/call AkShare。 | `NOT_RUN / NO-GO`：本地 `282 passed, 3 warnings` 定向回归已记录；尚无真实 provider、独立验收数据库、浏览器或生产证据，且独立 local hit/受控补齐/local-only reread 仍待完成。 |
...
| F2 quote/valuation/settlement/NAV/reference | `IN_PROGRESS` | `stock.liquidity`、`fund.liquidity`、`fund.nav` 和 `fx.range` 已具 request-time 候选 contract/route；其中 NAV 仅为 CN ETF 日线源报告净值。A 股估值已落地为默认关闭的私有 `market.stock_valuation_captured_snapshot / valuation_snapshot / snapshot` 离线批次 collector：无 fetch/HTTP/route/public family，精确 capture instant、封装 hash、quarantine 与写前证据预算均有本地回归。公开 `stock.valuation` 仍未配置。其余 quote、valuation、settlement、宽表 importer 保持 fail-closed；四个 B2 多记录 family 只有内部 local record-key foundation，公开路径仍 fail-closed。 | `NOT_RUN`：没有真实采集、回填或页面灰度验收。 |
| B2 多记录本地事实基础 | `DONE`（内部基础） | `semantic_record_key`、`(series,event,key,revision)` 唯一性、V3 revision seal、selector completeness、exact-event strict local reader 和 HMAC PIT cursor 已实现。非空 source allowlist 只强制 Store verified-source filter；未来 public wrapper 必须从 `MarketDataAccessGrant` 派生它并绑定 policy/access-grant descriptors。四个 B2 family 均只有 `unconfigured` registry entry，没有 ready public family binding、source policy、provider、route、scheduler、页面或 public DTO。 | `NOT_RUN / NO-GO`：未执行真实 source、MySQL/PostgreSQL、跨进程并发/规模、浏览器/API/数据库三方或 public authorization 验收。 |
| legacy `STOCK_ZH_A_HIST` A 股日线导入边界 | `IN_PROGRESS / NO-GO`（L-197-32 协议回归；延期发布基础设施本地候选） | 候选实现固定八列投影、读前物理 schema/read-authorization 回执、raw batch/import scope 双 hash、calendar PIT/identity revision/metadata version 绑定、逐 target authorization/lease permit（同时锁定 batch/scope/source receipt）、Store v2 `revision_key_sha256` 对规范化 UTC `provider_available_at` 的封存和重建验证、v1 `source_available_at=None` 的 adapter 拒绝、逐 bar 的唯一 revision-to-source-snapshot evidence、每个 publication receipt 不早于 Store local receipt 及写后 local-only reread 协议。新建 `md_publication_release_holds` 后，显式 deferred staging 会在事实事务内同时持久化 receipt 与 hold；通用 publish/recovery 不得释放 active hold，guarded promotion 与 quarantine 才能改变其可见性，bootstrap 也要求该表已迁移。历史表的 Eastmoney、Tencent 与 trends fallback 混合来源无法由表名或 attestation 自动升级为 AkShare 事实。详见 [受控导入边界](LEGACY_STOCK_DAILY_IMPORT_GUARD.md)。 | `NOT_RUN / NO-GO`：没有 concrete gate、当前 source authorization、lease、canonical writer、真实库/来源/页面验收。延期发布测试只证明未经 source-authorization 筛选的规范化 Store 读取；它不是 v2、页面或策略已授权读取证据。 |
...
“当前候选”描述 `market-data-family-bundle-v1` 的实际控制面状态。`*.realtime` 中的 `DONE（bars 兼容）` 只代表有 `market.bars` 的日/周/月 K 线兼容桥，不代表该页面已经有真实 quote snapshot。私有 `stock.kline_legacy` 故意不在这张 21 个公共页面 family 表或 public bundle 中；它只服务于旧 `/api/v1/data/kline` bridge。
...
| --- | --- | --- | --- | --- |
| `stock.realtime` | `DONE`：`market.bars` / `bars` / 1d、1w、1mo；仅 `close` contract；完整 OHLCV legacy `/kline` 仅由私有 `stock.kline_legacy` bridge 使用 | `market.quote_snapshot` / snapshot | `NOT_CONFIGURED` | 实时逐笔/盘口或源 tick 时间，以及将 private K 线 family 变成页面可选 public family。 |
...
| `futures.inventory` | `NOT_CONFIGURED`：`market.inventory` / inventory report | 仓单、库存、交割数量 | `IN_PROGRESS`：仅 B2 本地 facts/selector reader | 将不同来源库存/仓单拼成一条报告，或把内部 reader 当公开授权。 |
...
| `option.derivative` | `NOT_CONFIGURED`：`market.option_chain` / snapshot | chain、IV、OI、strike、expiry | `IN_PROGRESS`：仅 B2 本地 facts/selector reader | 以单合约或宽表的部分字段宣称全链，或把内部 reader 当公开授权。 |
| `option.risk_surface` | `NOT_CONFIGURED`：`market.option_risk_surface` / snapshot | IV、Greeks、model version | `IN_PROGRESS`：仅 B2 本地 facts/selector reader | 用单个 IV 或无模型版本值构造风险面，或把内部 reader 当公开授权。 |
...
| `crypto.cme_position` | `NOT_CONFIGURED`：`market.position_report` / report | long、short、net、OI | `IN_PROGRESS`：仅 B2 本地 facts/selector reader | 将 CME 比特币成交量报告称为持仓报告，或把内部 reader 当公开授权。 |
