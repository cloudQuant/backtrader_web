...
本次交付包含本目录中的需求、设计、验收文档，以及独立 197 工作树中的实现候选和本地自动化验收。候选实现不得修改迭代 196 的未提交工作树，也不得合并到 `dev`/`master`。

以下仍不在本次本地候选验收范围内：真实 AkShare/OpenBB 抓取、真实 OpenBB extension 或许可证/凭据启用、共享 MySQL/PostgreSQL 迁移、真实数据回填、浏览器灰度、生产开关启用、196 研究/回测工件绑定和部署。它们必须按验收文档中的 `NOT_RUN` 或 `BLOCKED` 条目另行执行，不能由 fixture、静态审计或候选测试替代。

### 2.4 当前候选的范围真相

候选实现已经登记七类资产的 21 个页面数据族，并以稳定 `family_id + family_contract_version` 将 bundle、精确 query contract、查询指纹和响应回显绑定。它不把“已登记”误写成“已取到真实数据”：当前只有经审核的 `market.bars` K 线兼容产品可为 `ready`，公开 quote、期权链、报告、估值和参考序列仍保持 `NOT_CONFIGURED`，直到各自的来源策略、身份映射、字段、时间语义和真实来源验收完成。

受控宽表 snapshot importer 仅接收调度或 shadow collector 已捕获的行集并验证后落库；它不是页面请求时的联网适配器。页面启用 family-control-plane 后，未配置数据族不能由 legacy 路径猜测补齐；控制面关闭时保留的裸 bars contract 只是旧兼容灰度路径，不能作为 21 个数据族已经覆盖的证据。
