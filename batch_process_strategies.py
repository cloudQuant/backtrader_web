...
def generate_simple_md(index: int, key_info: dict) -> Path:
    ...
    """
    # 生成安全的文件名
    safe_name = key_info['title'].replace(' ', '_').replace('/', '_').replace('\\', '_')
    ...
    with open(md_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"    ('risk_per_trade', 0.02),  # 每笔交易风险比例\n")
        f.write(")\n")
        f.write("```\n")
        f.write("\n---\n\n")
        ...
        f.write(f"\n")
        f.write(f"class {safe_name.replace(' ', '').replace('/', '_').replace('(', '').replace(')', '').replace('[', '').replace(']', '')}Strategy(bt.Strategy):\n")
        f.write(f"    \"\"\"\n")
